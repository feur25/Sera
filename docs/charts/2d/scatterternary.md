...
<div data-sp-registry-table="options" data-family="scatterternary"></div>

## Themes

<div data-sp-registry-table="themes" data-family="scatterternary"></div>
...
`x_values` (`list[float]`) — Première composante (A, sommet haut). `y_values` (`list[float]`) — Deuxième composante (B, sommet droit). `z_values` (`list[float]`) — Troisième composante (C, sommet gauche). `x_label` (`str`) — Libellé du sommet haut. `y_label` (`str`) — Libellé du sommet droit. `z_label` (`str`) — Libellé du sommet gauche. `color_values` (`list[float]`) — Valeurs continues pour la variante `"gradient"`.
