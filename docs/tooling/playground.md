# Playground

<style>
.pg-hero{margin:1.2em 0 1.6em;padding:22px 28px;border-radius:12px;background:linear-gradient(135deg,#0f172a 0%,#1e1b4b 60%,#312e81 100%);border:1px solid rgba(99,102,241,.3);position:relative;overflow:hidden}
.pg-hero::before{content:"";position:absolute;top:-50%;right:-5%;width:50%;height:200%;background:radial-gradient(ellipse,rgba(129,140,248,.15) 0%,transparent 65%);pointer-events:none}
.pg-hero h2{margin:0 0 6px;font-size:18px;color:#f5f3ff;font-weight:700;border:none}
.pg-hero p{margin:0;color:#94a3b8;font-size:13.5px;line-height:1.55;max-width:70ch;position:relative;z-index:1}
.pg-pills{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;position:relative;z-index:1}
.pg-pill{padding:3px 10px;background:rgba(99,102,241,.14);border:1px solid rgba(165,180,252,.25);border-radius:999px;font-size:11px;font-weight:600;color:#c7d2fe;letter-spacing:.04em}
</style>

<div class="lang-en">

<div class="pg-hero">
<h2>Live Playground</h2>
<p>Write SeraPlot code and see the output instantly — no installation needed. Powered by the Rust engine compiled to WebAssembly, running entirely in your browser.</p>
<div class="pg-pills">
<span class="pg-pill">⚡ In-browser WASM</span>
<span class="pg-pill">🐍 Python-compatible syntax</span>
<span class="pg-pill">🎨 All chart types</span>
<span class="pg-pill">🔄 Live render</span>
</div>
</div>

</div>

<div class="lang-fr">

<div class="pg-hero">
<h2>Playground en direct</h2>
<p>Écrivez du code SeraPlot et voyez le résultat instantanément — sans installation. Propulsé par le moteur Rust compilé en WebAssembly, tout s'exécute dans le navigateur.</p>
<div class="pg-pills">
<span class="pg-pill">⚡ WASM in-browser</span>
<span class="pg-pill">🐍 Syntaxe Python-compatible</span>
<span class="pg-pill">🎨 Tous les types de charts</span>
<span class="pg-pill">🔄 Rendu en direct</span>
</div>
</div>

</div>
