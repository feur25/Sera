...
Paid via PayPal — click a buy link, pay on PayPal's hosted checkout page, and your license key generates automatically once the payment is confirmed (a signed webhook triggers minting, verified against PayPal's own signature so nothing can fake a payment). The payment service is deployed and live at [Sera-Payment](https://github.com/SeraWorkspace/Sera-Payment) (private); PayPal is mid-setup as of this writing, so a buy link may briefly show a clear "not configured yet" page instead of checkout — that's normal during setup, not a dead link, and resolves itself once done. Prefer crypto (USDC/Polygon, self-custodied) or a manual sale instead? Use the [support channel](../about/support.md).
...
`firehose_push()` is `push()` with the indices computed for you — same rate limiting, same `on_anomaly()` detection, same `record()`/`replay()`, same `.serve()` broadcast over the same binary WebSocket frames every other native chart already streams over. Nothing about the transport is new; what's new is the client: instead of a `<canvas>` redrawn point-by-point on the CPU, Firehose renders through WebGL2 — one GPU buffer upload per animation frame, one `drawArrays` call, no per-point JavaScript work no matter how many readings landed since the last frame.

<small>Honest note: the original chantier sketch called for `wgpu` (Rust compiled to a WebGPU/WebGL client via WASM). SeraPlot doesn't have a wasm32 build pipeline yet, so v1 ships a plain WebGL2 client instead — same GPU-side payoff for this chart (a single buffer upload and draw call per frame, not one JS operation per point), reachable in every current browser without adding a new build target. A `wgpu` client remains a natural next step once the WASM pipeline exists.</small>
...
`.serve()` already streams one chart to everyone who opens it. Sera Live turns that into a room: everyone connected sees each other's cursor moving over the chart in real time, can pin a comment to a specific data point, and can pull the whole group back through a recorded incident together with one click.
...
Open that URL in two browser tabs (or send it to a teammate) — everything lives in one floating toolbar at the bottom, Figma-style: **↖ Select**, **📌 Pin**, **✏ Draw**, **🗑 Delete**, **💬 Comments**, **▶ Replay together**, **🌙 Dark mode**. Click **🌙** any time to flip the whole room — toolbar, panels, board chrome — into a dark theme built for a dim war room or a wall-mounted screen; it remembers your choice per browser and defaults to your OS preference the first time you open the link. Move the mouse in one tab, it shows up live in the other, labeled and colored per person. Click **📌**, then click anywhere on the chart, to pin a note there (or Alt-click a point directly, no button needed); everyone sees it appear instantly, as a real visible label next to the pin, not just a hover tooltip. Click **▶** and every connected tab jumps through `incident.spls` in sync, from the start — reliving what happened at the same pace, together, instead of squinting at a screen-share.

Click **✏** to freehand-sketch straight on top of the chart — circle the bar that's spiking, underline the anomaly, draw an arrow at the thing everyone should look at — broadcast live to the room the same way a pin is, cursor turns into a little pencil so you always know you're in draw mode. A small popover opens right above the toolbar with a real color swatch and a thickness slider — pick red for "this is the problem", pick a thick line to make it stand out, both apply to whatever you draw next. Click **↖ Select** — the cursor turns into a hand, since select mode is also how you pan the room — then click any pin or a stroke's line to select it (it lights up blue; strokes have a generous invisible hit-zone around the thin line so you don't have to click the exact pixel), then **🗑** or the Delete key removes it for everyone. On the board specifically, whichever tool is active takes priority over panning — clicking to place a pin or dragging to draw a stroke never fights the canvas for the same drag. Click **💬** to open a running list of every pin made so far, click one to jump straight to it. A small clock badge at the top shows elapsed time since the room opened, the same for everyone, so "it started 4 minutes ago" is a fact everyone in the room shares instead of a guess. Joining late doesn't lose any of it — pins and drawings made before you connected are replayed to you the moment you join.

Nothing about the wire format is new: cursors, pins, drawings, and replay all ride the exact same WebSocket `.serve()` already opens, using the same broadcast channel `push()` sends chart updates through. Pins are real `bookmark()` annotations under the hood, not a separate throwaway layer — list them later with `chart.bookmarks()` same as ever.

`collab` defaults to `True` on both `serve()` and `stream()`. Pass `collab=False` for the old single-viewer behavior with zero presence broadcast — nothing about you or your cursor ever leaves your machine in that mode.

```python
chart.serve(port=8787, collab=False)
```

<small>Closing a tab drops that person's cursor for everyone else immediately, not after a timeout — the 15-second window only ever catches an ungraceful disconnect (a crash, a lost connection), never a normal close. Pins and "Replay together" are rate-limited per chart (the same token-bucket primitive `set_push_rate_limit()` is built on, just its own budget) — a stray script or an overexcited click can't flood the room, and two people hitting "Replay together" at the same moment only broadcasts the recording once. Access itself still follows `.serve()`'s own model: pass `username`/`password` to keep a link private, same as you would for the data stream itself.</small>

---

## Sera Board

One chart at a time is fine for a single metric. A real incident never is — error rate, latency, queue depth, deploy markers, all at once, related but not the same shape. Sera Board puts every chart you've got on one infinite canvas: a Figma-style sidebar lists them all, each sits in its own frame you drag anywhere, overlap on purpose or not, and scroll to zoom from a full command-center view down to one chart filling the screen — no fixed limit either way.

`serve_board()` is Sera Board joined to Sera Live: it opens the same kind of local server `.serve()` does, and everyone who opens the link shares the room — live cursors moving over the canvas, pinned comments anywhere on it, all through the exact same WebSocket room `.serve()` already uses.
...
Send that link to the team — everything lives in one toolbar at the bottom, Figma-style: **↖ Select**, **📌 Pin**, **✏ Draw**, **🖼 Image**, **🎨 Recolor**, **🗑 Delete**, **💬 Comments**, **🗂 Sessions**, **🌙 Dark mode**. Move the mouse anywhere and it shows up live for everyone else. Pin a comment, drop an image, or draw directly on top of a chart and it glues to that chart's frame — drag the frame around and it follows; place it on the empty canvas instead and it stays fixed in the plane — panning and zooming move it exactly like everything else, it never chases your viewport. Hover a frame (with no tool armed) to turn on that chart's own hover/tooltip interactivity right there, move off and it turns back off, so the board never fights a chart for your mouse.

Click **🖼 Image** (or just drag an image file in from your desktop and drop it anywhere on the board) to pin a screenshot, a photo of a whiteboard, a diagram — anything — right on the canvas; drag it around afterward by grabbing it directly, no tool needed. Click **↖ Select** then click any pin, stroke, or image to select it (it lights up blue): **🎨** opens a color swatch to recolor a pin or a stroke on the spot, **🗑** or the Delete key removes whatever's selected for everyone. Drag any frame by its title bar, scroll to zoom, click a name in the sidebar to jump to it. Shift-click a frame's title bar to select it, shift-click more to build a group, then drag any selected frame to move the whole group together — cluster the charts that belong to the same story and reposition them as one; every frame move is shared live and saved too, so the layout you leave the room in is the layout it opens in next time.

Everything — pins, drawings, images, and frame layout — is saved to a named **session** on disk — today's date by default — and restored automatically the next time the same session is served, so closing the server and reopening it later picks up exactly where the room left off, images and moved frames included. Click **🗂 Sessions** to see every session ever saved (name, mark count, last touched) with a delete button on each — traceability that survives restarts, until you explicitly clean it up. Click any *other* session's name in that list to jump the whole room to it, live: everyone connected switches together, marks and layout swap instantly, no reload and no new link to send around — the session you're currently in is marked **(this board)** and stays put until you pick a different one.

Need one specific chart to keep streaming real live data on the board instead of a frozen snapshot? Serve it yourself first, then pass its URL in `live_urls`:
...
That frame gets a small green pulsing dot and streams for real, continuously — the other two stay static snapshots since their `live_urls` entry is `None`.

<small>Honest scope: presence (cursors) is board-wide, not per-chart — each embedded chart is its own `<iframe>`, and a page can't open a WebSocket from inside another origin's `srcdoc`, so a purely-static frame can't stream on its own (that's exactly what `live_urls` is for). Deleting a pin from the board removes it from the live view and the saved session file, but not from `chart.bookmarks()` if that specific chart was separately built with persistent bookmarks — the board's layer and a chart's own permanent record are kept independent on purpose. Deleting the *active* session stops that connection from saving further changes — open a new session to keep going.</small>

Don't need it live, or want a file you can just double-click and send around? `export_board_html(charts, path, titles=None)` writes the same board as one self-contained `.html` file, no server needed at all — same drag/zoom/sidebar, each chart keeps its own interactivity (hover, tooltips) inside its frame, frames are real `<iframe>`s not flattened screenshots. The static export has no toolbar, no pins/drawings, and nothing to save — it's a snapshot, not a room:

```python
sp.export_board_html([errors, latency, queue], "war_room.html", titles=["Errors", "Latency", "Queue"])
```

<small>Static export's layout doesn't persist either — reopening the file resets frames to the starting grid, dragging them around is per-session. Both are the same catalog, same license as everything else on this page.</small>
...
Payé via PayPal — cliquez un lien d'achat, payez sur la page de paiement hébergée par PayPal, et votre clé de licence est générée automatiquement une fois le paiement confirmé (un webhook signé déclenche la génération, vérifié contre la signature de PayPal pour qu'il soit impossible de simuler un paiement). Le service de paiement est déployé et en ligne sur [Sera-Payment](https://github.com/SeraWorkspace/Sera-Payment) (privé) ; PayPal est en cours de configuration au moment où ces lignes sont écrites, donc un lien d'achat peut brièvement afficher une page claire « pas encore configuré » plutôt que la page de paiement — normal pendant la configuration, pas un lien mort, et ça se résout tout seul une fois fait. Préférence pour la crypto (USDC/Polygon, auto-hébergé) ou une vente manuelle ? Passez par le [canal de support](../about/support.md).
...
`firehose_push()` est `push()` avec les indices calculés pour vous — même limitation de débit, même détection d'anomalies via `on_anomaly()`, même `record()`/`replay()`, même diffusion `.serve()` sur les mêmes trames WebSocket binaires que tout autre chart natif diffuse déjà. Rien de nouveau côté transport ; ce qui est nouveau c'est le client : au lieu d'un `<canvas>` redessiné point par point sur le CPU, Firehose se rend via WebGL2 — un envoi de buffer GPU par frame d'animation, un seul appel `drawArrays`, aucun travail JavaScript par point quel que soit le nombre de valeurs arrivées depuis la dernière frame.

<small>Note honnête : l'idée d'origine du chantier prévoyait `wgpu` (Rust compilé en client WebGPU/WebGL via WASM). SeraPlot n'a pas encore de chaîne de build wasm32, donc la v1 livre un client WebGL2 en JavaScript pur à la place — même gain côté GPU pour ce chart (un seul envoi de buffer et un seul appel de dessin par frame, pas une opération JS par point), disponible dans tous les navigateurs actuels sans ajouter de nouvelle cible de build. Un client `wgpu` reste une suite naturelle une fois la chaîne WASM en place.</small>
...
`.serve()` streame déjà un chart vers tous ceux qui l'ouvrent. Sera Live transforme ça en une salle : tout le monde connecté voit le curseur des autres bouger sur le chart en temps réel, peut épingler un commentaire sur un point de donnée précis, et peut ramener tout le groupe à travers un incident enregistré ensemble, en un clic.
...
Ouvrez cette URL dans deux onglets (ou envoyez-la à un collègue) — tout tient dans une barre d'outils flottante en bas, façon Figma : **↖ Select**, **📌 Pin**, **✏ Draw**, **🗑 Delete**, **💬 Comments**, **▶ Replay together**, **🌙 Dark mode**. Cliquez sur **🌙** à tout moment pour faire basculer toute la salle — barre d'outils, panneaux, habillage de la toile — dans un thème sombre pensé pour une war room dans le noir ou un écran mural ; le choix est mémorisé par navigateur et suit la préférence de votre système la première fois que vous ouvrez le lien. Bougez la souris dans un onglet, elle apparaît en direct dans l'autre, nommée et colorée par personne. Cliquez sur **📌**, puis n'importe où sur le chart, pour y épingler une note (ou Alt-clic direct sur un point, sans passer par le bouton) ; tout le monde la voit apparaître instantanément, comme une vraie étiquette visible à côté de l'épingle, pas seulement une infobulle au survol. Cliquez sur **▶** et chaque onglet connecté traverse `incident.spls` en même temps, depuis le début — revivre ce qui s'est passé au même rythme, ensemble, plutôt que de plisser les yeux sur un partage d'écran.

Cliquez sur **✏** pour dessiner à main levée directement sur le chart — entourer la barre qui explose, souligner l'anomalie, dessiner une flèche vers ce que tout le monde doit regarder — diffusé en direct dans la salle exactement comme une épingle, le curseur devient un petit crayon pour toujours savoir qu'on est en mode dessin. Un petit panneau s'ouvre juste au-dessus de la barre d'outils avec une vraie pastille de couleur et un curseur d'épaisseur — rouge pour « voilà le problème », un trait épais pour qu'il ressorte, les deux s'appliquent à ce que vous dessinez ensuite. Cliquez sur **↖ Select** — le curseur devient une main, puisque le mode sélection sert aussi à déplacer la vue de la salle — puis cliquez une épingle ou la ligne d'un trait pour le sélectionner (il s'allume en bleu ; les traits ont une zone de clic invisible généreuse autour de la ligne fine, pas besoin de viser le pixel exact), puis **🗑** ou la touche Suppr le retire pour tout le monde. Sur la toile en particulier, l'outil actif est toujours prioritaire sur le déplacement de la vue — cliquer pour poser une épingle ou glisser pour dessiner un trait n'entre jamais en conflit avec le glissement de la toile. Cliquez sur **💬** pour ouvrir la liste de toutes les épingles posées jusqu'ici, cliquez-en une pour y sauter directement. Un petit badge horloge en haut affiche le temps écoulé depuis l'ouverture de la salle, le même pour tout le monde, pour que « ça a commencé il y a 4 minutes » soit un fait partagé plutôt qu'une estimation. Rejoindre en retard ne fait rien perdre — les épingles et dessins faits avant votre connexion vous sont rejoués dès que vous rejoignez.

Rien de nouveau côté format réseau : curseurs, épingles, dessins et rejeu voyagent tous sur le même WebSocket que `.serve()` ouvre déjà, via le même canal de diffusion que `push()` utilise pour les mises à jour du chart. Les épingles sont de vraies annotations `bookmark()` sous le capot, pas une couche jetable à part — listez-les ensuite avec `chart.bookmarks()` comme d'habitude.

`collab` vaut `True` par défaut sur `serve()` et `stream()`. Passez `collab=False` pour l'ancien comportement mono-spectateur, sans aucune diffusion de présence — rien concernant vous ou votre curseur ne quitte jamais votre machine dans ce mode.

```python
chart.serve(port=8787, collab=False)
```

<small>Fermer un onglet retire le curseur de cette personne pour tout le monde immédiatement, pas après un délai — la fenêtre de 15 secondes ne sert plus qu'à rattraper une déconnexion brutale (un crash, une connexion perdue), jamais une fermeture normale. Les épingles et « Replay together » sont limités en débit par chart (le même primitif à jeton que `set_push_rate_limit()`, juste son propre budget) — un script errant ou un clic trop enthousiaste ne peut pas inonder la salle, et deux personnes qui cliquent « Replay together » au même moment ne diffusent l'enregistrement qu'une seule fois. L'accès lui-même suit toujours le modèle de `.serve()` : passez `username`/`password` pour garder un lien privé, comme vous le feriez déjà pour le flux de données lui-même.</small>

---

## Sera Board

Un chart à la fois, ça suffit pour une seule métrique. Un vrai incident, jamais — taux d'erreur, latence, profondeur de file, marqueurs de déploiement, tout en même temps, liés mais pas de la même forme. Sera Board pose tous vos charts sur une seule toile infinie : une barre latérale façon Figma les liste tous, chacun dans son propre cadre qu'on déplace où on veut, superposé exprès ou non, et on zoome à la molette d'une vue commandement complète jusqu'à un seul chart plein écran — sans limite fixe dans un sens ou l'autre.

`serve_board()` réunit Sera Board et Sera Live : il ouvre le même genre de serveur local que `.serve()`, et tous ceux qui ouvrent le lien partagent la même salle — curseurs en direct sur la toile, commentaires épinglés n'importe où dessus, via exactement la même salle WebSocket que `.serve()` utilise déjà.
...
Envoyez ce lien à l'équipe — tout tient dans une barre d'outils en bas, façon Figma : **↖ Select**, **📌 Pin**, **✏ Draw**, **🖼 Image**, **🎨 Recolor**, **🗑 Delete**, **💬 Comments**, **🗂 Sessions**, **🌙 Dark mode**. Bougez la souris n'importe où et ça apparaît en direct pour tout le monde. Épinglez un commentaire, déposez une image, ou dessinez directement sur un chart et ça se colle au cadre de ce chart — déplacez le cadre et ça suit ; posez-le sur la toile vide à la place et ça reste fixe dans le plan — zoomer et déplacer la vue le bouge exactement comme le reste, jamais en suivant votre viewport. Survolez un cadre (sans outil armé) pour activer directement le survol/les infobulles propres à ce chart, retirez la souris et ça se désactive — la toile ne se dispute jamais la souris avec un chart.

Cliquez **🖼 Image** (ou glissez-déposez directement un fichier image depuis votre bureau n'importe où sur la toile) pour épingler une capture d'écran, une photo de tableau blanc, un schéma — n'importe quoi — directement sur la toile ; déplacez-la ensuite en la saisissant directement, sans outil. Cliquez **↖ Select** puis cliquez une épingle, un trait, ou une image pour le sélectionner (il s'allume en bleu) : **🎨** ouvre une pastille de couleur pour recolorer une épingle ou un trait sur place, **🗑** ou la touche Suppr retire ce qui est sélectionné pour tout le monde. Déplacez un cadre par sa barre de titre, zoomez à la molette, cliquez un nom dans la barre latérale pour y sauter. Shift-cliquez la barre de titre d'un cadre pour le sélectionner, shift-cliquez-en d'autres pour construire un groupe, puis déplacez n'importe quel cadre sélectionné pour bouger tout le groupe ensemble — regroupez les charts qui racontent la même histoire et repositionnez-les comme un seul bloc ; chaque déplacement de cadre est aussi partagé en direct et sauvegardé, donc la disposition dans laquelle vous laissez la salle est celle qu'elle retrouve la prochaine fois.

Tout — épingles, dessins, images, et disposition des cadres — est sauvegardé dans une **session** nommée sur disque — la date du jour par défaut — et restauré automatiquement la prochaine fois que la même session est servie, images et cadres déplacés compris. Cliquez **🗂 Sessions** pour voir toutes les sessions jamais sauvegardées (nom, nombre de marques, dernière modification) avec un bouton de suppression sur chacune — une traçabilité qui survit aux redémarrages, jusqu'à ce que vous la nettoyiez vous-même. Cliquez le nom de n'importe quelle *autre* session dans cette liste pour y faire basculer toute la salle, en direct : tout le monde connecté bascule ensemble, marques et disposition changent instantanément, sans recharger la page et sans nouveau lien à envoyer — la session dans laquelle vous êtes actuellement est marquée **(this board)** et reste en place jusqu'à ce que vous en choisissiez une autre.

Besoin qu'un chart précis continue à streamer de vraies données en direct sur la toile plutôt qu'un instantané figé ? Servez-le vous-même d'abord, puis passez son URL dans `live_urls` :
...
Ce cadre reçoit un petit point vert qui pulse et streame pour de vrai, en continu — les deux autres restent des instantanés statiques puisque leur entrée `live_urls` est `None`.

<small>Portée honnête : la présence (curseurs) est propre à la toile entière, pas à chaque chart — chaque chart intégré est son propre `<iframe>`, et une page ne peut pas ouvrir de WebSocket depuis le `srcdoc` d'une autre origine, donc un cadre purement statique ne peut pas streamer par lui-même (c'est exactement à ça que sert `live_urls`). Supprimer une épingle de la toile la retire de la vue en direct et du fichier de session sauvegardé, mais pas de `chart.bookmarks()` si ce chart précis a été construit séparément avec des repères persistants — la couche de la toile et l'enregistrement permanent propre à un chart restent volontairement indépendants. Supprimer la session *active* arrête cette connexion de sauvegarder d'autres changements — ouvrez une nouvelle session pour continuer.</small>

Pas besoin que ce soit en direct, ou envie d'un fichier à double-cliquer et envoyer tel quel ? `export_board_html(charts, path, titles=None)` écrit la même toile en un seul fichier `.html` autonome, sans serveur du tout — même glisser/zoom/barre latérale, chaque chart garde sa propre interactivité (survol, infobulles) dans son cadre, les cadres sont de vrais `<iframe>`, pas des captures aplaties. L'export statique n'a ni barre d'outils, ni épingles/dessins, ni rien à sauvegarder — c'est un instantané, pas une salle :

```python
sp.export_board_html([erreurs, latence, file_attente], "war_room.html", titles=["Erreurs", "Latence", "File"])
```

<small>La disposition de l'export statique ne persiste pas non plus — rouvrir le fichier remet les cadres à la grille de départ, les déplacer est propre à la session. Les deux sont dans le même catalogue, la même licence que tout le reste de cette page.</small>
