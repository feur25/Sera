...
function rBarBlocks(mx,my,sc){
  ...
  var bars=[];
  for(var j=0;j<BN;j++){
    bars.push({i:j,nx:(BX[j]-BFIT.cx)/BFIT.sx,ny:(BY[j]-BFIT.cy)/BFIT.sy,z0:(BZ0[j]-e[4])/BFIT.sz-BFIT.dz/2,z1:(BZ1[j]-e[4])/BFIT.sz-BFIT.dz/2,hw:BHW[j]/BFIT.sx,hd:BHD[j]/BFIT.sy,ci:BCI[j],tone:(typeof BCT!=='undefined')?BCT[j]:-1});
  }
...
wrap.addEventListener('dblclick',function(){yaw=yaw0;pitch=pitch0;zoom=zoom0;panX=0;panY=0;pin=false;piI=-1;tip.className='c3t';velY=0;velP=0;R();});
...
document.addEventListener('keydown',function(e){
  ...
  if(e.key==='r'||e.key==='R'){yaw=yaw0;pitch=pitch0;zoom=zoom0;panX=0;panY=0;velY=0;velP=0;R();}
});
