...
var yaw0=yaw,pitch0=pitch,zoom0=1.0;zoom=zoom0;
...
var CAM={ex:0,ey:0,ez:0,fx:0,fy:0,fz:0,rx:0,ry:0,ux:0,uy:0,uz:0,kx:1,ky:1,px:0,py:0};
function camSet(){
  var cp=Math.cos(pitch),ex=zoom*Math.cos(yaw)*cp,ey=zoom*Math.sin(yaw)*cp,ez=zoom*Math.sin(pitch);
  var fl=Math.sqrt(ex*ex+ey*ey+ez*ez),fx=-ex/fl,fy=-ey/fl,fz=-ez/fl;
  var rx=-fy,ry=fx,rl=Math.sqrt(rx*rx+ry*ry)||1e-6;rx/=rl;ry/=rl;
  var th=Math.tan(fov/2),asp=W/H;
  CAM.ex=ex;CAM.ey=ey;CAM.ez=ez;CAM.fx=fx;CAM.fy=fy;CAM.fz=fz;CAM.rx=rx;CAM.ry=ry;
  CAM.ux=-fz*ry;CAM.uy=fz*rx;CAM.uz=fx*ry-fy*rx;
  CAM.kx=1/(th*asp);CAM.ky=1/th;CAM.px=panX/sc;CAM.py=panY/sc;
}
function pj(px,py,pz){var dx=px-CAM.ex,dy=py-CAM.ey,dz=pz-CAM.ez,dp=dx*CAM.fx+dy*CAM.fy+dz*CAM.fz;if(dp<0.001)return null;return{x:(dx*CAM.rx+dy*CAM.ry)*CAM.kx/dp+CAM.px,y:(dx*CAM.ux+dy*CAM.uy+dz*CAM.uz)*CAM.ky/dp+CAM.py,d:dp};}
var LODB=typeof BN!=='undefined'?(BN>2500?2:(BN>500?1:0)):0,QP=0,SLOW=0;
function lod(){return Math.min(3,LODB+QP);}
function adapt(dt){if(dt>32){SLOW++;if(SLOW>=2&&QP<3-LODB){QP++;SLOW=0;}}else{SLOW=0;if(dt<9&&QP>0)QP--;}}
var FCC={},THX={};
function shadeCol(rgb,k){return 'rgb('+Math.max(0,Math.min(255,rgb[0]+k))+','+Math.max(0,Math.min(255,rgb[1]+k))+','+Math.max(0,Math.min(255,rgb[2]+k))+')';}
function faceCols(baseCol){var f=FCC[baseCol];if(f)return f;var rgb=hx2rgb(baseCol);f={top:shadeCol(rgb,45),front:shadeCol(rgb,0),right:shadeCol(rgb,-45),back:shadeCol(rgb,-14),left:shadeCol(rgb,-30),bottom:shadeCol(rgb,-60)};FCC[baseCol]=f;return f;}
function toneHexQ(t){var q=Math.round(Math.max(0,Math.min(1,t))*127),h=THX[q];if(!h){h=cmapHex(q/127,CMAP);THX[q]=h;}return h;}
var ZN={x0:-0.5,x1:0.5,y0:-0.5,y1:0.5,z0:-0.5,z1:0.5},TK={x0:-0.5,x1:0.5,y0:-0.5,y1:0.5,z0:-0.5,z1:0.5},cV=[];
function rebuildCorners(){cV=[[ZN.x0,ZN.y0,ZN.z0],[ZN.x1,ZN.y0,ZN.z0],[ZN.x1,ZN.y1,ZN.z0],[ZN.x0,ZN.y1,ZN.z0],[ZN.x0,ZN.y0,ZN.z1],[ZN.x1,ZN.y0,ZN.z1],[ZN.x1,ZN.y1,ZN.z1],[ZN.x0,ZN.y1,ZN.z1]];}
function zoneDiv(len){var m=Math.max(ZN.x1-ZN.x0,ZN.y1-ZN.y0,ZN.z1-ZN.z0);return Math.max(2,Math.round(10*len/m));}
function zoneAt(a,b,q,n){return a+(b-a)*q/n;}
function applyFit(){
  if(typeof BFIT==='undefined'||isEnvScene())return;
  var P=0.03,e=BFIT.ext;
...
function drawFloor(mx,my,sc){
  var nx=zoneDiv(ZN.x1-ZN.x0),ny=zoneDiv(ZN.y1-ZN.y0);
  for(var q=0;q<=nx;q++){var f=zoneAt(ZN.x0,ZN.x1,q,nx);strokeSeg(pj(f,ZN.y0,ZN.z0),pj(f,ZN.y1,ZN.z0),q%5===0);}
  for(var w=0;w<=ny;w++){var h=zoneAt(ZN.y0,ZN.y1,w,ny);strokeSeg(pj(ZN.x0,h,ZN.z0),pj(ZN.x1,h,ZN.z0),w%5===0);}
  var c0=pj(ZN.x0,ZN.y0,ZN.z0),c1=pj(ZN.x1,ZN.y0,ZN.z0),c2=pj(ZN.x1,ZN.y1,ZN.z0),c3=pj(ZN.x0,ZN.y1,ZN.z0);
  if(c0&&c1&&c2&&c3){
    ...
    var fg=g.createRadialGradient(mx,my,0,mx,my,sc*0.6*Math.max(ZN.x1-ZN.x0,ZN.y1-ZN.y0));
...
function drawBackWall(mx,my,sc){
  var nx=zoneDiv(ZN.x1-ZN.x0),nz=zoneDiv(ZN.z1-ZN.z0);
  for(var q=0;q<=nx;q++){var f=zoneAt(ZN.x0,ZN.x1,q,nx);strokeSeg(pj(f,ZN.y1,ZN.z0),pj(f,ZN.y1,ZN.z1),q%5===0);}
  for(var w=0;w<=nz;w++){var h=zoneAt(ZN.z0,ZN.z1,w,nz);strokeSeg(pj(ZN.x0,ZN.y1,h),pj(ZN.x1,ZN.y1,h),w%5===0);}
  drawWallQuad(pj(ZN.x0,ZN.y1,ZN.z0),pj(ZN.x1,ZN.y1,ZN.z0),pj(ZN.x1,ZN.y1,ZN.z1),pj(ZN.x0,ZN.y1,ZN.z1));
}
function drawSideWall(mx,my,sc){
  var ny=zoneDiv(ZN.y1-ZN.y0),nz=zoneDiv(ZN.z1-ZN.z0);
  for(var q=0;q<=ny;q++){var f=zoneAt(ZN.y0,ZN.y1,q,ny);strokeSeg(pj(ZN.x0,f,ZN.z0),pj(ZN.x0,f,ZN.z1),q%5===0);}
  for(var w=0;w<=nz;w++){var h=zoneAt(ZN.z0,ZN.z1,w,nz);strokeSeg(pj(ZN.x0,ZN.y0,h),pj(ZN.x0,ZN.y1,h),w%5===0);}
  drawWallQuad(pj(ZN.x0,ZN.y0,ZN.z0),pj(ZN.x0,ZN.y1,ZN.z0),pj(ZN.x0,ZN.y1,ZN.z1),pj(ZN.x0,ZN.y0,ZN.z1));
}
...
var CMAPS={
  ...
  updown:[[0,239,68,68],[0.5,250,204,21],[1,34,197,94]],
  bullet:[[0,220,38,38],[0.1,217,119,6],[0.2,22,163,74],[0.3,71,85,105],[0.4,100,116,139],[0.5,148,163,184],[0.6,51,65,85],[0.7,56,189,248],[0.8,203,213,225],[0.9,250,204,21],[1,239,68,68]],
  dial:[[0,22,163,74],[0.1,217,119,6],[0.2,220,38,38],[0.3,100,116,139],[0.4,15,23,42],[0.5,56,189,248],[1,56,189,248]]
};
...
function drawContactShadow(nx,ny,hw,hd){
  if(SCENE!=='default'||lod()>0)return;
  var psh=pj(nx,ny,ZN.z0);
  if(!psh)return;
...
function fillFace(a,b,c,d,col,lv){
  ...
  if(lv<2){g.strokeStyle='rgba(0,0,0,0.18)';g.lineWidth=0.5;g.stroke();}
}
function drawCuboidBar(nx,ny,z0,z1,hw,hd,baseCol,e0,e1){
  var f0=e0===undefined?z0:e0,f1=e1===undefined?z1:e1;
  var a0=pj(nx-hw,ny-hd,z0),a1=pj(nx+hw,ny-hd,f0),a2=pj(nx+hw,ny+hd,f0),a3=pj(nx-hw,ny+hd,z0);
  var b0=pj(nx-hw,ny-hd,z1),b1=pj(nx+hw,ny-hd,f1),b2=pj(nx+hw,ny+hd,f1),b3=pj(nx-hw,ny+hd,z1);
  if(!a0||!a1||!a2||!a3||!b0||!b1||!b2||!b3)return;
  var lv=lod();
  if(Math.min(z0,f0)-ZN.z0<1e-6)drawContactShadow(nx,ny,hw,hd);
  var fc=faceCols(baseCol);
  fillFace(a0,a1,b1,b0,fc.front,lv);
  fillFace(a2,a3,b3,b2,fc.back,lv);
  fillFace(a1,a2,b2,b1,fc.right,lv);
...
var BP=null,ENVB=null;
function prepBlocks(){
  var e=BFIT.ext,n=BN,hasEnd=typeof BEZ0!=='undefined',hasTone=typeof BCT!=='undefined';
  BP={nx:new Float64Array(n),ny:new Float64Array(n),z0:new Float64Array(n),z1:new Float64Array(n),e0:new Float64Array(n),e1:new Float64Array(n),hw:new Float64Array(n),hd:new Float64Array(n),col:new Array(n),dep:new Float64Array(n),ord:new Uint32Array(n),psx:new Float32Array(n),psy:new Float32Array(n),live:0,view:null};
  for(var j=0;j<n;j++){
    BP.nx[j]=(BX[j]-BFIT.cx)/BFIT.sx;BP.ny[j]=(BY[j]-BFIT.cy)/BFIT.sy;
    BP.z0[j]=(BZ0[j]-e[4])/BFIT.sz-BFIT.dz/2;BP.z1[j]=(BZ1[j]-e[4])/BFIT.sz-BFIT.dz/2;
    BP.e0[j]=hasEnd?(BEZ0[j]-e[4])/BFIT.sz-BFIT.dz/2:BP.z0[j];BP.e1[j]=hasEnd?(BEZ1[j]-e[4])/BFIT.sz-BFIT.dz/2:BP.z1[j];
    BP.hw[j]=BHW[j]/BFIT.sx;BP.hd[j]=BHD[j]/BFIT.sy;
    BP.col[j]=(hasTone&&BCT[j]>=0)?toneHexQ(BCT[j]):PAL[BCI[j]%PAL.length];
  }
}
function rBlocksFast(){
  if(!BP)prepBlocks();
  var n=BN,dep=BP.dep,ord=BP.ord,live=0,mxm=W*0.25,mym=H*0.25;
  for(var j=0;j<n;j++){
    if(HIDDEN[BCI[j]%PAL.length])continue;
    var p=pj(BP.nx[j],BP.ny[j],(BP.z1[j]+BP.e1[j])/2);
    if(!p)continue;
    var sx=mx+p.x*sc,sy=my-p.y*sc;
    if(sx<-mxm||sx>W+mxm||sy<-mym||sy>H+mym)continue;
    dep[j]=p.d;BP.psx[j]=sx;BP.psy[j]=sy;ord[live++]=j;
  }
  var view=ord.subarray(0,live);
  view.sort(function(a,c){return dep[c]-dep[a];});
  for(var k=0;k<live;k++){var i=view[k];drawCuboidBar(BP.nx[i],BP.ny[i],BP.z0[i],BP.z1[i],BP.hw[i],BP.hd[i],BP.col[i],BP.e0[i],BP.e1[i]);}
  BP.live=live;BP.view=view;
}
function hitBlock(ex,ey){
  if(!BP||!BP.view)return -1;
  var best=-1,bd=1e18,v=BP.view;
  for(var k=BP.live-1;k>=0;k--){var i=v[k],dx=BP.psx[i]-ex,dy=BP.psy[i]-ey,d2=dx*dx+dy*dy;if(d2<bd&&d2<256){bd=d2;best=i;}}
  return best;
}
function rBarBlocks(mx,my,sc){
  ...
  if(isEnvScene()){
    if(!ENVB){
      var bxr=(e[1]-e[0])||1,byr=(e[3]-e[2])||1,bzr=(e[5]-e[4])||1;
      var pnx=new Float64Array(BN),pny=new Float64Array(BN),pnz=new Float64Array(BN);
      for(var ti=0;ti<BN;ti++){
        pnx[ti]=(BX[ti]-e[0])/bxr-0.5;pny[ti]=(BY[ti]-e[2])/byr-0.5;pnz[ti]=(BZ1[ti]-e[4])/bzr;
      }
      ENVB=buildEnvironmentBars(pnx,pny,pnz,BN);
    }
...
function R(){
  drawBG();
  mx=W/2;my=H/2;sc=Math.min(W,H)*0.34;camSet();
  if(ttl){
  ...
  for(var k=0;k<=4;k++){var f=k/4;
    var ax0=pj(zoneAt(TK.x0,TK.x1,k,4),ZN.y0,ZN.z0);if(ax0){g.textAlign='center';g.textBaseline='top';g.fillStyle=isDark?'rgba(244,114,182,0.55)':'#6b7280';g.fillText((xmn+xr*f).toFixed(1),mx+ax0.x*sc,my-ax0.y*sc+5);}
    var ay0=pj(ZN.x1,zoneAt(TK.y0,TK.y1,k,4),ZN.z0);if(ay0){g.textAlign='center';g.textBaseline='top';g.fillStyle=isDark?'rgba(34,211,238,0.55)':'#6b7280';g.fillText((ymn+yr*f).toFixed(1),mx+ay0.x*sc,my-ay0.y*sc+5);}
    var az0=pj(ZN.x1,ZN.y0,zoneAt(TK.z0,TK.z1,k,4));if(az0){g.textAlign='left';g.textBaseline='middle';g.fillStyle=isDark?'rgba(251,191,36,0.55)':'#6b7280';g.fillText((zmn+zr*f).toFixed(1),mx+az0.x*sc+6,my-az0.y*sc);}
  }
  g.font='700 10.5px -apple-system,sans-serif';
  var lp=pj((ZN.x0+ZN.x1)/2,ZN.y0,ZN.z0);if(lp){g.textAlign='center';g.textBaseline='top';g.fillStyle=AX;g.fillText(xl,mx+lp.x*sc,my-lp.y*sc+17);}
  lp=pj(ZN.x1,(ZN.y0+ZN.y1)/2,ZN.z0);if(lp){g.textAlign='center';g.textBaseline='top';g.fillStyle=AY;g.fillText(yl,mx+lp.x*sc,my-lp.y*sc+17);}
  lp=pj(ZN.x0,ZN.y0,(ZN.z0+ZN.z1)/2);if(lp){g.textAlign='right';g.textBaseline='middle';g.fillStyle=AZ;g.fillText(zl,mx+lp.x*sc-10,my-lp.y*sc);}
  }
  pp=[];
...
var shiftDrag=false;
var raf=0,need=false,vis=true;
function wake(){need=true;if(!raf&&vis)raf=requestAnimationFrame(tick);}
function tick(){
  raf=0;var dirty=need,keep=false;need=false;
  if(autoR&&!dg){yaw+=0.004;dirty=true;keep=true;}
  if(Math.abs(velY)>0.0003||Math.abs(velP)>0.0003){
    yaw+=velY;pitch=Math.max(-1.47,Math.min(1.47,pitch+velP));
    velY*=fric;velP*=fric;dirty=true;keep=true;
  }
  if(keys.ArrowLeft||keys.a||keys.A){yaw-=kSpd;dirty=true;keep=true;}
  if(keys.ArrowRight||keys.d||keys.D){yaw+=kSpd;dirty=true;keep=true;}
  if(keys.ArrowUp||keys.w||keys.W){pitch=Math.min(1.47,pitch+kSpd);dirty=true;keep=true;}
...
window.addEventListener('mousemove',function(e){
  if(dg&&shiftDrag){
    panX+=(e.clientX-lx);panY+=(e.clientY-ly);lx=e.clientX;ly=e.clientY;wake();return;
  }
  if(dg){
...
window.addEventListener('mouseup',function(e){
  if(!dg)return;var wasSh=shiftDrag;dg=false;shiftDrag=false;wake();
  if(wasSh)return;
  if(!mv){var bx=wrap.getBoundingClientRect(),sf=W/bx.width,ex=(e.clientX-bx.left)*sf,ey=(e.clientY-bx.top)*sf;var lci=legHt(ex,ey);if(lci>=0){HIDDEN[lci]=!HIDDEN[lci];recomputeVis();wake();return;}var idx=ht(ex,ey);if(idx>=0){pin=true;piI=idx;sT(idx,e.clientX,e.clientY);wake();}else{pin=false;piI=-1;tip.className='c3t';wake();}}
});
...
wrap.addEventListener('dblclick',function(){yaw=yaw0;pitch=pitch0;zoom=zoom0;panX=0;panY=0;pin=false;piI=-1;tip.className='c3t';velY=0;velP=0;wake();});
wrap.addEventListener('mouseleave',function(){if(!pin)hT();});
wrap.addEventListener('contextmenu',function(e){e.preventDefault();});
document.addEventListener('keydown',function(e){
  keys[e.key]=true;wake();
  if(e.key==='Escape'){if(pin){pin=false;piI=-1;tip.className='c3t';wake();}}
  if(e.key===' '){autoR=!autoR;wake();e.preventDefault();}
  if(e.key==='='||e.key==='+'){zoom=Math.min(5,zoom*1.1);wake();}
  if(e.key==='-'){zoom=Math.max(0.3,zoom*0.9);wake();}
  if(e.key==='r'||e.key==='R'){yaw=yaw0;pitch=pitch0;zoom=zoom0;panX=0;panY=0;velY=0;velP=0;wake();}
});
...
wrap.addEventListener('touchmove',function(e){
  if(e.touches.length===2){var nd=Math.hypot(e.touches[1].clientX-e.touches[0].clientX,e.touches[1].clientY-e.touches[0].clientY);if(t0d>0){zoom=Math.max(0.3,Math.min(5,zoom*(nd/t0d)));t0d=nd;wake();}e.preventDefault();return;}
  if(!dg||e.touches.length!==1)return;var dx=e.touches[0].clientX-lx,dy=e.touches[0].clientY-ly;if(Math.abs(e.touches[0].clientX-dwX)>3||Math.abs(e.touches[0].clientY-dwY)>3)mv=true;velY=dx*0.008;velP=dy*0.008;yaw+=velY;pitch=Math.max(-1.47,Math.min(1.47,pitch+velP));lx=e.touches[0].clientX;ly=e.touches[0].clientY;wake();e.preventDefault();
},{passive:false});
wrap.addEventListener('touchend',function(e){if(e.touches.length===0){dg=false;t0d=0;if(!mv){var bx=wrap.getBoundingClientRect(),sf=W/bx.width,ex=(dwX-bx.left)*sf,ey=(dwY-bx.top)*sf;var idx=ht(ex,ey);if(idx>=0){pin=true;piI=idx;sT(idx,dwX,dwY);wake();}else{pin=false;piI=-1;tip.className='c3t';}}}});
if(typeof IntersectionObserver!=='undefined'){new IntersectionObserver(function(en){vis=en[en.length-1].isIntersecting;if(vis&&need&&!raf)raf=requestAnimationFrame(tick);}).observe(wrap);}
document.addEventListener('visibilitychange',function(){vis=!document.hidden;if(vis&&need&&!raf)raf=requestAnimationFrame(tick);});
"##;
