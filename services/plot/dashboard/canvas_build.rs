    ...
    pub fn build(&self) -> crate::Chart {
        ...
        let hover_js = if has_hover {
            concat!(
                ...
                "function qAll(){return root.querySelectorAll('.sp-hvl[data-sp-grp],.sp-anch[data-sp-grp],div[data-sp-grp],rect[data-sp-grp],text[data-sp-grp],path.sp-wedge[data-sp-grp]');}",
                "function famDim(g){qAll().forEach(function(e){if(e.getAttribute('data-sp-grp')!==g){e.style.opacity='0.12';e.style.transition='opacity .18s ease';}});}",
                "function famUndim(){qAll().forEach(function(e){e.style.opacity='';});}",
                "function act(g){",
                  "if(aG===g)return;if(aG)deact(aG);aG=g;famDim(g);",
                  "var lEls=qL(g),aEls=qA(g),sEls=qS(g);",
                  "if(lEls.length+aEls.length+sEls.length>40)return;",
                  "lEls.forEach(function(l){",
                ...
                "}",
                "root.querySelectorAll('.sp-hvh,div[data-sp-grp],rect[data-sp-grp],text[data-sp-grp],.sp-anch[data-sp-grp],path.sp-wedge[data-sp-grp]').forEach(function(hit){",
