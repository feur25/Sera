...
use crate::plot::statistical::common::{
    escape_xml, palette_color, push_b, push_f2, push_i, sort_indices, sorted, svg_legend_item,
    svg_open, svg_title, svg_x_label, svg_y_label, truncate,
};
...
pub fn prepare(cfg: &RidgelineConfig, overlap_override: Option<f64>) -> Option<Prepared> {
    ...
    for cat in cfg.categories[..n].iter() {
        if !group_order.contains(cat) {
            group_order.push(cat.clone());
        }
    }
    ...
    let xs: Vec<f64> = (0..n_pts)
        .map(|i| x0 + xr * i as f64 / (n_pts - 1) as f64)
        .collect();

    let curves: Vec<Vec<f64>> = group_vals
        .iter()
        .map(|vals| {
            if vals.is_empty() {
            ...
            let sampled: Vec<f64> = if step > 1 {
                vals.iter().step_by(step).copied().collect()
            } else {
                vals.to_vec()
            };
            let bw = if cfg.bandwidth > 0.0 {
                cfg.bandwidth
            } else {
                scott_bw(&sampled).max(1e-12)
            };
            xs.iter()
                .map(|&x| kde_eval(&sampled, x, bw) * step as f64)
                .collect()
        })
        .collect();
...
pub fn open_svg(buf: &mut Vec<u8>, cfg: &RidgelineConfig, layout: &Layout, x0: f64, xr: f64) {
    ...
    svg_title(
        ...
        cfg.title,
        cfg.width / 2,
        if layout.title_h > 0 { 24 } else { 0 },
    );
    let n_xticks: i32 = 6;
    push_b(buf, b"<line x1=\"");
    push_i(buf, layout.pad_l);
    push_b(buf, b"\" y1=\"");
    push_i(buf, layout.axis_y);
    push_b(buf, b"\" x2=\"");
    push_i(buf, layout.pad_l + layout.plot_w);
    push_b(buf, b"\" y2=\"");
    push_i(buf, layout.axis_y);
    push_b(
        buf,
...
pub fn ridge_label(buf: &mut Vec<u8>, layout: &Layout, base_y: i32, name: &str) {
    ...
    push_b(buf, b"<text x=\"");
    push_i(buf, layout.pad_l - 8);
    push_b(buf, b"\" y=\"");
    push_i(buf, base_y - layout.row_h / 2 + 4);
...
pub fn close_svg(buf: &mut Vec<u8>, cfg: &RidgelineConfig, p: &Prepared, draw_legend: bool) {
    if draw_legend && p.group_order.len() > 1 {
        ...
        for (gi, name) in p.group_order.iter().enumerate() {
            svg_legend_item(
                ...
                legend_x,
                p.layout.title_h + 10 + gi as i32 * 22,
                12,
            );
        }
    }
    svg_x_label(
        buf,
        cfg.x_label,
        p.layout.pad_l + p.layout.plot_w / 2,
        cfg.height - 4,
    );
...
pub fn y_at_x(p: &Prepared, ys: &[f64], x: f64, base_y: i32) -> f64 {
    if x <= p.x0 {
        return base_y as f64 - ys[0] / p.global_max * p.layout.curve_h as f64;
    }
    if x >= p.x1 {
        return base_y as f64 - ys[ys.len() - 1] / p.global_max * p.layout.curve_h as f64;
    }
