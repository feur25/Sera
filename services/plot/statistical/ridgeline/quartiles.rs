use super::common::{
    area_path, close_svg, finalize, open_svg, percentile, polyline, prepare, project_pts,
    ridge_label, x_to_px, y_at_x,
};
...
pub fn render(cfg: &RidgelineConfig) -> String {
    ...
    for gi in (0..n_groups).rev() {
        ...
        if !sv.is_empty() {
            ...
            let items: [(f64, f64, &[u8]); 3] =
                [(q1, 1.2, b"3,3"), (med, 2.0, b""), (q3, 1.2, b"3,3")];
            for (q, sw, dash) in items {
                ...
                push_f2(&mut b, sw);
                if !dash.is_empty() {
                    push_b(&mut b, b"\" stroke-dasharray=\"");
                    push_b(&mut b, dash);
                }
                push_b(&mut b, b"\"/>");
            }
