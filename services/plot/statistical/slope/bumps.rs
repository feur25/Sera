use super::common::{dot, draw_axes, finalize, label_left, label_right, open_svg, prepare};
...
pub fn render(cfg: &SlopeConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        push_i(&mut b, y1);
        push_b(&mut b, b" C ");
        push_i(&mut b, l.x_left + dx as i32);
        b.push(b' ');
        push_i(&mut b, y1);
        b.push(b' ');
        push_i(&mut b, l.x_right - dx as i32);
        b.push(b' ');
        push_i(&mut b, y2);
        b.push(b' ');
        ...
        push_b(
            &mut b,
            b"\" stroke-width=\"2.6\" stroke-linecap=\"round\" opacity=\"0.85\"/>",
        );
