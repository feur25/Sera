...
pub fn default_x_widths(n_cols: usize) -> Vec<f64> {
    (0..n_cols).map(|i| 1.0 + (i as f64 * 1.31).sin().abs() * 1.6).collect()
}

pub fn default_y_heights(n_rows: usize) -> Vec<f64> {
...
pub fn render(cfg: &HeatmapConfig) -> String {
    ...
    let xw_default: Vec<f64> = if cfg.x_widths.is_empty() && n_cols > 0 {
        default_x_widths(n_cols)
    } else {
        Vec::new()
    };
    let yh_default: Vec<f64> = if cfg.y_heights.is_empty() && n_rows > 0 {
        default_y_heights(n_rows)
    } else {
        Vec::new()
    };
    let xw: &[f64] = if cfg.x_widths.is_empty() {
        &xw_default[..]
    } else {
        cfg.x_widths
    };
    let yh: &[f64] = if cfg.y_heights.is_empty() {
        &yh_default[..]
    } else {
        cfg.y_heights
    };
