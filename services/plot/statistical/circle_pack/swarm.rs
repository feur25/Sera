...
pub fn render(cfg: &CirclePackConfig) -> String {
    ...
    for i in 0..n {
        ...
        let group = cfg.parents[i].clone();
        if !group_order.iter().any(|g| g == &group) {
        ...
        commits.push(Commit { ctype, bot, author, lines, group });
    }
    if commits.is_empty() {
        return String::new();
    }
    ...
    for (ci, c) in commits.iter().enumerate() {
        by_group.entry(c.group.as_str()).or_default().push(ci);
    }
    ...
    let mut groups: Vec<Group> = Vec::with_capacity(group_order.len());
    for group in &group_order {
        let idxs = by_group.get(group.as_str()).cloned().unwrap_or_default();
    ...
    let ng = groups.len();
    let aspect = (plot_w / plot_h).max(0.1);
    let cols = ((ng as f64 * aspect).sqrt().ceil() as usize).clamp(1, ng);
    let rows = ng.div_ceil(cols);
    let cell_w = plot_w / cols as f64;
    let cell_h = plot_h / rows as f64;
    let cell_r = cell_w.min(cell_h) * 0.46;
    ...
    for (k, g) in groups.iter().enumerate() {
        let row = k / cols;
        let col = k % cols;
        let gx = left as f64 + col as f64 * cell_w + cell_w / 2.0;
        let gy = top as f64 + row as f64 * cell_h + cell_h / 2.0;
        ...
        if ng > 1 {
            ...
            push_f2(&mut buf, gx);
            push_b(&mut buf, b"\" y=\"");
            push_f2(&mut buf, gy + cell_r + 16.0);
            push_b(&mut buf, b"\" text-anchor=\"middle\" font-family=\"Arial,sans-serif\" font-size=\"10\" fill=\"#94a3b8\">");
            let sub = if g.total_lines >= 1000.0 {
                format!("{} \u{b7} {} commits \u{b7} {:.1}k lines", group_order[k], g.count, g.total_lines / 1000.0)
            } else {
                format!("{} \u{b7} {} commits \u{b7} {:.0} lines", group_order[k], g.count, g.total_lines)
            };
            escape_xml(&mut buf, &sub);
        ...
        for (slot_i, &ci) in g.order.iter().enumerate() {
            ...
            let px = gx + lx * fit_scale;
            let py = gy + ly * fit_scale;
    ...
    let stats: [(&str, String); 4] = [
        ...
        ("GROUPS", group_order.len().to_string()),
    ];
    let mut sy = 22_i32;
...
mod tests {
    ...
    fn synth(n: usize) -> (Vec<String>, Vec<String>, Vec<f64>) {
        ...
        let groups = ["2026-01", "2026-02", "2026-03", "2026-04"];
        ...
        for i in 0..n {
            ...
            parents.push(groups[i % groups.len()].to_string());
    ...
    fn a_single_group_renders_without_a_frame_ring() {
        ...
        let labels: Vec<String> = (0..n)
            ...
            .collect();
        let parents = vec!["only".to_string(); n];
        let values: Vec<f64> = (0..n).map(|i| ((i % 40) + 1) as f64 * 6.0).collect();
        let html = render(&cfg(&labels, &parents, &values));
        assert!(!html.is_empty());
    ...
    #[test]
    fn every_group_cluster_stays_within_the_plot_bounds() {
