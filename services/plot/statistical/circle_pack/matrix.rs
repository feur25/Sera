...
fn draw_glyph(buf: &mut Vec<u8>, kind: &str, cx: f64, cy: f64, r: f64) {
    ...
    match kind {
        "star" => {
            ...
            push_b(buf, b"\" fill=\"none\" stroke=\"#f5f7fb\" stroke-width=\"");
            push_f2(buf, sw);
...
fn staircase_path(buf: &mut Vec<u8>, col_x: &[f64], col_w: &[f64], ys: &[f64]) {
    ...
    push_b(buf, b"\" fill=\"none\" stroke=\"#eef2fa\" stroke-width=\"2.2\" stroke-linecap=\"round\" stroke-opacity=\"0.92\"/>");
}
...
pub fn render(cfg: &CirclePackConfig) -> String {
...

    {
        ...
        for sym in sym_order.iter() {
            ...
            legend_dot(&mut buf, fx + 6.0, fy - 3.0, 6.5, b"5b6b90", 0.85);
    ...
    push_b(&mut buf, b"\" rx=\"10\" fill=\"none\" stroke=\"#eef2fa\" stroke-width=\"2.2\" stroke-opacity=\"0.92\"/>");
    ...
    for r in 0..n_rows {
        for c in 0..n_cols {
            ...
            order.sort_by(|&a, &b| hash01(a).partial_cmp(&hash01(b)).unwrap_or(std::cmp::Ordering::Equal));
            let base_radii: Vec<f64> = order.iter().map(|&i| radii[i]).collect();
            let shelf_order: Vec<usize> = (0..base_radii.len()).collect();
            ...
            let fits = |k: f64| -> (Vec<(f64, f64)>, f64, f64) {
                let scaled: Vec<f64> = base_radii.iter().map(|&r| r * k).collect();
                let (pos, h) = shelf_pack(&shelf_order, &scaled, avail_w, 0.9 * k.max(0.2));
                let mut min_x = f64::MAX;
                let mut max_x = f64::MIN;
                for (i, &(px, _py)) in pos.iter().enumerate() {
                    let rr = scaled[i];
                    min_x = min_x.min(px - rr);
                    max_x = max_x.max(px + rr);
                }
                (pos, h, (max_x - min_x).max(1e-6))
            };
            ...
            let k = lo;
            let (raw_pos, natural_h, bbox_w) = fits(k);
            let local_radii: Vec<f64> = base_radii.iter().map(|&r| r * k).collect();

            let mid_x = {
                let mut min_x = f64::MAX;
                for (i, &(px, _py)) in raw_pos.iter().enumerate() {
                    min_x = min_x.min(px - local_radii[i]);
                }
                min_x + bbox_w / 2.0
            };
            let mid_y = natural_h / 2.0;
            ...
            for (oi, &ci) in order.iter().enumerate() {
                ...
                let px = cell_cx + (lx - mid_x);
                let py = cell_cy + (ly - mid_y);
                let rr = local_radii[oi].max(0.7);
                let s = &sats[ci];
                ...
                buf.extend_from_slice(&bg_hex);
                push_b(&mut buf, b"\" stroke-width=\"0.5\"/>");
