...
pub fn render(cfg: &CandlestickConfig) -> String {
    ...
    push_b(
        &mut b,
        b"<defs><linearGradient id=\"sp-cm-grad\" x1=\"0\" y1=\"0\" x2=\"0\" y2=\"1\">",
    );
    push_b(&mut b, b"<stop offset=\"0%\" stop-color=\"#");
    b.extend_from_slice(&hx);
    push_b(&mut b, b"\" stop-opacity=\"0.55\"/>");
    ...
    push_b(
        &mut b,
        b"<polygon fill=\"url(#sp-cm-grad)\" stroke=\"none\" points=\"",
    );
    push_i(&mut b, cx_at(l, 0));
    push_b(&mut b, b",");
    push_i(&mut b, y_base);
    for i in 0..p.n {
        push_b(&mut b, b" ");
        push_i(&mut b, cx_at(l, i));
        push_b(&mut b, b",");
        push_i(&mut b, val_to_y(l, p.close[i]));
    }
    push_b(&mut b, b" ");
    push_i(&mut b, cx_at(l, p.n - 1));
    push_b(&mut b, b",");
    push_i(&mut b, y_base);
    push_b(&mut b, b"\"/>");
    ...
    push_b(
        &mut b,
        b"\" stroke-width=\"2\" stroke-linejoin=\"round\" stroke-linecap=\"round\" points=\"",
    );
    for i in 0..p.n {
        let cx = cx_at(l, i);
    ...
    for i in 0..p.n {
        let cx = cx_at(l, i);
        let y = val_to_y(l, p.close[i]);
        push_b(&mut b, b"<circle");
        data_attrs(&mut b, &p, i);
        push_b(&mut b, b" cx=\"");
        push_i(&mut b, cx);
        push_b(&mut b, b"\" cy=\"");
        push_i(&mut b, y);
        push_b(&mut b, b"\" r=\"2\" fill=\"#");
        b.extend_from_slice(&hx);
        push_b(&mut b, b"\"/>");
    }
    finalize(b, cfg)
