...
pub fn draw(b: &mut Vec<u8>, cfg: &GaugeConfig, p: &super::common::Prepared) {
    ...
    for i in 0..=20 {
        ...
        push_b(b, b"\" y2=\"");
        push_f2(b, yb);
        push_b(b, b"\" stroke=\"#475569\" stroke-width=\"1\"/>");
        if i % 5 == 0 {
            ...
            push_b(b, b"\" y=\"");
            push_f2(b, yl + 3.0);
            push_b(b, b"\" text-anchor=\"middle\" font-family=\"Arial,sans-serif\" font-size=\"9\" fill=\"#64748b\">");
    ...
    push_b(
        b,
        b"\" stroke=\"#0f172a\" stroke-width=\"2.4\" stroke-linecap=\"round\"/>",
    );
    push_b(b, b"<circle cx=\"");
    push_f2(b, p.cx);
    push_b(b, b"\" cy=\"");
    push_f2(b, p.cy);
    push_b(b, b"\" r=\"6\" fill=\"#0f172a\"/>");
    value_text(b, cfg, p.cx, p.cy + 50.0, 22);
    label_text(b, cfg, p.cx, p.cy + 68.0);
}
