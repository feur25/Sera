...
pub(crate) fn ordered_leaves(nodes: &[TreeNode], roots: &[usize]) -> Vec<usize> {
    let n = nodes.len();
...
pub(crate) fn write_radial_link(
    buf: &mut Vec<u8>,
    cx: f64, cy: f64,
    px: f64, py: f64,
    x: f64, y: f64,
...
pub(crate) fn svg_header(buf: &mut Vec<u8>, cfg: &DendrogramConfig, hid: u64, title_x: f64) {
...
pub fn render_impl(cfg: &DendrogramConfig, horizontal: bool, compact: bool) -> String {
    ...
    let (frontier, hidden) = if compact {
        super::compact::frontier_and_hidden(&nodes, &roots)
    } else {
    ...
    if compact {
        super::compact::write_frontier_wedges(&mut buf, cfg, &nodes, &frontier);
    }
