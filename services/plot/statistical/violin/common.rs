...
pub fn write_violin_v(
    ...
    push_i(&mut f.buf, start_x);
    f.buf.push(b' ');
    push_i(&mut f.buf, start_y);
    if build_right {
        for si in 1..=steps {
    ...
    push_f2(&mut f.buf, fill_opacity);
    push_b(&mut f.buf, b"\" stroke=\"#");
    f.buf.extend_from_slice(&hx);
    push_b(&mut f.buf, b"\" stroke-width=\"");
    push_f2(&mut f.buf, stroke_width);
    push_b(&mut f.buf, b"\"/>");
}
...
pub fn write_violin_h(
    ...
    escape_xml(&mut f.buf, label);
    push_b(&mut f.buf, b"\" d=\"M ");
    push_i(&mut f.buf, xv(0));
    f.buf.push(b' ');
    push_i(&mut f.buf, cy + h_at(0));
    for si in 1..=steps {
        push_b(&mut f.buf, b" L ");
        push_i(&mut f.buf, xv(si));
        f.buf.push(b' ');
        push_i(&mut f.buf, cy + h_at(si));
    }
    for si in (0..=steps).rev() {
        push_b(&mut f.buf, b" L ");
        push_i(&mut f.buf, xv(si));
...
pub fn draw_inner_box_v(
    ...
    push_i(&mut f.buf, cx - bw);
    push_b(&mut f.buf, b"\" y=\"");
    push_i(&mut f.buf, top);
    push_b(&mut f.buf, b"\" width=\"");
    push_i(&mut f.buf, bw * 2);
    push_b(&mut f.buf, b"\" height=\"");
    push_i(&mut f.buf, (bot - top).max(2));
    push_b(
        &mut f.buf,
        b"\" fill=\"#1a202c\" fill-opacity=\"0.85\" rx=\"2\"/>",
    );
    push_b(&mut f.buf, b"<line x1=\"");
    push_i(&mut f.buf, cx);
    push_b(&mut f.buf, b"\" y1=\"");
    push_i(&mut f.buf, yv(g.min));
    push_b(&mut f.buf, b"\" x2=\"");
    push_i(&mut f.buf, cx);
    push_b(&mut f.buf, b"\" y2=\"");
    push_i(&mut f.buf, yv(g.max));
    push_b(
        &mut f.buf,
        b"\" stroke=\"#1a202c\" stroke-width=\"1\" stroke-opacity=\"0.55\"/>",
    );
...
pub fn draw_mean_v(f: &mut Frame, cx: i32, half_w: i32, g: &GroupStat, v_min: f64, range: f64) {
    let yv = |v: f64| f.pt + f.ph - ((v - v_min) / range * f.ph as f64) as i32;
    let y_mean = yv(g.mean);
    push_b(&mut f.buf, b"<line x1=\"");
    push_i(&mut f.buf, cx - half_w);
    push_b(&mut f.buf, b"\" y1=\"");
    push_i(&mut f.buf, y_mean);
    push_b(&mut f.buf, b"\" x2=\"");
    push_i(&mut f.buf, cx + half_w);
    push_b(&mut f.buf, b"\" y2=\"");
    push_i(&mut f.buf, y_mean);
    push_b(
        &mut f.buf,
        b"\" stroke=\"#facc15\" stroke-width=\"2\" stroke-dasharray=\"6 3\"/>",
    );
    push_b(&mut f.buf, b"<circle cx=\"");
    push_i(&mut f.buf, cx);
    push_b(&mut f.buf, b"\" cy=\"");
    ...
    push_b(
        &mut f.buf,
        b"\" r=\"3\" fill=\"#fff\" stroke=\"#1a202c\" stroke-width=\"1\"/>",
    );
}
