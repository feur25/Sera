use super::common::{
    draw_cat_label_h, estimate_bw, finish, group_data, kde_curve, make_frame, open_axes_x,
    sort_groups, value_range, write_violin_h,
};
...
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    for (ci, g) in groups.iter().enumerate() {
        ...
        push_i(&mut f.buf, xv(g.q1));
        push_b(&mut f.buf, b"\" y1=\"");
        push_i(&mut f.buf, cy - (half_h as f64 * 0.25) as i32);
        push_b(&mut f.buf, b"\" x2=\"");
        push_i(&mut f.buf, xv(g.q3));
        ...
        push_i(&mut f.buf, xv(g.median));
        push_b(&mut f.buf, b"\" cy=\"");
        push_i(&mut f.buf, cy - (half_h as f64 * 0.25) as i32);
        push_b(&mut f.buf, b"\" r=\"2.4\" fill=\"#fff\" data-kv-Median=\"");
        push_f2(&mut f.buf, g.median);
