use super::common::{finalize, open, point, prepare, write_dots};
...
pub fn render(cfg: &ParallelConfig) -> String {
    ...
    push_b(
        &mut b,
        b"<filter id=\"dlxpf\" x=\"-40%\" y=\"-40%\" width=\"280%\" height=\"280%\">",
    );
    push_b(
        &mut b,
        b"<feGaussianBlur stdDeviation=\"3\" result=\"bk\"/>",
    );
    push_b(
        &mut b,
        b"<feMerge><feMergeNode in=\"bk\"/><feMergeNode in=\"SourceGraphic\"/></feMerge>",
    );
