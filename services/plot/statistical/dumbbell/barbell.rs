...
pub fn render(cfg: &DumbbellConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        push_i(&mut f.buf, (i as i32) * 2);
        push_b(&mut f.buf, b"\" data-series=\"0\" x=\"");
        push_i(&mut f.buf, x1 - 7);
        push_b(&mut f.buf, b"\" y=\"");
        push_i(&mut f.buf, cy - 11);
        push_b(
            &mut f.buf,
            b"\" width=\"14\" height=\"22\" rx=\"2\" fill=\"#",
        );
        f.buf.extend_from_slice(&hx1);
        push_b(&mut f.buf, b"\"/>");
        push_b(&mut f.buf, b"<rect data-idx=\"");
        push_i(&mut f.buf, (i as i32) * 2 + 1);
        push_b(&mut f.buf, b"\" data-series=\"1\" x=\"");
        push_i(&mut f.buf, x2 - 7);
        push_b(&mut f.buf, b"\" y=\"");
        push_i(&mut f.buf, cy - 11);
        push_b(
            &mut f.buf,
