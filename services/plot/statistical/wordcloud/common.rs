...
pub fn mask_box_inside(
    ...
    for (px, py) in pts.iter() {
        let nx = (px - cx) / rx;
        let ny = (py - cy) / ry;
        if !mask_inside(mask, mw, mh, nx, ny) {
            return false;
        }
    }
    true
...
pub fn place_words(
    ...
    height: f64,
    pad_t: f64,
    test: &dyn Fn(f64, f64, f64, f64, f64, f64, f64, f64) -> bool,
) -> Vec<PlacedWord> {
    let n = words.len();
...
pub fn render_labelmap(cfg: &WordCloudConfig) -> String {
    ...
    for i in 0..n {
        let xv = cfg.points_x[i];
        let yv = cfg.points_y[i];
        if xv < x_min {
            x_min = xv;
        }
        if xv > x_max {
    ...
    for i in 0..n {
        let cid = if i < cfg.point_clusters.len() {
            cfg.point_clusters[i].max(0) as usize
        } else {
            0
        };
    ...
    for cid in 0..n_lab {
        ...
        push_f2(&mut buf, cy);
        push_b(&mut buf, b"\" x2=\"");
        push_f2(&mut buf, anchor_x);
        push_b(&mut buf, b"\" y2=\"");
        push_f2(&mut buf, ly + 7.0);
        push_b(
            &mut buf,
            b"\" stroke=\"#475569\" stroke-width=\"0.6\" stroke-opacity=\"0.65\"/>",
        );
...
pub fn render_network(cfg: &WordCloudConfig) -> String {
    ...
    for k in 0..m {
        ...
        let weight = if k < cfg.edges_w.len() {
            cfg.edges_w[k]
        } else {
            1.0
        };
...
pub fn render_bubble(cfg: &WordCloudConfig) -> String {
    ...
    let radii: Vec<f64> = p
        .freqs
        .iter()
        .map(|&f| min_r + (f / max_f).powf(0.5) * (max_r - min_r))
        .collect();
    ...
    for &(bx, by, br, idx) in &circles {
        ...
        push_b(
            &mut buf,
            b"\" fill-opacity=\"0.82\" stroke=\"rgba(255,255,255,0.18)\" stroke-width=\"1\"/>",
        );
        ...
        push_f2(&mut buf, bx);
        push_b(&mut buf, b"\" y=\"");
        push_f2(&mut buf, by + fs * 0.38);
        push_b(&mut buf, b"\" text-anchor=\"middle\" font-family=\"'Segoe UI',Arial,sans-serif\" font-weight=\"700\" font-size=\"");
        push_f2(&mut buf, fs);
        push_b(
            &mut buf,
            b"\" fill=\"rgba(255,255,255,0.92)\" style=\"cursor:pointer\">",
        );
        escape_xml(&mut buf, word);
        push_b(&mut buf, b"</text>");
    }
...
pub fn render_context(cfg: &WordCloudConfig) -> String {
    ...
    if m > 0 {
        ...
        for _iter in 0..220 {
            ...
            for k in 0..m {
                ...
                let we = if k < cfg.edges_w.len() {
                    cfg.edges_w[k].abs().max(0.1)
                } else {
                    1.0
                };
    ...
    for i in 0..n {
        ...
        buf.push(b'%');
        push_b(&mut buf, b"\" x=\"");
        push_f2(&mut buf, pos_x[i]);
        push_b(&mut buf, b"\" y=\"");
        push_f2(&mut buf, pos_y[i]);
