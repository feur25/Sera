...
#[crate::chart_demo(
    "labels=[\"Design\",\"Build\",\"Test\",\"Launch\"], start=[0,5,12,18], end=[6,14,19,22], categories=[\"Plan\",\"Dev\",\"Dev\",\"Plan\"], color_values=[1.0,0.6,0.25,0.0]"
)]
...
pub fn render(cfg: &GanttConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        push_b(&mut f.buf, b"\" fill-opacity=\"0.28\" stroke=\"#");
        f.buf.extend_from_slice(&color);
        push_b(&mut f.buf, b"\" stroke-width=\"1\"/>");

        let fill_w = (w as f64 * p.progress[i]).round() as i32;
        if fill_w > 0 {
        ...
        if bar_h > 10.0 {
            ...
            push_i(&mut f.buf, (p.progress[i] * 100.0).round() as i32);
