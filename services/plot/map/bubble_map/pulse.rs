...
#[crate::chart_demo(
    "labels=[\"JP\",\"ID\",\"CL\",\"TR\",\"US\",\"NZ\",\"PH\",\"MX\",\"IR\",\"PK\",\"GR\",\"IT\",\"PE\",\"CN\",\"IN\",\"AF\",\"NP\",\"EC\",\"PG\",\"VU\"], values=[9.1,8.7,8.9,7.4,7.2,7.8,7.6,7.0,6.8,6.5,6.2,6.0,7.1,6.9,5.8,6.6,7.3,6.7,7.9,7.5], title=\"Seismic Activity Index\", variant=\"pulse\""
)]
...
pub fn render(cfg: &BubbleMapConfig) -> String {
    ...
    for &(i, value, shape) in &matched {
        ...
        for (ring, opacity) in [(1.7, 0.16), (1.25, 0.32)] {
            svg.push_str(&format!(
                "<circle cx=\"{cx:.1}\" cy=\"{cy:.1}\" r=\"{:.1}\" fill=\"rgb({r},{g},{b})\" fill-opacity=\"{opacity}\"/>",
                radius * ring
            ));
        }
        svg.push_str(&format!(
            "<circle cx=\"{cx:.1}\" cy=\"{cy:.1}\" r=\"{radius:.1}\" fill=\"rgb({r},{g},{b})\" fill-opacity=\"0.92\" stroke=\"white\" stroke-width=\"1\" data-index=\"{i}\"/>"
        ));
        svg.push_str(&format!(
