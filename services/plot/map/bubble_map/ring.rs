...
#[crate::chart_demo(
    "labels=[\"CA\",\"TX\",\"NY\",\"FL\",\"IL\",\"PA\",\"OH\",\"WA\",\"GA\",\"NC\",\"MI\",\"NJ\",\"VA\",\"AZ\",\"MA\"], values=[38.9,30.5,19.6,22.6,12.6,12.9,11.8,7.8,11.0,10.8,10.0,9.3,8.7,7.4,7.0], title=\"Population by State, ring width = magnitude\", map=\"usa_states\", variant=\"ring\""
)]
...
pub fn render(cfg: &BubbleMapConfig) -> String {
    ...
    for &(i, value, shape) in &matched {
        ...
        let stroke_width = 1.5 + t * 9.0;
        let (r, g, b) = PALETTE[i % PALETTE.len()];
        svg.push_str(&format!(
            "<circle cx=\"{cx:.1}\" cy=\"{cy:.1}\" r=\"{radius:.1}\" fill=\"none\" stroke=\"rgb({r},{g},{b})\" stroke-opacity=\"0.85\" stroke-width=\"{stroke_width:.1}\" data-index=\"{i}\"/>"
        ));
        svg.push_str(&format!(
