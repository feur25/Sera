...
macro_rules! declare_named_region_set {
    ($(($mod_name:ident, $svg_key:literal, $width:expr, $height:expr, $key:literal, $aliases:expr, $display:literal, $groups:expr)),+ $(,)?) => {
        $(
            pub mod $mod_name {
                ...
                fn get_shapes() -> &'static Vec<CountryShape> {
                    SHAPES.get_or_init(|| {
                        let svg = $crate::plot::map::regions::pack::map_asset($svg_key);
