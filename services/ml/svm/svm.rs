use crate::ml::linalg::{dot, solve_spd, splitmix64};
use rayon::prelude::*;
    ...
    pub fn fit(&mut self, x: &[f64], n: usize, p: usize, y: &[i32]) {
        ...
        if self.classes.len() == 2 {
            let labels: Vec<f64> = y
                .iter()
                .map(|&yi| if yi == self.classes[1] { 1.0 } else { -1.0 })
                .collect();
            let (w, b) = train_linear_svm(
                x,
                n,
                p,
                &labels,
            ...
            let results: Vec<(Vec<f64>, f64)> = cls
                .par_iter()
                .map(|&ci| {
                    let labels: Vec<f64> = y
                        .iter()
                        .map(|&yi| if yi == ci { 1.0 } else { -1.0 })
                        .collect();
...
impl LinearSVR {
            ...
            for si in 0..n {
...

                {
                    ...
                    if a > 1e-12 {
                        ...
                        for j in 0..p {
                            unsafe {
                                *w.get_unchecked_mut(j) += d * *xi.get_unchecked(j);
                            }
                        }
    ...
    pub fn predict(&self, x: &[f64], n: usize, p: usize) -> Vec<f64> {
        if n >= 512 {
            (0..n)
                .into_par_iter()
                .map(|i| dot(unsafe { x.get_unchecked(i * p..(i + 1) * p) }, &self.w) + self.b)
                .collect()
        } else {
            (0..n)
                .map(|i| dot(&x[i * p..(i + 1) * p], &self.w) + self.b)
                .collect()
        }
    }
...
fn train_linear_svm(
    ...
    for iter in 0..max_iter.min(100) {
        ...
        if gnorm < tol * gnorm_init.max(1.0) {
            break;
        }
