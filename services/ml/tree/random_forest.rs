use crate::ml::tree::decision_tree::{
    compute_bins, BinInfo, DecisionTreeClassifier, DecisionTreeRegressor, TreeCriterion,
};
use rayon::prelude::*;
...
impl RandomForestClassifier {
    ...
    pub fn fit(&mut self, x: &[f64], n: usize, p: usize, y: &[i32]) {
        ...
        let trees: Vec<DecisionTreeClassifier> = (0..self.n_estimators)
            .into_par_iter()
            .map(|seed| {
                ...
                let bins = BinInfo {
                    edges: master_bins.edges.clone(),
                    n_bins: master_bins.n_bins.clone(),
                    binned: binned_boot,
                    p,
                    n,
                };
    ...
    pub fn predict(&self, x: &[f64], n: usize, p: usize) -> Vec<i32> {
        ...
        let vote_one = |i: usize| -> i32 {
            ...
            for preds in &all_preds {
                unsafe {
                    *votes.get_unchecked_mut(*cmap.get_unchecked((preds[i] - cmin) as usize)) += 1;
                }
            }
            let mut best = 0;
            let mut best_v = 0u32;
    ...
    pub fn predict_proba(&self, x: &[f64], n: usize, p: usize) -> Vec<Vec<f64>> {
        let nc = self.n_classes;
        let all_proba: Vec<Vec<Vec<f64>>> = self
            .trees
            .par_iter()
            .map(|t| t.predict_proba(x, n, p))
            .collect();
        let inv = 1.0 / self.n_estimators as f64;
        let proba_one = |i: usize| -> Vec<f64> {
            let mut avg = vec![0.0; nc];
            for proba in &all_proba {
                for (j, &v) in proba[i].iter().enumerate() {
                    avg[j] += v;
                }
            }
            for v in &mut avg {
        ...
        if n >= 256 {
            (0..n)
                ...
                .map(|i| {
                    let mut s = 0.0;
                    for preds in &all_preds {
                        s += unsafe { *preds.get_unchecked(i) };
                    }
                    s * inv
                })
                .collect()
