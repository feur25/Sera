...
#[crate::sera_doc(
    ...
    en = "PCA — Principal Component Analysis via truncated SVD.",
    fr = "PCA — Analyse en Composantes Principales via SVD tronquée.",
...
pub fn ml_pca(input: &str) -> String {
    ...
    model.fit(&xf, n, p);
    let transformed = if nt > 0 {
        model.transform(&xtf, nt, p)
    } else {
        model.transform(&xf, n, p)
    };
    let out_n = if nt > 0 { nt } else { n };
    let rows: Vec<Vec<f64>> = (0..out_n)
        .map(|i| transformed[i * n_components..(i + 1) * n_components].to_vec())
        .collect();
    format!(
        r#"{{"components":{},"explained_variance_ratio":{}}}"#,
