use crate::ml::linalg::{dot, mat_t_mat, mat_t_y, mat_vec, qr_solve, solve_spd};
use rayon::prelude::*;
                    ...
                    .map(|ci| {
                        ...
                        while r + 4 <= e {
                            ...
                            for i in 0..p {
                                ...
                                for j in i..p {
                                    pa[base + j] +=
                                        a0 * r0[j] + a1 * r1[j] + a2 * r2[j] + a3 * r3[j];
                                }
...
fn solve_normal(x: &[f64], n: usize, p: usize, y: &[f64]) -> Vec<f64> {
    if n >= p * 4 {
        ...
        if let Some(w) = solve_spd(&ata, p, &atb) {
            return w;
        }
    }
    qr_solve(x, n, p, y)
