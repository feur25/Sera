...
pub fn render_bars_3d(ctx: Bar3DRenderContext) {
    ...
    for (vis_idx, &actual_idx) in ctx.visible_indices.iter().enumerate() {
        ...
        if let (Some(proj_base), Some(proj_top)) = (
            ...
            if screen_base.x.is_finite()
                && screen_base.y.is_finite()
                && screen_top.x.is_finite()
                && screen_top.y.is_finite()
            {
    ...
    for (screen_base, screen_top, color, actual_idx, _) in bars_to_render {
        ...
        ctx.painter
            .circle_filled(screen_base, base_radius, display_color);
        ctx.painter
            .circle_filled(screen_top, top_radius, display_color);
