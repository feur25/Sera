...
pub fn build_grid(input: &str) -> String {
    ...
    let bg_color = opts.background.clone().or_else(crate::plot::get_global_bg).unwrap_or_else(|| "transparent".to_string());
...
pub fn build_slideshow(input: &str) -> String {
    ...
    let show_title = if !title.is_empty() { title } else { opts.label.as_deref().unwrap_or("") };
...
pub fn chart_append(input: &str) -> String {
    ...
    let payload: In = match serde_json::from_str(input) { Ok(v) => v, Err(_) => return String::new() };
    let mut xs = payload.prev_x.unwrap_or_default();
    let mut ys = payload.prev_y.unwrap_or_default();
    if let Some(nx) = payload.x { xs.extend(nx); }
    if let Some(ny) = payload.y { ys.extend(ny); }
    if let Some(limit) = payload.max_points {
        if xs.len() > limit { let cut = xs.len() - limit; xs.drain(..cut); }
        if ys.len() > limit { let cut = ys.len() - limit; ys.drain(..cut); }
    }
    let title = payload.title.unwrap_or_default();
