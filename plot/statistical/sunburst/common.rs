...
pub const SB_PALETTE: &[u32] = &[
    ...
    0x14B8A6, 0x818CF8, 0xFB7185, 0x34D399, 0xFBBF24, 0xA78BFA, 0x22D3EE, 0xF472B6, 0xA3E635,
    0xF87171, 0x2DD4BF,
];

pub const FLAME_PALETTE: &[u32] = &[0xB91C1C, 0xEA580C, 0xF59E0B, 0xFBBF24, 0xFDE68A, 0xFEF3C7];
...
pub fn prepare(cfg: &SunburstConfig) -> Option<Prepared> {
    prepare_with_hole(cfg, 0.09)
}
...
pub fn arc_path(buf: &mut Vec<u8>, cx: i32, cy: i32, r1: i32, r2: i32, a1: f64, a2: f64) {
    ...
    let (x1o, y1o) = (
        cx as f64 + r2 as f64 * a1.cos(),
        cy as f64 + r2 as f64 * a1.sin(),
    );
    let (x2o, y2o) = (
        cx as f64 + r2 as f64 * a2.cos(),
        cy as f64 + r2 as f64 * a2.sin(),
    );
    let (x1i, y1i) = (
        cx as f64 + r1 as f64 * a2.cos(),
        cy as f64 + r1 as f64 * a2.sin(),
    );
    let (x2i, y2i) = (
        cx as f64 + r1 as f64 * a1.cos(),
        cy as f64 + r1 as f64 * a1.sin(),
    );
    push_b(buf, b"M ");
    push_f2(buf, x1o);
    buf.push(b' ');
...
pub fn label_arc(buf: &mut Vec<u8>, p: &Prepared, i: usize, white: bool) {
    ...
    let short: &str = if n_chars <= max_ch {
        lbl.as_str()
    } else {
        let end = lbl
            .char_indices()
