use super::common::{
    arc_path, color_hex, finalize, label_arc, open_svg, prepare_with_hole, ring_radii,
    slice_data_attrs,
};
...
pub fn render(cfg: &SunburstConfig) -> String {
    let p = match prepare_with_hole(cfg, 0.20) {
        Some(v) => v,
    ...
    if p.grand_total >= 1_000_000.0 {
        ...
        push_b(&mut b, b"M");
    } else if p.grand_total >= 1000.0 {
        push_f2(&mut b, p.grand_total / 1000.0);
        push_b(&mut b, b"k");
    } else {
        push_f2(&mut b, p.grand_total);
    }
    push_b(&mut b, b"</text>");
    let _ = escape_xml;
    finalize(b, cfg)
