...
pub fn render(cfg: &GaugeConfig) -> String {
    ...
    arc_path(&mut b, p.cx, p.cy, p.radius, start_a, start_a - total, 0x1E293B, p.arc_w + 2.0, 0.4);
    if p.frac > 0.001 {
        ...
        push_b(&mut b, b"\" stroke-linecap=\"round\" filter=\"url(#sp-g-glow)\"/>");
        push_b(&mut b, b"<circle cx=\""); push_f2(&mut b, x2);
        push_b(&mut b, b"\" cy=\""); push_f2(&mut b, y2);
        push_b(&mut b, b"\" r=\""); push_f2(&mut b, p.arc_w * 0.6);
