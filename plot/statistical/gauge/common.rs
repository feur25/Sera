...
pub fn prepare_with(cfg: &GaugeConfig, cy_factor: f64, r_factor: f64) -> Prepared {
    ...
    let thresholds: Vec<(f64, u32)> = if cfg.thresholds.is_empty() {
        ...
        cfg.thresholds
            .iter()
            .map(|&(v, c)| (((v - cfg.min_val) / range).clamp(0.0, 1.0), c))
            .collect()
    };
    Prepared {
        range,
...
pub fn prepare(cfg: &GaugeConfig) -> Prepared {
    prepare_with(cfg, 0.65, 0.38)
}
...
pub fn arc_path(
    ...
    let large = if sweep_diff > std::f64::consts::PI {
        1
    } else {
        0
    };
    ...
    push_f2(buf, w);
    push_b(buf, b"\" stroke-linecap=\"round\" opacity=\"");
    push_f2(buf, opacity);
    push_b(buf, b"\"/>");
}
