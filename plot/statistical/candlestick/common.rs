...
use crate::plot::statistical::common::{sorted, push_b, push_i, push_f2, escape_xml, hex6, truncate, svg_open, svg_title, svg_axis_lines, svg_x_label, svg_y_label, sort_indices};
...
pub fn prepare(cfg: &CandlestickConfig) -> Option<Prepared> {
    ...
    for i in 0..n {
        if low[i] < vmin { vmin = low[i]; }
        if high[i] > vmax { vmax = high[i]; }
    }
    ...
    let pad_l: i32 = 68; let pad_t: i32 = 38; let pad_b: i32 = 52; let pad_r: i32 = 20;
    ...
    Some(Prepared {
        n, labels, open, high, low, close,
        layout: Layout { pad_l, pad_t, pad_b, pad_r, plot_w, plot_h, vmin, vmax, vrange, pitch, body_w },
        up_color, dn_color,
    })
}
...
pub fn open_with_axes(buf: &mut Vec<u8>, cfg: &CandlestickConfig, p: &Prepared) {
    ...
    for i in 0..=n_yticks {
        ...
        if cfg.gridlines && i > 0 {
            push_b(buf, b"<line class=\"sp-gl\" x1=\""); push_i(buf, l.pad_l);
            push_b(buf, b"\" y1=\""); push_i(buf, y);
            push_b(buf, b"\" x2=\""); push_i(buf, l.pad_l + l.plot_w);
            push_b(buf, b"\" y2=\""); push_i(buf, y);
            push_b(buf, b"\" stroke=\"#e5e7eb\" stroke-width=\"0.5\"/>");
        }
        push_b(buf, b"<text class=\"sp-yt\" x=\""); push_i(buf, l.pad_l - 6);
        push_b(buf, b"\" y=\""); push_i(buf, y + 3);
        push_b(buf, b"\" text-anchor=\"end\" font-family=\"Arial,sans-serif\" font-size=\"9\" fill=\"#6b7280\">");
        if val >= 1000.0 { push_i(buf, val as i32); } else { push_f2(buf, val); }
        push_b(buf, b"</text>");
    }
    svg_axis_lines(buf, l.pad_l, l.pad_t, l.plot_w, l.plot_h);
    svg_x_label(buf, cfg.x_label, l.pad_l + l.plot_w / 2, l.pad_t + l.plot_h + 42);
    svg_y_label(buf, cfg.y_label, 14, l.pad_t, l.plot_h);
    let n = p.n;
    if n <= 60 {
        for i in 0..n {
            if i % ((n / 12).max(1)) == 0 {
                push_b(buf, b"<text x=\""); push_i(buf, cx_at(l, i));
                push_b(buf, b"\" y=\""); push_i(buf, l.pad_t + l.plot_h + 14);
...
pub fn data_attrs(buf: &mut Vec<u8>, p: &Prepared, i: usize) {
    ...
    push_b(buf, b"\" data-lbl=\""); escape_xml(buf, &p.labels[i]);
    push_b(buf, b"\" data-kv-Open=\""); push_f2(buf, p.open[i]);
    push_b(buf, b"\" data-kv-High=\""); push_f2(buf, p.high[i]);
    push_b(buf, b"\" data-kv-Low=\""); push_f2(buf, p.low[i]);
    push_b(buf, b"\" data-kv-Close=\""); push_f2(buf, p.close[i]);
    push_b(buf, b"\"");
}
...
pub fn color_hex(c: u32) -> [u8; 6] { hex6(c) }

pub fn heikin_ashi(open: &[f64], high: &[f64], low: &[f64], close: &[f64]) -> (Vec<f64>, Vec<f64>, Vec<f64>, Vec<f64>) {
    let n = open.len();
    let mut ho = vec![0.0; n]; let mut hh = vec![0.0; n];
    let mut hl = vec![0.0; n]; let mut hc = vec![0.0; n];
    if n == 0 { return (ho, hh, hl, hc); }
