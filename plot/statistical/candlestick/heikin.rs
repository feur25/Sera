use super::common::{prepare, open_with_axes, finalize, val_to_y, cx_at, data_attrs, color_hex, heikin_ashi};
...
pub fn render(cfg: &CandlestickConfig) -> String {
    ...
    let l = &p.layout;
    let bw = l.body_w;
    let (ho, hh, hl, hc) = heikin_ashi(&p.open, &p.high, &p.low, &p.close);
    for i in 0..p.n {
        let cx = cx_at(l, i);
        let y_high = val_to_y(l, hh[i]);
        let y_low = val_to_y(l, hl[i]);
        let y_open = val_to_y(l, ho[i]);
        let y_close = val_to_y(l, hc[i]);
        let up = hc[i] >= ho[i];
        ...
        push_b(&mut b, b"\" stroke-width=\"1.2\"/>");
        ...
        push_b(&mut b, b"\" rx=\"2\" fill=\"#"); b.extend_from_slice(&hx);
        push_b(&mut b, b"\" stroke=\"#"); b.extend_from_slice(&hx);
