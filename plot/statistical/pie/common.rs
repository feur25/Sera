...
pub fn render_pie_svg(
    ...
    let span = if piece.arc_span > 0.0 && piece.arc_span <= 2.0 * PI { piece.arc_span } else { 2.0 * PI };
    ...
    if !piece.pattern.is_empty() {
        ...
        for i in 0..n {
            ...
            match piece.pattern.as_str() {
                "dots" => { push_b(buf, b"<circle cx=\"5\" cy=\"5\" r=\"1.6\" fill=\"#0d1117\" opacity=\"0.55\"/>"); }
                "diagonal" => { push_b(buf, b"<path d=\"M0,10 L10,0\" stroke=\"#0d1117\" stroke-width=\"1.4\" opacity=\"0.55\"/>"); }
                "cross" => { push_b(buf, b"<path d=\"M0,5 L10,5 M5,0 L5,10\" stroke=\"#0d1117\" stroke-width=\"1.2\" opacity=\"0.5\"/>"); }
                _ => { push_b(buf, b"<path d=\"M0,3 L10,3 M0,7 L10,7\" stroke=\"#0d1117\" stroke-width=\"1.4\" opacity=\"0.55\"/>"); }
            }
    ...
    if !piece.center_text.is_empty() {
        let main_size = if r_inner > 0.0 { (r_inner * 0.55).max(14.0).min(36.0) } else { 22.0 };
        push_b(buf, b"<text x=\""); push_f2(buf, cx);
        ...
        if !piece.center_subtext.is_empty() {
            push_b(buf, b"<text x=\""); push_f2(buf, cx);
            push_b(buf, b"\" y=\""); push_f2(buf, cy + main_size * 0.85);
...
pub fn write_title(buf: &mut Vec<u8>, w: i32, title: &str) {
    ...
    push_b(buf, b"<text x=\""); push_i(buf, w / 2);
...
where F: FnOnce(&mut PiePiece, &PieConfig) {
