use super::common::{prepare, open, finalize, color_for, dot, data_attrs};
use super::config::LollipopConfig;
use crate::plot::statistical::common::{push_b, push_i, push_f2, escape_xml, hex6, truncate, svg_axis_lines, svg_vgrid};

pub const DEMO_KWARGS: &str = "labels=[\"Alpha\",\"Beta\",\"Gamma\",\"Delta\",\"Epsilon\"], series=[[24,38,17,42,29],[18,28,22,35,33]], series_names=[\"2023\",\"2024\"]";
pub fn render(cfg: &LollipopConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        push_b(&mut b, b"\" stroke=\"#e5e7eb\" stroke-width=\"0.8\" stroke-dasharray=\"2,3\"/>");
        push_b(&mut b, b"<line");
        data_attrs(&mut b, &p, i);
        push_b(&mut b, b" x1=\""); push_i(&mut b, baseline);
        push_b(&mut b, b"\" y1=\""); push_i(&mut b, cy);
        push_b(&mut b, b"\" x2=\""); push_i(&mut b, x_v);
        push_b(&mut b, b"\" y2=\""); push_i(&mut b, cy);
        push_b(&mut b, b"\" stroke=\"#"); b.extend_from_slice(&hx);
        push_b(&mut b, b"\" stroke-width=\"2\"/>");
        dot(&mut b, &p, i, x_v, cy, 6, col);
        push_b(&mut b, b"<text x=\""); push_i(&mut b, pl - 8);
        ...
        if cfg.show_values {
            ...
            push_b(&mut b, b"<text x=\""); push_i(&mut b, dx);
            push_b(&mut b, b"\" y=\""); push_i(&mut b, cy + 4);
            push_b(&mut b, b"\" text-anchor=\""); b.extend_from_slice(anchor);
    ...
    for ti in 0..=n_xticks {
        ...
        push_b(&mut b, b"\" y=\""); push_i(&mut b, pt + ph + 14);
