...
pub const DEMO_KWARGS: &str = "labels=[\"Revenue\",\"Profit\",\"CSAT\"], values=[80,65,4.2], targets=[90,70,4.5], max_vals=[120,100,5], comparisons=[70,55,3.8]";
pub fn render(cfg: &BulletConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        push_b(&mut b, b"\" height=\""); push_i(&mut b, ghost_h);
        push_b(&mut b, b"\" fill=\"#"); b.extend_from_slice(&hx_track);
        push_b(&mut b, b"\" rx=\"3\"/>");
        let prior = if p.comparisons[i] > 0.0 { p.comparisons[i] } else if p.ranges[i] > 0.0 { p.ranges[i] } else { 0.0 };
        if prior > 0.0 {
            let pw = ((prior / max_v).min(1.0) * p.plot_w as f64) as i32;
            push_b(&mut b, b"<rect x=\""); push_i(&mut b, p.pad_l);
            push_b(&mut b, b"\" y=\""); push_i(&mut b, ry);
            push_b(&mut b, b"\" width=\""); push_i(&mut b, pw);
            push_b(&mut b, b"\" height=\""); push_i(&mut b, ghost_h);
            push_b(&mut b, b"\" fill=\"#"); b.extend_from_slice(&hx_ghost);
            push_b(&mut b, b"\" rx=\"3\"/>");
        }
        let value_w = ((p.values[i] / max_v).min(1.0) * p.plot_w as f64) as i32;
        data_value_rect(&mut b, &p, i, p.pad_l, by, value_w, bar_h, 0x6366F1, 2, 0.95);
        if p.targets[i] > 0.0 {
            ...
            push_b(&mut b, b"\" y2=\""); push_i(&mut b, ry + ghost_h + 2);
