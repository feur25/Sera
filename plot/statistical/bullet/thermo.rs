use super::common::{prepare, max_for};
...
pub fn render(cfg: &BulletConfig) -> String {
    ...
    let pad_l = 36;
    let pad_r = 24;
    let auto_w = if cfg.width < p.n as i32 * 90 + pad_l + pad_r { p.n as i32 * 90 + pad_l + pad_r } else { cfg.width };
    ...
    for i in 0..p.n {
        ...
        let tube_h = by - ty;
        push_b(&mut b, b"<rect x=\""); push_i(&mut b, cx - tube_w / 2);
        push_b(&mut b, b"\" y=\""); push_i(&mut b, ty);
        push_b(&mut b, b"\" width=\""); push_i(&mut b, tube_w);
        push_b(&mut b, b"\" height=\""); push_i(&mut b, tube_h);
        push_b(&mut b, b"\" fill=\"#"); b.extend_from_slice(&hx_track);
        push_b(&mut b, b"\" rx=\""); push_i(&mut b, tube_w / 2);
        push_b(&mut b, b"\"/>");
        ...
        push_b(&mut b, b"\" x=\""); push_i(&mut b, cx - tube_w / 2);
        push_b(&mut b, b"\" y=\""); push_i(&mut b, fy);
        push_b(&mut b, b"\" width=\""); push_i(&mut b, tube_w);
        push_b(&mut b, b"\" height=\""); push_i(&mut b, fill_h);
        push_b(&mut b, b"\" fill=\"#"); b.extend_from_slice(&hx_value);
        push_b(&mut b, b"\" rx=\""); push_i(&mut b, tube_w / 2);
        push_b(&mut b, b"\"/>");
        push_b(&mut b, b"<circle cx=\""); push_i(&mut b, cx);
        push_b(&mut b, b"\" cy=\""); push_i(&mut b, by + bulb_r - 2);
        push_b(&mut b, b"\" r=\""); push_i(&mut b, bulb_r);
        push_b(&mut b, b"\" fill=\"#"); b.extend_from_slice(&hx_value);
        push_b(&mut b, b"\"/>");
        if p.targets[i] > 0.0 {
            let yt = by - ((p.targets[i] / max_v).min(1.0) * tube_h as f64) as i32;
            push_b(&mut b, b"<line x1=\""); push_i(&mut b, cx - tube_w);
            push_b(&mut b, b"\" y1=\""); push_i(&mut b, yt);
            push_b(&mut b, b"\" x2=\""); push_i(&mut b, cx + tube_w);
            push_b(&mut b, b"\" y2=\""); push_i(&mut b, yt);
            push_b(&mut b, b"\" stroke=\"#1a202c\" stroke-width=\"2.2\"/>");
        }
        push_b(&mut b, b"<text x=\""); push_i(&mut b, cx);
        push_b(&mut b, b"\" y=\""); push_i(&mut b, cfg.height - pad_b + 18);
