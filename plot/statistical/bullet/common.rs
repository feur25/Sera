...
use crate::plot::statistical::common::{sorted, sort_indices, push_b, push_i, push_f2, escape_xml, hex6, truncate};
...
pub fn prepare(cfg: &BulletConfig) -> Option<Prepared> {
    ...
    let comparisons: Vec<f64> = idx.iter().map(|&i| cfg.comparisons.get(i).copied().unwrap_or(0.0)).collect();
    ...
    Some(Prepared { n, labels, values, targets, max_vals, ranges, comparisons, auto_h, pad_l, pad_r, pad_t, row_h, plot_w })
}

pub fn max_for(p: &Prepared, i: usize) -> f64 {
    if p.max_vals[i] > 0.0 { p.max_vals[i] } else { (p.values[i].max(p.targets[i]).max(p.comparisons[i]) * 1.2).max(1.0) }
}
...
pub fn open_svg(buf: &mut Vec<u8>, cfg: &BulletConfig, p: &Prepared) {
    ...
    push_i(buf, p.auto_h); push_b(buf, b"\" viewBox=\"0 0 ");
    push_i(buf, cfg.width); push_b(buf, b" ");
    push_i(buf, p.auto_h); push_b(buf, b"\">");
...
pub fn label_left(buf: &mut Vec<u8>, p: &Prepared, i: usize, by: i32, bar_h: i32) {
    push_b(buf, b"<text x=\""); push_i(buf, p.pad_l - 6);
    push_b(buf, b"\" y=\""); push_i(buf, by + bar_h / 2 + 4);
...
pub fn value_text(buf: &mut Vec<u8>, v: f64, x: i32, y: i32) {
    ...
    push_b(buf, b"\" font-family=\"Arial,sans-serif\" font-size=\"9\" fill=\"#374151\">");
    if v.abs() >= 1_000_000.0 { push_f2(buf, v / 1_000_000.0); push_b(buf, b"M"); }
    else if v.abs() >= 1_000.0 { push_f2(buf, v / 1_000.0); push_b(buf, b"k"); }
    else { push_f2(buf, v); }
    push_b(buf, b"</text>");
}

pub fn data_value_rect(buf: &mut Vec<u8>, p: &Prepared, i: usize, x: i32, y: i32, w: i32, h: i32, color: u32, rx: i32, opacity: f32) {
    let hx = hex6(color);
    push_b(buf, b"<rect data-idx=\""); push_i(buf, i as i32);
    push_b(buf, b"\" data-y=\""); push_f2(buf, p.values[i]);
    push_b(buf, b"\" data-lbl=\""); escape_xml(buf, &p.labels[i]);
    push_b(buf, b"\" x=\""); push_i(buf, x);
    push_b(buf, b"\" y=\""); push_i(buf, y);
    push_b(buf, b"\" width=\""); push_i(buf, w.max(2));
    push_b(buf, b"\" height=\""); push_i(buf, h);
    push_b(buf, b"\" fill=\"#"); buf.extend_from_slice(&hx);
    push_b(buf, b"\" rx=\""); push_i(buf, rx);
    push_b(buf, b"\" opacity=\""); push_f2(buf, opacity as f64);
    push_b(buf, b"\"/>");
}
