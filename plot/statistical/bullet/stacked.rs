...
pub fn render(cfg: &BulletConfig) -> String {
    ...
    let zones = [
        (0.40_f64, 0xCBD5E1_u32),
        (0.75_f64, 0xE0E7FF_u32),
        (1.00_f64, 0xC7D2FE_u32),
    ];
    for i in 0..p.n {
        ...
        for (frac, color) in zones.iter() {
            ...
            push_i(&mut b, p.pad_l + prev);
            push_b(&mut b, b"\" y=\"");
            push_i(&mut b, ry);
            push_b(&mut b, b"\" width=\"");
            push_i(&mut b, x_end - (p.pad_l + prev));
        ...
        data_value_rect(
            &mut b, &p, i, p.pad_l, by, value_w, bar_h, 0x1E293B, 2, 0.92,
        );
        if p.targets[i] > 0.0 {
