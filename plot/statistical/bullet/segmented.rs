...
pub fn render(cfg: &BulletConfig) -> String {
    ...
    let zones = [
        (0.40_f64, 0xFCA5A5_u32),
        (0.75_f64, 0xFCD34D_u32),
        (1.00_f64, 0x86EFAC_u32),
    ];
    for i in 0..p.n {
        ...
        for (frac, color) in zones.iter() {
            ...
            push_i(&mut b, p.pad_l + prev + 1);
            push_b(&mut b, b"\" y=\"");
            push_i(&mut b, ry);
            push_b(&mut b, b"\" width=\"");
            push_i(&mut b, (x_end - prev - 2).max(1));
            push_b(&mut b, b"\" height=\"");
            push_i(&mut b, range_h);
            push_b(&mut b, b"\" fill=\"#");
        ...
        data_value_rect(
            &mut b, &p, i, p.pad_l, by, value_w, bar_h, 0x111827, 2, 0.92,
        );
        if p.targets[i] > 0.0 {
