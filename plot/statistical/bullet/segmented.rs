...
pub const DEMO_KWARGS: &str = "labels=[\"Revenue\",\"Profit\",\"CSAT\"], values=[80,65,4.2], targets=[90,70,4.5], max_vals=[120,100,5], ranges=[[40,70,100],[30,60,90],[2,3,4]]";
pub fn render(cfg: &BulletConfig) -> String {
    ...
    let zones = [(0.40_f64, 0xFCA5A5_u32), (0.75_f64, 0xFCD34D_u32), (1.00_f64, 0x86EFAC_u32)];
    for i in 0..p.n {
        ...
        for (frac, color) in zones.iter() {
            let x_end = ((*frac) * p.plot_w as f64) as i32;
            let hx = hex6(*color);
            push_b(&mut b, b"<rect x=\""); push_i(&mut b, p.pad_l + prev + 1);
            push_b(&mut b, b"\" y=\""); push_i(&mut b, ry);
            push_b(&mut b, b"\" width=\""); push_i(&mut b, (x_end - prev - 2).max(1));
        ...
        data_value_rect(&mut b, &p, i, p.pad_l, by, value_w, bar_h, 0x111827, 2, 0.92);
        if p.targets[i] > 0.0 {
            let tx = p.pad_l + ((p.targets[i] / max_v).min(1.0) * p.plot_w as f64) as i32;
            push_b(&mut b, b"<line x1=\""); push_i(&mut b, tx);
            push_b(&mut b, b"\" y1=\""); push_i(&mut b, ry - 3);
            push_b(&mut b, b"\" x2=\""); push_i(&mut b, tx);
            push_b(&mut b, b"\" y2=\""); push_i(&mut b, ry + range_h + 3);
            push_b(&mut b, b"\" stroke=\"#0f172a\" stroke-width=\"2.6\"/>");
        }
