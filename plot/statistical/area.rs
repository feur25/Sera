...
pub fn render_area_html(cfg: &AreaConfig) -> String {
    ...
    for si in (0..n_ser).rev() {
        ...
        for i in 0..n_pts {
            ...
            let val = if cfg.stacked {
                stacked_sums[si][i]
            } else {
                cfg.series[si].1.get(i).copied().unwrap_or(0.0)
            };
        ...
        push_b(&mut f.buf, b"\" fill-opacity=\".35\" stroke=\"#");
        f.buf.extend_from_slice(&hx);
    ...
    for si in 0..n_ser {
        ...
        for i in (0..n_pts).step_by(hover_step) {
            let val = if cfg.stacked {
                ...
                svals.get(i).copied().unwrap_or(0.0)
            };
            let frac = (val / max_val).clamp(0.0, 1.0);
            let x = f.pl + (i as f64 * step_x) as i32;
            let y = f.pt + ((1.0 - frac) * f.ph as f64) as i32;
            ...
            push_b(
                &mut f.buf,
                b"\" stroke=\"#fff\" stroke-width=\".8\" opacity=\"0\"/>",
            );
        }
    }
    ...
    for (si, (sname, _)) in series.iter().enumerate() {
        ...
        push_b(
            &mut f.buf,
            b"\" width=\"12\" height=\"12\" rx=\"2\" fill=\"#",
        );
        f.buf.extend_from_slice(&hx);
...
pub fn build_area_chart(input: &str) -> String {
    ...
    use crate::plot::statistical::{render_area_html, AreaConfig};
    let hover = o.hj();
    ...
    let html = render_area_html(&AreaConfig {
        ...
        x_labels: &x_labels,
        series: &series,
        stacked: o.stacked.unwrap_or(false),
        palette: &o.pal(),
        x_label: &o.xl(),
        y_label: &o.yl(),
        gridlines: o.grid(),
