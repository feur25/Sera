...
pub fn render(cfg: &FunnelConfig) -> String {
    ...
    let n_i = p.n as i32;
    for i in 0..p.n {
        let next_ratio = if i + 1 == p.n {
            0.0
        } else {
            (n_i - 1 - i as i32) as f64 / n_i as f64
        };
        let ratio = next_ratio.max(p.values[i] / p.max_val * (n_i - i as i32) as f64 / n_i as f64);
