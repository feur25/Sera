...
pub fn render(cfg: &FunnelConfig) -> String {
    ...
    for i in 0..n {
        ...
        push_b(&mut b, b"<path data-idx=\""); push_i(&mut b, rev_i as i32);
        push_b(&mut b, b"\" data-lbl=\""); escape_xml(&mut b, &p.labels[rev_i]);
        push_b(&mut b, b"\" d=\"M ");
        ...
        if cfg.show_text {
            ...
            label_left_pct(&mut b, l.pad_l - 8, y + l.step_h / 2, p.values[rev_i], p.max_val);
            label_right_val(&mut b, cfg.width - l.pad_r + 8, y + l.step_h / 2, p.values[rev_i]);
        }
        prev_ratio = ratio;
    }
    finalize(b, cfg)
