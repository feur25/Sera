...
pub fn render(cfg: &FunnelConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        push_b(&mut b, b"\" d=\"M ");
        push_i(&mut b, x0); b.push(b' '); push_i(&mut b, y);
        push_b(&mut b, b" L "); push_i(&mut b, x1 - tip); b.push(b' '); push_i(&mut b, y);
        push_b(&mut b, b" L "); push_i(&mut b, x1); b.push(b' '); push_i(&mut b, y + h / 2);
        push_b(&mut b, b" L "); push_i(&mut b, x1 - tip); b.push(b' '); push_i(&mut b, y + h);
        push_b(&mut b, b" L "); push_i(&mut b, x0); b.push(b' '); push_i(&mut b, y + h);
        push_b(&mut b, b" L "); push_i(&mut b, x0 + tip); b.push(b' '); push_i(&mut b, y + h / 2);
