...
use crate::plot::statistical::common::{sorted, sort_indices, push_b, push_i, push_f2, escape_xml};
...
pub fn prepare(cfg: &WaterfallConfig) -> Option<Prepared> {
    ...
    let is_total: Vec<bool> = labels.iter().map(|l| {
        let lo = l.to_lowercase();
        lo.contains("total") || lo.contains("net") || lo.contains("final")
    }).collect();
    for i in 0..n {
        if is_total[i] { starts.push(0.0); ends.push(running); }
        else { starts.push(running); running += values[i]; ends.push(running); }
    }
    ...
    Some(Prepared {
        n, labels, values, starts, ends, is_total,
        layout: Layout { pad_l, pad_t, pad_b, plot_w, plot_h, bar_w, bar_step, min_val, max_val, range, zero_y },
    })
}
...
pub fn axes(buf: &mut Vec<u8>, cfg: &WaterfallConfig, p: &Prepared) {
    ...
    for ti in 0..=n_yticks {
        ...
        if cfg.gridlines && ti > 0 {
            push_b(buf, b"<line x1=\""); push_i(buf, l.pad_l);
        ...
        push_b(buf, b"<text x=\""); push_i(buf, l.pad_l - 4);
        ...
        else if v.abs() >= 1_000.0 { push_i(buf, v as i32); }
        else { push_f2(buf, v); }
        push_b(buf, b"</text>");
    }
    push_b(buf, b"<line x1=\""); push_i(buf, l.pad_l);
    push_b(buf, b"\" y1=\""); push_i(buf, l.zero_y);
    push_b(buf, b"\" x2=\""); push_i(buf, l.pad_l + l.plot_w);
    push_b(buf, b"\" y2=\""); push_i(buf, l.zero_y);
    push_b(buf, b"\" stroke=\"#cbd5e1\" stroke-width=\"0.8\"/>");
    push_b(buf, b"<line x1=\""); push_i(buf, l.pad_l);
    push_b(buf, b"\" y1=\""); push_i(buf, l.pad_t);
    push_b(buf, b"\" x2=\""); push_i(buf, l.pad_l);
...
pub fn bar_color(p: &Prepared, i: usize) -> u32 {
    if p.is_total[i] { COLOR_TOTAL }
    else if p.values[i] >= 0.0 { COLOR_POS }
    else { COLOR_NEG }
}
