...
pub fn render(cfg: &WaterfallConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        if i > 0 && !p.is_total[i] && p.ends[i - 1].abs() > 1e-9 {
            ...
            let arrow: &[u8] = if pct >= 0.0 { b"&#8593; +" } else { b"&#8595; " };
            let by = top - 4;
            push_b(&mut b, b"<text x=\""); push_i(&mut b, bx + l.bar_w / 2);
            push_b(&mut b, b"\" y=\""); push_i(&mut b, by);
            push_b(&mut b, b"\" text-anchor=\"middle\" font-family=\"Arial,sans-serif\" font-size=\"9\" font-weight=\"700\" fill=\"");
            b.extend_from_slice(badge_color);
            push_b(&mut b, b"\">"); b.extend_from_slice(arrow);
            push_f2(&mut b, pct.abs());
            push_b(&mut b, b"%</text>");
        }
    }
    finalize(b, cfg)
