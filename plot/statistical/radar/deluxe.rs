use super::common::{
    angle_at, draw_legend, finalize, polygon_pts, prepare, project, series_points,
};
...
pub fn render(cfg: &RadarConfig) -> String {
    ...
    push_b(
        &mut b,
        b"<filter id=\"dlxradf\" x=\"-50%\" y=\"-50%\" width=\"200%\" height=\"200%\">",
    );
    ...
    push_b(
        &mut b,
        b"<filter id=\"dlxradg\" x=\"-60%\" y=\"-60%\" width=\"220%\" height=\"220%\">",
    );
    ...
    push_b(&mut b, b"</filter>");
    for si in 0..p.n_ser {
        let col = if !cfg.palette.is_empty() {
            palette_color(cfg.palette, si)
        } else {
            neon_color(si)
        };
        push_b(&mut b, b"<radialGradient id=\"dlxradrg");
        ...
        b.extend_from_slice(&hex6(col));
        push_b(&mut b, b"\" stop-opacity=\"0.06\"/>");
    ...
    for ring in 1..=n_rings {
        ...
        for ai in 0..p.n_axes {
            ...
            push_f2(&mut b, cx + rr * a.cos());
            b.push(b',');
            push_f2(&mut b, cy - rr * a.sin());
        }
        push_b(
            &mut b,
        ...
        push_b(
            &mut b,
            b"\" font-family=\"-apple-system,Arial,sans-serif\" font-size=\"9\" fill=\"#334155\">",
        );
    ...
    for si in 0..p.n_ser {
        ...
        push_b(&mut b, b" fill=\"url(#dlxradrg");
        push_i(&mut b, si as i32);
        push_b(&mut b, b")\" stroke=\"#");
        b.extend_from_slice(&hx);
        push_b(
            &mut b,
            b"\" stroke-width=\"2\" stroke-linejoin=\"round\" filter=\"url(#dlxradg)\"/>",
        );
        for &(px, py) in &pts {
