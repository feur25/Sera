...
use crate::plot::statistical::common::{
    escape_xml, palette_color, push_b, push_f2, push_i, svg_legend_item, svg_open, svg_title,
};
...
pub fn draw_grid(buf: &mut Vec<u8>, cfg: &RadarConfig, p: &Prepared) {
    ...
    for ring in 1..=n_rings {
        ...
        push_b(
            buf,
            b"\" fill=\"none\" stroke=\"#e2e8f0\" stroke-width=\"0.9\" class=\"sp-gl\"/>",
        );
        let lx = cx + 4.0;
        let ly = cy - rr - 2.0;
        push_b(buf, b"<text x=\"");
...
pub fn dots_outlined(buf: &mut Vec<u8>, pts: &[(f64, f64)], hx: &[u8], r: f64) {
    for &(px, py) in pts {
        ...
        push_b(buf, b"\" r=\"");
        push_f2(buf, r);
        push_b(buf, b"\" fill=\"#ffffff\" stroke=\"#");
        buf.extend_from_slice(hx);
