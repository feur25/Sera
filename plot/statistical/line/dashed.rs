...
pub fn render(cfg: &LineConfig) -> String {
    ...
    let dash_patterns = ["8,4", "2,3", "10,3,2,3", "6,2", "1,3"];
    ...
    for (si, (_, vals)) in series_iter.iter().enumerate() {
        ...
        let hx = hex6(color);
        let pat = if cfg.dash_pattern.is_empty() || cfg.dash_pattern == "auto" {
            dash_patterns[si % dash_patterns.len()]
        } else { cfg.dash_pattern };
        ...
        push_b(&mut f.buf, b"\" stroke-dasharray=\""); push_b(&mut f.buf, pat.as_bytes());
        push_b(&mut f.buf, b"\" stroke-linecap=\"round\" points=\"");
        for i in 0..n {
