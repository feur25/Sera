...
pub fn render(cfg: &LineConfig) -> String {
    ...
    let mut f = Frame::new_html(cfg.title, cfg.width, cfg.height, 56, 42, 52, legend_w, n * 120 + 2048);
    ...
    for (si, (_, vals)) in series_iter.iter().enumerate() {
        ...
        push_b(&mut f.buf, b"\" fill-opacity=\"0."); push_i(&mut f.buf, opa);
        push_b(&mut f.buf, b"\" stroke=\"none\" d=\"M ");
        let x0 = f.pl;
        let y0 = if cfg.stack_fill { prev_top[0] } else { baseline_y };
        push_i(&mut f.buf, x0); f.buf.push(b' '); push_i(&mut f.buf, y0);
        let mut tops: Vec<i32> = Vec::with_capacity(n);
        for i in 0..n {
            let v = vals.get(i).copied().unwrap_or(0.0);
            let xi = f.pl + (i as f64 * step_x) as i32;
            let base = if cfg.stack_fill { prev_top[i] } else { baseline_y };
        ...
        for i in (0..n).rev() {
            let xi = f.pl + (i as f64 * step_x) as i32;
            let base = if cfg.stack_fill { prev_top[i] } else { baseline_y };
            push_b(&mut f.buf, b" L "); push_i(&mut f.buf, xi); f.buf.push(b' '); push_i(&mut f.buf, base);
        }
