...
pub fn render(cfg: &LineConfig) -> String {
    ...
    push_i(&mut b, total_w); push_b(&mut b, b"\" height=\"");
    push_i(&mut b, total_h); push_b(&mut b, b"\" viewBox=\"0 0 ");
    push_i(&mut b, total_w); push_b(&mut b, b" "); push_i(&mut b, total_h);
    push_b(&mut b, b"\"><rect class=\"sp-bg\" width=\"100%\" height=\"100%\"/>");

    if !cfg.title.is_empty() {
        push_b(&mut b, b"<text x=\""); push_i(&mut b, total_w / 2);
    ...
    for (si, (name, vals)) in series.iter().enumerate() {
        ...
        let color = if cfg.color_hex != 0 { cfg.color_hex } else { palette_color(cfg.palette, si) };
        let hx = hex6(color);

        push_b(&mut b, b"<rect x=\""); push_i(&mut b, x0);
        push_b(&mut b, b"\" y=\""); push_i(&mut b, y0);
        push_b(&mut b, b"\" width=\""); push_i(&mut b, cell_w);
        push_b(&mut b, b"\" height=\""); push_i(&mut b, cell_h);
        push_b(&mut b, b"\" fill=\"#f8fafc\" stroke=\"#e2e8f0\" stroke-width=\"1\" rx=\"4\"/>");

        if !name.is_empty() {
            push_b(&mut b, b"<text x=\""); push_i(&mut b, x0 + 6);
            push_b(&mut b, b"\" y=\""); push_i(&mut b, y0 + 12);
        ...
        let n = vals.len();
        if n < 2 { continue; }
        let mx = vals.iter().cloned().filter(|v| v.is_finite()).fold(f64::NEG_INFINITY, f64::max);
        let mn = vals.iter().cloned().filter(|v| v.is_finite()).fold(f64::INFINITY, f64::min);
        ...
        if let Some(last) = vals.last() {
            push_b(&mut b, b"<text x=\""); push_i(&mut b, x0 + cell_w - 6);
            push_b(&mut b, b"\" y=\""); push_i(&mut b, y0 + 12);
        ...
        push_b(&mut b, b"\" stroke-width=\""); push_f2(&mut b, cfg.stroke_width);
        push_b(&mut b, b"\" stroke-linecap=\"round\" stroke-linejoin=\"round\" points=\"");
        for i in 0..n {
            let v = vals[i];
            let xi = x0 + 6 + (i as f64 * step_x) as i32;
            let yi = y0 + inset_t + ((1.0 - (v - mn) / range) * plot_h as f64) as i32;
            if i > 0 { b.push(b' '); }
            push_i(&mut b, xi); b.push(b','); push_i(&mut b, yi);
        }
        push_b(&mut b, b"\"/>");
    }
