...
use crate::plot::statistical::common::{
    escape_xml, hex6, palette_color, push_b, push_i, truncate, Frame,
};

fn neon_col(i: usize, palette: &[u32]) -> u32 {
    if !palette.is_empty() {
        return palette_color(palette, i);
    }
    const COLS: [u32; 8] = [
        0x00F5FF, 0xFF00C8, 0x00FF88, 0xFFD600, 0xFF4D00, 0x7B00FF, 0x40FFAA, 0xFF0055,
    ];
    COLS[i % COLS.len()]
}
...
pub fn render(cfg: &LineConfig) -> String {
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"nnglw\" x=\"-80%\" y=\"-80%\" width=\"360%\" height=\"360%\">",
    );
    push_b(
        &mut f.buf,
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"nndot\" x=\"-100%\" y=\"-100%\" width=\"400%\" height=\"400%\">",
    );
    push_b(
        &mut f.buf,
    ...
    for (si, (_, vals)) in series_iter.iter().enumerate() {
        ...
        push_b(
            &mut f.buf,
            b"\" fill-opacity=\"0.07\" stroke=\"none\" d=\"M ",
        );
        ...
        if cfg.show_points {
            for &(xi, yi) in &pts {
                ...
                push_b(
                    &mut f.buf,
                    b"\" fill-opacity=\"1\" filter=\"url(#nndot)\"/>",
                );
            }
        }
    }
