...
fn prism_color(i: usize) -> u32 {
    const COLS: [u32; 8] = [
        0xF43F5E, 0xFB923C, 0xFACC15, 0x34D399,
...
pub fn render(cfg: &BarConfig) -> String {
    ...
    let mut f = Frame::new_html(cfg.title, cfg.width, cfg.height, 52, 38, 52, 20, n * 400 + 4096);
    f.open(cfg.title, false);

    push_b(&mut f.buf, b"<defs>");
    push_b(&mut f.buf, b"<filter id=\"prsmf\" x=\"-20%\" y=\"-20%\" width=\"140%\" height=\"140%\">");
    ...
    for i in 0..n {
        ...
        push_b(&mut f.buf, b"<stop offset=\"0\" stop-color=\"#"); f.buf.extend_from_slice(&hex6(base)); push_b(&mut f.buf, b"\" stop-opacity=\"1\"/>");
        push_b(&mut f.buf, b"<stop offset=\"0.5\" stop-color=\"#"); f.buf.extend_from_slice(&hex6(mid)); push_b(&mut f.buf, b"\" stop-opacity=\"0.95\"/>");
        ...
        push_b(&mut f.buf, b"<stop offset=\"0\" stop-color=\"#ffffff\" stop-opacity=\"0.35\"/>");
        push_b(&mut f.buf, b"<stop offset=\"0.4\" stop-color=\"#"); f.buf.extend_from_slice(&hex6(bright)); push_b(&mut f.buf, b"\" stop-opacity=\"0.5\"/>");
        push_b(&mut f.buf, b"<stop offset=\"1\" stop-color=\"#ffffff\" stop-opacity=\"0.0\"/>");
        push_b(&mut f.buf, b"</linearGradient>");
    }
    push_b(&mut f.buf, b"</defs>");
    ...
    for i in 0..n {
        ...
        push_b(&mut f.buf, b"\" x=\""); push_i(&mut f.buf, bx);
        ...
        push_b(&mut f.buf, b"\" fill=\"url(#prsmg"); push_i(&mut f.buf, i as i32);
        ...
        push_b(&mut f.buf, b"\" width=\""); push_i(&mut f.buf, bar_w.max(1));
        push_b(&mut f.buf, b"\" height=\""); push_i(&mut f.buf, bar_h.max(1));
        push_b(&mut f.buf, b"\" fill=\"url(#prsmfacet"); push_i(&mut f.buf, i as i32);
        push_b(&mut f.buf, b")\" rx=\"3\"/>");

        if bar_h > 10 {
            push_b(&mut f.buf, b"<line x1=\""); push_i(&mut f.buf, bx + facet_off);
            push_b(&mut f.buf, b"\" y1=\""); push_i(&mut f.buf, by);
            push_b(&mut f.buf, b"\" x2=\""); push_i(&mut f.buf, bx);
            push_b(&mut f.buf, b"\" y2=\""); push_i(&mut f.buf, by + facet_off);
            push_b(&mut f.buf, b"\" stroke=\"#ffffff\" stroke-width=\"0.7\" stroke-opacity=\"0.4\"/>");
            push_b(&mut f.buf, b"<line x1=\""); push_i(&mut f.buf, bx + bar_w - facet_off);
            push_b(&mut f.buf, b"\" y1=\""); push_i(&mut f.buf, by);
            push_b(&mut f.buf, b"\" x2=\""); push_i(&mut f.buf, bx + bar_w);
            push_b(&mut f.buf, b"\" y2=\""); push_i(&mut f.buf, by + facet_off);
            push_b(&mut f.buf, b"\" stroke=\"#ffffff\" stroke-width=\"0.7\" stroke-opacity=\"0.25\"/>");
        }

        push_b(&mut f.buf, b"<rect x=\""); push_i(&mut f.buf, bx);
        push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, by);
        push_b(&mut f.buf, b"\" width=\""); push_i(&mut f.buf, (bar_w as f64 * 0.28) as i32);
        push_b(&mut f.buf, b"\" height=\""); push_i(&mut f.buf, bar_h.max(1));
        push_b(&mut f.buf, b"\" fill=\"#ffffff\" fill-opacity=\"0.08\" rx=\"3\" filter=\"url(#prsmf)\"/>");
        ...
        if cfg.show_text && bar_h > 16 {
            push_b(&mut f.buf, b"<text x=\""); push_i(&mut f.buf, cx);
            push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, by + 14);
