...
pub fn render(cfg: &BarConfig) -> String {
    ...
    if !cfg.unit_description.is_empty() {
        push_b(&mut f.buf, b"<text x=\""); push_i(&mut f.buf, pl - 10);
        push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, pt - 12);
        push_b(&mut f.buf, b"\" font-family=\"Arial,sans-serif\" font-size=\"10\" fill=\"#6b7280\">Each icon = ");
    ...
    for i in 0..n {
        let count = counts[i];
        let n_cols = cols[i];
        let color = if cfg.color_hex != 0 { cfg.color_hex } else { palette_color(cfg.palette, i) };
        ...
        for ci in 0..n_cols {
            ...
            for k in 0..in_col {
                ...
                push_b(&mut f.buf, b"<rect x=\""); push_i(&mut f.buf, cx - icon_size / 2);
                push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, cy - icon_size / 2);
                push_b(&mut f.buf, b"\" width=\""); push_i(&mut f.buf, icon_size);
                push_b(&mut f.buf, b"\" height=\""); push_i(&mut f.buf, icon_size);
                push_b(&mut f.buf, b"\" rx=\"3\" fill=\""); push_hex(&mut f.buf, color);
                push_b(&mut f.buf, b"\"/>");
            }
            placed += in_col;
        }
        ...
        push_b(&mut f.buf, b"<text x=\""); push_i(&mut f.buf, center_x);
        push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, pt - 4);
        ...
        push_b(&mut f.buf, b"<text x=\""); push_i(&mut f.buf, center_x);
        push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, pt + col_h + 18);
