...
pub fn draw_box_vertical(
    ...
    push_i(&mut f.buf, y_whi);
    push_b(&mut f.buf, b"\" x2=\"");
    push_i(&mut f.buf, cx);
    push_b(&mut f.buf, b"\" y2=\"");
    push_i(&mut f.buf, box_top);
    push_b(&mut f.buf, b"\" stroke=\"#6b7280\" stroke-width=\"1.4\"/>");

    push_b(&mut f.buf, b"<line x1=\"");
    push_i(&mut f.buf, cx);
    push_b(&mut f.buf, b"\" y1=\"");
    push_i(&mut f.buf, box_bot);
    push_b(&mut f.buf, b"\" x2=\"");
    push_i(&mut f.buf, cx);
    push_b(&mut f.buf, b"\" y2=\"");
    push_i(&mut f.buf, y_wlo);
    push_b(&mut f.buf, b"\" stroke=\"#6b7280\" stroke-width=\"1.4\"/>");

    push_b(&mut f.buf, b"<line x1=\"");
    push_i(&mut f.buf, cx - box_hw / 2);
    push_b(&mut f.buf, b"\" y1=\"");
    push_i(&mut f.buf, y_whi);
    push_b(&mut f.buf, b"\" x2=\"");
    push_i(&mut f.buf, cx + box_hw / 2);
    ...
    if notch && st.n > 1 {
        ...
        push_f2(&mut f.buf, st.whisker_lo);
        push_b(&mut f.buf, b"\" data-kv-Max=\"");
        push_f2(&mut f.buf, st.whisker_hi);
        push_b(&mut f.buf, b"\" data-kv-Mean=\"");
        push_f2(&mut f.buf, st.mean);
        push_b(&mut f.buf, b"\" data-kv-N=\"");
        push_i(&mut f.buf, st.n as i32);
        push_b(&mut f.buf, b"\" data-kv-Outliers=\"");
        push_i(&mut f.buf, st.outliers.len() as i32);
        push_b(&mut f.buf, b"\" x=\"");
        push_i(&mut f.buf, cx - box_hw);
        push_b(&mut f.buf, b"\" y=\"");
        push_i(&mut f.buf, box_top);
        push_b(&mut f.buf, b"\" width=\"");
        push_i(&mut f.buf, box_hw * 2);
        push_b(&mut f.buf, b"\" height=\"");
        push_i(&mut f.buf, box_h);
        push_b(&mut f.buf, b"\" fill=\"#");
        f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" fill-opacity=\"");
        push_f2(&mut f.buf, fill_opacity);
        push_b(&mut f.buf, b"\" rx=\"3\" stroke=\"#");
        f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stroke-width=\"");
        push_f2(&mut f.buf, stroke_width);
        push_b(&mut f.buf, b"\"/>");
...
pub fn draw_outliers_vertical(
    ...
    for &ov in &st.outliers {
        ...
        push_b(
            &mut f.buf,
            b"\" fill-opacity=\"0.75\" stroke=\"#fff\" stroke-width=\"0.8\"/>",
        );
    }
}
