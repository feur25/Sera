...
#[crate::chart_demo(
    "labels=[\"Design\",\"Build\",\"Test\",\"Launch\"], start=[0,5,12,18], end=[6,14,19,18], categories=[\"Plan\",\"Dev\",\"Dev\",\"Plan\"]"
)]
...
pub fn render(cfg: &GanttConfig) -> String {
    ...
    for i in 0..p.n {
        ...
        if (p.end[i] - p.start[i]).abs() < 1e-9 {
            let s = bar_h * 0.62;
            let cxf = x1 as f64;
            let cyf = cy as f64;
            push_b(&mut f.buf, b"<polygon");
            bar_data_attrs(&mut f, &p, i);
            push_b(&mut f.buf, b" points=\"");
            push_f2(&mut f.buf, cxf);
            push_b(&mut f.buf, b",");
            push_f2(&mut f.buf, cyf - s);
            push_b(&mut f.buf, b" ");
            push_f2(&mut f.buf, cxf + s);
            push_b(&mut f.buf, b",");
            push_f2(&mut f.buf, cyf);
            push_b(&mut f.buf, b" ");
            push_f2(&mut f.buf, cxf);
            push_b(&mut f.buf, b",");
            push_f2(&mut f.buf, cyf + s);
