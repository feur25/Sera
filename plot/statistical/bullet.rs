...
pub fn render_bullet_html(cfg: &BulletConfig) -> String {
    ...
    for i in 0..n {
        let max_v = if i < cfg.max_vals.len() && cfg.max_vals[i] > 0.0 {
            cfg.max_vals[i]
        } else {
            let v = cfg.values[i];
            let t = if i < cfg.targets.len() { cfg.targets[i] } else { 0.0 };
            (v.max(t) * 1.2).max(1.0)
        };
        let ry = pad_t + i as i32 * row_h + (row_h - range_h) / 2;
        let by = pad_t + i as i32 * row_h + (row_h - bar_h) / 2;
        let range_pct = if i < cfg.ranges.len() && cfg.ranges[i] > 0.0 {
            (cfg.ranges[i] / max_v).min(1.0)
        } else { 0.75 };
        let range_w = (plot_w as f64 * range_pct) as i32;
        let value_w = ((cfg.values[i] / max_v).min(1.0) * plot_w as f64) as i32;
        ...
        if i < cfg.targets.len() && cfg.targets[i] > 0.0 {
            let tx = pad_l + ((cfg.targets[i] / max_v).min(1.0) * plot_w as f64) as i32;
        ...
        if cfg.values[i] >= 1_000_000.0 { push_f2(&mut b, cfg.values[i] / 1_000_000.0); push_b(&mut b, b"M"); }
        else if cfg.values[i] >= 1_000.0 { push_f2(&mut b, cfg.values[i] / 1_000.0); push_b(&mut b, b"k"); }
        else { push_f2(&mut b, cfg.values[i]); }
        push_b(&mut b, b"</text>");
    }
    push_b(&mut b, b"</svg>");
