...
pub fn render(cfg: &KdeConfig) -> String {
    ...
    let mut f = Frame::new_html(cfg.title, cfg.width, cfg.height, 56, 42, 52, legend_w, n_ser * n_pts * 28 + 2048);
    ...
    for (si, ys) in curves.iter().enumerate() {
        ...
        push_f2(&mut f.buf, sx0); push_b(&mut f.buf, b","); push_f2(&mut f.buf, base_y);
        for i in 0..xs.len() {
            ...
            push_b(&mut f.buf, b" L"); push_f2(&mut f.buf, sx_next); push_b(&mut f.buf, b","); push_f2(&mut f.buf, sy);
        }
        let sx_last = f.pl as f64 + (xs[xs.len() - 1] - x0) / xr * f.pw as f64;
        push_b(&mut f.buf, b" L"); push_f2(&mut f.buf, sx_last); push_b(&mut f.buf, b","); push_f2(&mut f.buf, base_y);
        push_b(&mut f.buf, b" Z\" fill=\"#"); f.buf.extend_from_slice(&hx);
        let op = cfg.fill_opacity as f64 / 255.0;
        push_b(&mut f.buf, b"\" fill-opacity=\""); push_f2(&mut f.buf, op);
