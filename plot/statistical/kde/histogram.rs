...
pub fn render(cfg: &KdeConfig) -> String {
    ...
    let curves: Vec<Vec<f64>> = series
        ...
        .map(|(_, v)| {
            ...
            let area: f64 = (1..raw.len())
                .map(|i| 0.5 * (raw[i] + raw[i - 1]) * (xs[i] - xs[i - 1]))
                .sum::<f64>()
                .max(1e-12);
    ...
    let hists: Vec<(Vec<f64>, f64)> = series
        .iter()
        .map(|(_, v)| histogram_normalized(v, x0, x1, bins))
        .collect();

    let y_max_curve = curves
        .iter()
