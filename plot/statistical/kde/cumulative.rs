use super::common::{build_curve, build_xs, cumulative as cum_integ, integrate, ordered_series, x_range};
...
pub fn render(cfg: &KdeConfig) -> String {
    ...
    let cdfs: Vec<Vec<f64>> = series.iter().map(|(_, v)| {
    ...
    let mut f = Frame::new_html(cfg.title, cfg.width, cfg.height, 56, 42, 52, legend_w, n_ser * n_pts * 18 + 2048);
    f.open(cfg.title, false);
    let yl = if cfg.y_label.is_empty() { "Cumulative density (CDF)" } else { cfg.y_label };
    ...
    for (si, ys) in cdfs.iter().enumerate() {
        ...
        push_b(&mut f.buf, b"<circle cx=\""); push_f2(&mut f.buf, mx);
        push_b(&mut f.buf, b"\" cy=\""); push_f2(&mut f.buf, my);
        push_b(&mut f.buf, b"\" r=\"3.2\" fill=\"#"); f.buf.extend_from_slice(&hx);
