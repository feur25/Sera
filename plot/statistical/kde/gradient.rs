...
pub fn render(cfg: &KdeConfig) -> String {
    ...
    let mut f = Frame::new_html(cfg.title, cfg.width, cfg.height, 56, 42, 52, legend_w, n_ser * n_pts * 24 + 4096);
    ...
    for si in 0..n_ser {
        ...
        let hx = hex6(color);
        push_b(&mut f.buf, b"<linearGradient id=\"sp-kdeg-"); push_i(&mut f.buf, si as i32);
        push_b(&mut f.buf, b"\" x1=\"0\" y1=\"0\" x2=\"0\" y2=\"1\">");
        push_b(&mut f.buf, b"<stop offset=\"0%\" stop-color=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stop-opacity=\"0.85\"/>");
        push_b(&mut f.buf, b"<stop offset=\"60%\" stop-color=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stop-opacity=\"0.30\"/>");
        push_b(&mut f.buf, b"<stop offset=\"100%\" stop-color=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stop-opacity=\"0\"/>");
    ...
    for (si, ys) in curves.iter().enumerate() {
        ...
        push_b(&mut f.buf, b" Z\" fill=\"url(#sp-kdeg-"); push_i(&mut f.buf, si as i32);
        push_b(&mut f.buf, b")\"/>");
