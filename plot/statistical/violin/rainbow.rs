use super::common::{
    ...
    make_frame, open_axes_y, rainbow_color, sort_groups, value_range, write_violin_v, Side,
};
...
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    if rainbow_palette.is_empty() { rainbow_palette.push(0x6366F1); }

    for (ci, g) in groups.iter().enumerate() {
        let cx = f.pl + (ci as f64 * slot_w + slot_w / 2.0) as i32;
        let color = rainbow_palette[ci];
        ...
        write_violin_v(&mut f, cx, half_w, Side::Both, &dens, max_d, vr.min, vr.range,
            color, (cfg.fill_opacity + 0.1).min(0.85), cfg.stroke_width, ci as i32, &g.label, &kv);
    ...
    finish(&mut f, &names, &rainbow_palette, cfg.x_label, cfg.y_label, legend_w);
