use super::common::{
    ...
    draw_cat_label_v, draw_quartile_lines_v, estimate_bw, finish, group_data, kde_curve,
...
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    for (ci, g) in groups.iter().enumerate() {
        ...
        write_violin_v(&mut f, cx, half_w, Side::Both, &dens, max_d, vr.min, vr.range,
            color, cfg.fill_opacity * 0.55, cfg.stroke_width, ci as i32, &g.label, &kv);
        draw_quartile_lines_v(&mut f, cx, half_w, g, vr.min, vr.range, color);
