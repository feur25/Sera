use super::common::{
    draw_cat_label_v, draw_points_v, estimate_bw, finish, group_data, kde_curve, make_frame,
...
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    for (ci, g) in groups.iter().enumerate() {
        ...
        draw_points_v(
            &mut f, cx, half_w, g, vr.min, vr.range, color, cfg.jitter, &mut rng,
        );
