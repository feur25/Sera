use super::common::{
    ...
    draw_cat_label_v, draw_points_v, estimate_bw, finish, group_data, kde_curve,
    make_frame, open_axes_y, sort_groups, value_range, write_violin_v, Side,
};
...
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    for (ci, g) in groups.iter().enumerate() {
        ...
        write_violin_v(&mut f, cx, half_w, Side::Both, &dens, max_d, vr.min, vr.range,
            color, cfg.fill_opacity * 0.45, cfg.stroke_width, ci as i32, &g.label, &kv);
        draw_points_v(&mut f, cx, half_w, g, vr.min, vr.range, color, cfg.jitter, &mut rng);
