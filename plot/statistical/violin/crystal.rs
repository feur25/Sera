...
fn crystal_color(ci: usize) -> u32 {
    const COLS: [u32; 6] = [0x7DD3FC, 0xC4B5FD, 0x6EE7B7, 0xFDA4AF, 0xFDE68A, 0xA5B4FC];
    COLS[ci % COLS.len()]
}
...
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"crygf\" x=\"-30%\" y=\"-10%\" width=\"160%\" height=\"120%\">",
    );
    push_b(
        &mut f.buf,
    ...
    for ci in 0..n_cats {
        ...
        push_b(
            &mut f.buf,
            b"<stop offset=\"0.55\" stop-color=\"#ffffff\" stop-opacity=\"0.72\"/>",
        );
        push_b(&mut f.buf, b"<stop offset=\"0.75\" stop-color=\"#");
        f.buf.extend_from_slice(&hex6(col));
    ...
    for (ci, g) in groups.iter().enumerate() {
        ...
        push_b(&mut f.buf, b" Z\" fill=\"url(#cryg");
        push_i(&mut f.buf, ci as i32);
        push_b(&mut f.buf, b")\" stroke=\"#");
        f.buf.extend_from_slice(&hx);
        push_b(
            &mut f.buf,
            b"\" stroke-width=\"1.2\" stroke-opacity=\"0.65\" filter=\"url(#crygf)\"/>",
        );
        ...
        while sy < bot_y {
            ...
            if hw > 2 {
                ...
                push_b(
                    &mut f.buf,
                    b"\" stroke=\"#ffffff\" stroke-width=\"0.5\" stroke-opacity=\"0.22\"/>",
                );
                let _ = si_frac;
            }
            sy += stripe_step;
        ...
        push_b(&mut f.buf, b" Z\" fill=\"none\" stroke=\"#");
        f.buf.extend_from_slice(&hx);
        push_b(
            &mut f.buf,
            b"\" stroke-width=\"1.8\" stroke-opacity=\"0.9\"/>",
        );
