...
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"dlxvf\" x=\"-80%\" y=\"-10%\" width=\"260%\" height=\"120%\">",
    );
    push_b(
        &mut f.buf,
    ...
    for (ci, g) in groups.iter().enumerate() {
        ...
        push_b(&mut f.buf, b" Z\" fill=\"url(#dlxvg");
        push_i(&mut f.buf, ci as i32);
        push_b(&mut f.buf, b")\" fill-opacity=\"");
        push_f2(&mut f.buf, (cfg.fill_opacity + 0.2).min(0.92));
        push_b(&mut f.buf, b"\" stroke=\"url(#dlxvg");
        push_i(&mut f.buf, ci as i32);
        push_b(&mut f.buf, b")\" stroke-width=\"");
        push_f2(&mut f.buf, (cfg.stroke_width + 0.5).min(2.5));
