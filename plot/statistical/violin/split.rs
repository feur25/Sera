...
pub const DEMO_KWARGS: &str = "labels=[\"A\",\"B\",\"C\"], values=[1.2,2.4,2.7,3.1,3.5,3.8,2.0,2.8,3.2,3.6,4.1,4.5,1.8,2.2,2.6,3.0,3.4,3.9], categories=[\"A\",\"A\",\"A\",\"A\",\"A\",\"A\",\"B\",\"B\",\"B\",\"B\",\"B\",\"B\",\"C\",\"C\",\"C\",\"C\",\"C\",\"C\"], color_groups=[\"L\",\"L\",\"L\",\"R\",\"R\",\"R\",\"L\",\"L\",\"L\",\"R\",\"R\",\"R\",\"L\",\"L\",\"L\",\"R\",\"R\",\"R\"]";
pub fn render(cfg: &ViolinConfig) -> String {
    ...
    for (ci, g) in groups.iter().enumerate() {
        ...
        let side = if in_pair == 0 { Side::Left } else { Side::Right };
        ...
        write_violin_v(&mut f, cx, half_w, side, &dens, max_d, vr.min, vr.range,
    ...
    for pair_idx in 0..n_pairs {
        ...
        let b_label = if pair_idx * 2 + 1 < n_cats { Some(&groups[pair_idx * 2 + 1].label) } else { None };
        ...
        push_b(&mut f.buf, b"\" stroke=\"#cbd5e1\" stroke-width=\"0.6\" stroke-dasharray=\"3 3\"/>");
        ...
        let trimmed = if combined.len() <= 22 { combined.as_str() } else { &combined[..22] };
