...
pub fn render(cfg: &HeatmapConfig) -> String {
    ...
    let svg_h = if cfg.height > 0 { cfg.height } else { pad_top + cell_w * n_rows_ext as i32 + pad_bottom };
    let plot_h = (svg_h - pad_top - pad_bottom).max(40);
    let cell_h = (plot_h / n_rows_ext as i32).max(6);

    let col_totals: Vec<f64> = (0..n_cols).map(|c| (0..n_rows).map(|r| data[r * n_cols + c]).filter(|v| v.is_finite()).sum()).collect();
    let row_totals: Vec<f64> = (0..n_rows).map(|r| (0..n_cols).map(|c| data[r * n_cols + c]).filter(|v| v.is_finite()).sum()).collect();
    ...
    for row in 0..n_rows {
        ...
        for col in 0..n_cols {
            ...
            let lum = 0.299 * ((color >> 16 & 0xFF) as f64) + 0.587 * ((color >> 8 & 0xFF) as f64) + 0.114 * ((color & 0xFF) as f64);
        ...
        push_b(&mut buf, b"\" height=\"");
        push_i(&mut buf, cell_h);
        push_b(&mut buf, b"\" fill=\"#1e293b\" stroke=\"#ffffff\" stroke-width=\"0.6\"/>");
        push_b(&mut buf, b"<text x=\"");
        push_i(&mut buf, cx + cell_w / 2);
        push_b(&mut buf, b"\" y=\"");
        push_i(&mut buf, ry + cell_h / 2 + 3);
    ...
    push_b(&mut buf, b"\" height=\"");
    push_i(&mut buf, cell_h);
    push_b(&mut buf, b"\" fill=\"#312e81\" stroke=\"#ffffff\" stroke-width=\"0.6\"/>");
    push_b(&mut buf, b"<text x=\"");
    push_i(&mut buf, cx + cell_w / 2);
    push_b(&mut buf, b"\" y=\"");
    push_i(&mut buf, ry + cell_h / 2 + 3);
