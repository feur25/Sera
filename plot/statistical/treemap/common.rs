...
pub fn prepare_with_pad(cfg: &TreemapConfig, inner_pad: f64) -> Option<Prepared> {
    ...
    let sorted_parents: Vec<String> =
        if has_parents && !cfg.sort_order.is_empty() && cfg.sort_order != "none" {
    ...
    let (leaf_indices, leaf_values): (Vec<usize>, Vec<f64>) = if has_parents {
        (0..n)
            .filter(|&i| sorted_values[i] > 0.0 && !sorted_parents[i].is_empty())
            .map(|i| (i, sorted_values[i]))
            .unzip()
    } else {
        (0..n)
            .filter(|&i| sorted_values[i] > 0.0)
            .map(|i| (i, sorted_values[i]))
            .unzip()
    };
    ...
    let rects: Vec<Rect> = if has_parents {
        ...
        for (gi, g) in groups.iter().enumerate() {
            ...
            for (k, &li) in g.1.iter().enumerate() {
                rects[li] = local[k];
            }
            let pi = parent_color.len();
            parent_color
                .entry(g.0.clone())
                .or_insert_with(|| palette_color(&palette, pi));
        }
        rects
    } else {
...
pub fn prepare(cfg: &TreemapConfig) -> Option<Prepared> {
    prepare_with_pad(cfg, 0.0)
}
...
fn layout(areas: &[f64], rect: &Rect, out: &mut [Rect], start: usize, end: usize) {
    ...
    if vertical {
        ...
        for i in start..split {
            let h = if row_total > 0.0 {
                areas[i] / row_total * rect.h
            } else {
                0.0
            };
            out[i] = Rect {
        ...
        let mut cx = rect.x;
        for i in start..split {
            let w = if row_total > 0.0 {
                areas[i] / row_total * rect.w
            } else {
                0.0
            };
            out[i] = Rect {
...
pub fn tile_data_attrs(buf: &mut Vec<u8>, p: &Prepared, ri: usize) {
    ...
    let pct = if p.total > 0.0 {
        p.leaf_values[ri] / p.total * 100.0
    } else {
        0.0
    };
    push_b(buf, b" data-idx=\"");
    push_i(buf, ri as i32);
    push_b(buf, b"\" data-lbl=\"");
    escape_xml(buf, &p.labels[oi]);
    push_b(buf, b"\" data-v=\"");
    push_f2(buf, p.leaf_values[ri]);
    push_b(buf, b"\" data-kv-Pct=\"");
    push_f2(buf, pct);
    buf.push(b'%');
    if p.has_parents {
        push_b(buf, b"\" data-kv-Group=\"");
        escape_xml(buf, &p.parents[oi]);
    }
    push_b(buf, b"\"");
}
...
pub fn label_inside(buf: &mut Vec<u8>, p: &Prepared, ri: usize, fill: &[u8], show_pct: bool) {
    ...
    let font_size: i32 = if r.w > 100.0 && r.h > 40.0 {
        12
    } else if r.w > 60.0 && r.h > 28.0 {
        10
    } else {
        8
    };
    ...
    if r.h > 36.0 {
        ...
        push_b(buf, b"\" font-weight=\"600\" fill=\"");
