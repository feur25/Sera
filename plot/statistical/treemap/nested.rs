use super::common::{prepare_with_pad, open_svg, finalize, leaf_color, label_inside, tile_data_attrs, rect_attrs, fill_hex};
...
pub fn render(cfg: &TreemapConfig) -> String {
    ...
    if p.has_parents {
        for (name, gr) in p.parent_groups.iter() {
            let mut frame = *gr;
            frame.x -= 8.0; frame.y -= 22.0; frame.w += 16.0; frame.h += 30.0;
            if frame.w < 4.0 || frame.h < 4.0 { continue; }
            let pi = p.parent_color.get(name).copied().unwrap_or(p.palette[0]);
            let hx = hex6(pi);
            push_b(&mut b, b"<rect");
            rect_attrs(&mut b, frame);
            push_b(&mut b, b" rx=\"6\" fill=\"#"); b.extend_from_slice(&hx);
            push_b(&mut b, b"\" fill-opacity=\"0.10\" stroke=\"#"); b.extend_from_slice(&hx);
            push_b(&mut b, b"\" stroke-width=\"1.4\"/>");
            push_b(&mut b, b"<text x=\""); push_f2(&mut b, frame.x + 10.0);
            push_b(&mut b, b"\" y=\""); push_f2(&mut b, frame.y + 14.0);
            push_b(&mut b, b"\" font-family=\"Arial,sans-serif\" font-size=\"11\" font-weight=\"700\" fill=\"#"); b.extend_from_slice(&hx);
