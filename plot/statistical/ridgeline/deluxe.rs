use super::common::{prepare, ridge_label, project_pts, area_path, polyline, close_svg, finalize};
...
pub fn render(cfg: &RidgelineConfig) -> String {
    ...
    for gi in 0..n_groups {
        ...
        push_b(&mut b, b"<linearGradient id=\"dlxrlg"); push_i(&mut b, gi as i32);
        push_b(&mut b, b"\" x1=\"0\" y1=\"0\" x2=\"1\" y2=\"0\">");
        push_b(&mut b, b"<stop offset=\"0\" stop-color=\"#"); b.extend_from_slice(&hex6(c0)); push_b(&mut b, b"\" stop-opacity=\"0.85\"/>");
        push_b(&mut b, b"<stop offset=\"0.5\" stop-color=\"#"); b.extend_from_slice(&hex6(c1)); push_b(&mut b, b"\" stop-opacity=\"0.7\"/>");
        push_b(&mut b, b"<stop offset=\"1\" stop-color=\"#"); b.extend_from_slice(&hex6(c0)); push_b(&mut b, b"\" stop-opacity=\"0.4\"/>");
    ...
    push_b(&mut b, b"<line x1=\""); push_i(&mut b, p.layout.pad_l);
    push_b(&mut b, b"\" y1=\""); push_i(&mut b, p.layout.axis_y);
    push_b(&mut b, b"\" x2=\""); push_i(&mut b, p.layout.pad_l + p.layout.plot_w);
    push_b(&mut b, b"\" y2=\""); push_i(&mut b, p.layout.axis_y);
    ...
    for gi in (0..n_groups).rev() {
        ...
        push_b(&mut b, b" fill=\"#0f172a\"/>");
        area_path(&mut b, &pts, base_y as f64);
        push_b(&mut b, b" fill=\"url(#dlxrlg"); push_i(&mut b, gi as i32);
        push_b(&mut b, b")\" filter=\"url(#dlxrf)\"/>");
        polyline(&mut b, &pts, &hx, 1.8);

        push_b(&mut b, b"<line x1=\""); push_i(&mut b, p.layout.pad_l - 4);
        push_b(&mut b, b"\" y1=\""); push_i(&mut b, base_y);
        push_b(&mut b, b"\" x2=\""); push_i(&mut b, p.layout.pad_l + p.layout.plot_w);
        push_b(&mut b, b"\" y2=\""); push_i(&mut b, base_y);
        push_b(&mut b, b"\" stroke=\"#1e293b\" stroke-width=\"0.5\"/>");
        push_b(&mut b, b"<text x=\""); push_i(&mut b, p.layout.pad_l - 8);
        push_b(&mut b, b"\" y=\""); push_i(&mut b, base_y - p.layout.row_h / 2 + 4);
        push_b(&mut b, b"\" text-anchor=\"end\" font-family=\"-apple-system,Arial,sans-serif\" font-size=\"11\" font-weight=\"600\" fill=\"#94a3b8\">");
        let name_str = if p.group_order[gi].len() > 14 { &p.group_order[gi][..14] } else { &p.group_order[gi] };
