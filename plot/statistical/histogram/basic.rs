...
use crate::plot::statistical::common::{
    escape_xml, hex6, push_b, push_f2, push_i, sorted, svg_legend_item, Frame,
};
...
pub fn render(cfg: &HistogramConfig) -> String {
    ...
    if cfg.count_scale > 1 {
        for c in bin_counts.iter_mut() {
            *c *= cfg.count_scale as u64;
        }
    }
    ...
    if n_bins > 0 && cfg.sort_order != "none" && !cfg.sort_order.is_empty() {
        ...
        if let Some(ref oc) = overlay_counts {
            overlay_counts = Some(sorted(&idx, oc));
        }
    ...
    let max_count = {
        ...
        if let Some(ref oc) = overlay_counts {
            m = m.max(*oc.iter().max().unwrap_or(&0) as f64);
        }
        m.max(1.0)
    };
    let pad_l: i32 = 52;
    ...
    for (i, &cnt) in bin_counts.iter().enumerate() {
        ...
        f.buf.extend_from_slice(&hx_main);
