...
pub fn render(cfg: &HistogramConfig) -> String {
    ...
    let n_curve = 96usize;
    let xs: Vec<f64> = (0..n_curve).map(|i| x_min + (x_max - x_min) * i as f64 / (n_curve as f64 - 1.0)).collect();
    let kde = gaussian_kde(cfg.values, &xs);

    let max_d = densities.iter().cloned().fold(0.0_f64, f64::max)
    ...
    let mut f = Frame::new_html(cfg.title, cfg.width, cfg.height, pad_l, 36, 46, pad_r, n_bins * 240 + n_curve * 32 + 2048);
    ...
    for (i, (&xv, &yv)) in xs.iter().zip(kde.iter()).enumerate() {
        ...
        if i == 0 { push_b(&mut f.buf, b"M"); } else { push_b(&mut f.buf, b" L"); }
        push_i(&mut f.buf, px);
        push_b(&mut f.buf, b" ");
        push_i(&mut f.buf, py);
    }
