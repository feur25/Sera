use super::common::{compute_bins, group_indices, bin_to_edges};
...
pub fn render(cfg: &HistogramConfig) -> String {
    ...
    for g in 0..n_groups { for b in 0..n_bins { totals[b] += series[g][b]; } }
    ...
    for b in 0..n_bins {
        ...
        for g in 0..n_groups {
            ...
            let hx = hex6(col);
            push_b(&mut f.buf, b"<rect data-idx=\""); push_i(&mut f.buf, b as i32);
            push_b(&mut f.buf, b"\" data-series=\""); push_i(&mut f.buf, g as i32);
            push_b(&mut f.buf, b"\" data-lbl=\""); escape_xml(&mut f.buf, &groups[g]);
            push_b(&mut f.buf, b"\" data-kv-Range=\""); push_f2(&mut f.buf, edges[b]); f.buf.extend_from_slice("\u{2013}".as_bytes()); push_f2(&mut f.buf, edges.get(b+1).copied().unwrap_or(edges[b]));
            ...
            push_b(&mut f.buf, b"\" fill-opacity=\"0.85\" stroke=\"#fff\" stroke-width=\"0.4\"/>");
            acc_h += bh;
        }
    }
    ...
    if has_legend {
        ...
        for (gi, name) in groups.iter().enumerate() {
            ...
            svg_legend_item(&mut f.buf, gi as i32, name, col, lx, f.pt + 4 + (gi as i32) * 20, 18);
        }
    }
    let _ = bin_to_edges;
