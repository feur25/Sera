...
use crate::plot::statistical::common::{
    escape_xml, hex6, push_b, push_f2, push_i, sort_indices, sorted, svg_legend_item, truncate,
    Frame,
};
...
pub fn data_dot(
    ...
    push_i(&mut f.buf, (i as i32) * 2 + series);
    push_b(&mut f.buf, b"\" data-series=\"");
    push_i(&mut f.buf, series);
    push_b(&mut f.buf, b"\" data-y=\"");
    push_f2(&mut f.buf, if series == 0 { p.start[i] } else { p.end[i] });
