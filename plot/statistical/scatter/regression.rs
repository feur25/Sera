...
fn fit_poly2(xs: &[f64], ys: &[f64]) -> Option<(f64, f64, f64)> {
    ...
    let c0 = det3([[b[0], m[0][1], m[0][2]], [b[1], m[1][1], m[1][2]], [b[2], m[2][1], m[2][2]]]) / d;
    let c1 = det3([[m[0][0], b[0], m[0][2]], [m[1][0], b[1], m[1][2]], [m[2][0], b[2], m[2][2]]]) / d;
...
pub fn render(cfg: &ScatterConfig) -> String {
    ...
    let steps = 60usize;
    let xmin = layout.xmin2; let xmax = layout.xmax2;
    ...
    let (curve, equation) = match cfg.regression_type {
        "polynomial2" | "poly2" | "quadratic" => {
            if let Some((c0, c1, c2)) = fit_poly2(xs, ys) {
                let pts: Vec<(f64, f64)> = (0..=steps).map(|k| {
    ...
    if !curve.is_empty() {
        ...
        if curve.len() >= 2 {
            ...
            push_b(&mut tmp, b"<path fill=\"none\" stroke=\"#"); tmp.extend_from_slice(&hx);
            push_b(&mut tmp, b"\" stroke-width=\"2.4\" stroke-dasharray=\"6,4\" stroke-linecap=\"round\" d=\"");
            for (k, &(xv, yv)) in curve.iter().enumerate() {
                let (px, py) = point_px(&layout, &f, xv, yv);
                if k == 0 { tmp.push(b'M'); } else { tmp.push(b'L'); }
                push_i(&mut tmp, px); tmp.push(b' '); push_i(&mut tmp, py);
            }
            push_b(&mut tmp, b"\"/>");
        }
    ...
    if !equation.is_empty() {
        let tx = f.pl + 12;
        let ty = f.pt + 18;
        push_b(&mut f.buf, b"<rect x=\""); push_i(&mut f.buf, tx - 6);
        push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, ty - 13);
        push_b(&mut f.buf, b"\" rx=\"4\" width=\""); push_i(&mut f.buf, (equation.len() as i32) * 7 + 14);
