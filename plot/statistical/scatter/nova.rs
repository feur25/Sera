...
pub fn render(cfg: &ScatterConfig) -> String {
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"nvgf\" x=\"-200%\" y=\"-200%\" width=\"600%\" height=\"600%\">",
    );
    push_b(
        &mut f.buf,
        b"<feGaussianBlur stdDeviation=\"5\" result=\"b\"/>",
    );
    push_b(
        &mut f.buf,
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"nvcore\" x=\"-120%\" y=\"-120%\" width=\"340%\" height=\"340%\">",
    );
    push_b(
        &mut f.buf,
    ...
    for i in 0..layout.n {
        ...
        push_f2(&mut f.buf, cy as f64 - spike);
        ...
        push_b(
            &mut f.buf,
            b"\" stroke-width=\"1.2\" stroke-opacity=\"0.6\" filter=\"url(#nvgf)\"/>",
        );

        push_b(&mut f.buf, b"<line x1=\"");
        push_f2(&mut f.buf, cx as f64 - spike);
        push_b(&mut f.buf, b"\" y1=\"");
        push_i(&mut f.buf, cy);
        push_b(&mut f.buf, b"\" x2=\"");
        push_f2(&mut f.buf, cx as f64 + spike);
        ...
        push_b(&mut f.buf, b"<line x1=\"");
        push_f2(&mut f.buf, cx as f64 - spike2 * 0.707);
        push_b(&mut f.buf, b"\" y1=\"");
        push_f2(&mut f.buf, cy as f64 - spike2 * 0.707);
        push_b(&mut f.buf, b"\" x2=\"");
        push_f2(&mut f.buf, cx as f64 + spike2 * 0.707);
        push_b(&mut f.buf, b"\" y2=\"");
        push_f2(&mut f.buf, cy as f64 + spike2 * 0.707);
        ...
        push_b(
            &mut f.buf,
            b"\" stroke-width=\"0.8\" stroke-opacity=\"0.45\" filter=\"url(#nvgf)\"/>",
        );
        ...
        push_b(
            &mut f.buf,
            b"\" fill-opacity=\"1\" filter=\"url(#nvcore)\"/>",
        );
