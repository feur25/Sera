...
pub fn render(cfg: &ScatterConfig) -> String {
    ...
    let legend_w: i32 = if has_legend { 160 } else { 20 };
    ...
    for i in 0..layout.n {
        ...
        let (col, sym) = if has_legend && i < cfg.categories.len() {
            let ci = cat_order.iter().position(|c| c == &cfg.categories[i]).unwrap_or(0);
            let s = if i < cfg.symbols.len() && !cfg.symbols[i].is_empty() {
                cfg.symbols[i].as_str()
            } else { cycle_symbol(ci) };
            (palette_color(cfg.palette, ci), s)
        } else {
            let s = if i < cfg.symbols.len() && !cfg.symbols[i].is_empty() {
                cfg.symbols[i].as_str()
            } else { cycle_symbol(i) };
        ...
        push_b(&mut f.buf, b"\">");
        draw_marker(&mut f.buf, sym, cx, cy, cfg.point_size, &hx, &hx, cfg.stroke_width.max(1.2), 0.78);
    ...
    if has_legend {
        ...
        for (li, name) in cat_order.iter().enumerate() {
            ...
            let hx = hex6(col);
            let sym = cycle_symbol(li);
            let ly = f.pt + 10 + (li as i32) * 22;
            draw_marker(&mut f.buf, sym, lx + 8, ly, 6.0, &hx, &hx, 1.2, 0.85);
            pb(&mut f.buf, b"<text x=\""); push_i(&mut f.buf, lx + 22);
            pb(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, ly + 4);
            pb(&mut f.buf, b"\" font-family=\"Arial,sans-serif\" font-size=\"11\" fill=\"#cbd5e1\">");
