...
pub fn render(cfg: &ScatterConfig) -> String {
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"stlgf\" x=\"-150%\" y=\"-150%\" width=\"400%\" height=\"400%\">",
    );
    push_b(
        &mut f.buf,
        b"<feGaussianBlur stdDeviation=\"4.5\" result=\"b\"/>",
    );
    push_b(
        &mut f.buf,
    ...
    push_b(
        &mut f.buf,
        b"<radialGradient id=\"stlhaze\" cx=\"30%\" cy=\"70%\" r=\"55%\">",
    );
    push_b(
        &mut f.buf,
        b"<stop offset=\"0\" stop-color=\"#F59E0B\" stop-opacity=\"0.18\"/>",
    );
    push_b(
        &mut f.buf,
        b"<stop offset=\"1\" stop-color=\"#0f172a\" stop-opacity=\"0\"/>",
    );
    ...
    for i in 0..layout.n {
        ...
        push_b(
            &mut f.buf,
            b"\" fill-opacity=\"0.93\" filter=\"url(#stlgf)\"/>",
        );
