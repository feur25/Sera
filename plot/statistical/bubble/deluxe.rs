...
pub fn render(cfg: &BubbleConfig) -> String {
    ...
    push_b(
        &mut f.buf,
        b"<filter id=\"irdsf\" x=\"-50%\" y=\"-50%\" width=\"200%\" height=\"200%\">",
    );
    push_b(
        &mut f.buf,
        b"<feGaussianBlur stdDeviation=\"6\" result=\"b\"/>",
    );
    push_b(
        &mut f.buf,
    ...
    for i in 0..n {
        ...
        push_b(
            &mut f.buf,
            b"\" cx=\"32%\" cy=\"28%\" r=\"72%\" fx=\"32%\" fy=\"28%\">",
        );
        push_b(&mut f.buf, b"<stop offset=\"0\" stop-color=\"#");
        f.buf.extend_from_slice(&hex6(pale));
        push_b(&mut f.buf, b"\" stop-opacity=\"0.98\"/>");
        push_b(&mut f.buf, b"<stop offset=\"0.3\" stop-color=\"#");
        f.buf.extend_from_slice(&hex6(bright));
        push_b(&mut f.buf, b"\" stop-opacity=\"0.82\"/>");
        push_b(&mut f.buf, b"<stop offset=\"0.65\" stop-color=\"#");
        ...
        push_b(&mut f.buf, b"\" stop-opacity=\"0.22\"/>");
        push_b(&mut f.buf, b"</radialGradient>");
        push_b(&mut f.buf, b"<radialGradient id=\"irdsh");
        push_i(&mut f.buf, i as i32);
        push_b(&mut f.buf, b"\" cx=\"38%\" cy=\"30%\" r=\"35%\">");
        push_b(
            &mut f.buf,
            b"<stop offset=\"0\" stop-color=\"#ffffff\" stop-opacity=\"0.7\"/>",
        );
        push_b(
            &mut f.buf,
            b"<stop offset=\"1\" stop-color=\"#ffffff\" stop-opacity=\"0\"/>",
        );
        push_b(&mut f.buf, b"</radialGradient>");
    }
    push_b(&mut f.buf, b"</defs>");
    ...
    for &i in &layout.indices {
        ...
        push_f2(&mut f.buf, r * 1.2);
        push_b(&mut f.buf, b"\" fill=\"#");
        f.buf.extend_from_slice(&hx);
        push_b(
            &mut f.buf,
            b"\" fill-opacity=\"0.12\" filter=\"url(#irdsf)\"/>",
        );
        push_b(&mut f.buf, b"<circle cx=\"");
        push_i(&mut f.buf, cx);
        push_b(&mut f.buf, b"\" cy=\"");
        ...
        push_b(&mut f.buf, b"\" r=\"");
        push_f2(&mut f.buf, r);
        push_b(&mut f.buf, b"\" fill=\"url(#irdrg");
        push_i(&mut f.buf, i as i32);
        push_b(&mut f.buf, b")\" stroke=\"#");
        f.buf.extend_from_slice(&hx);
        push_b(
            &mut f.buf,
            b"\" stroke-width=\"0.8\" stroke-opacity=\"0.45\"/>",
        );
        push_b(&mut f.buf, b"<circle cx=\"");
        push_i(&mut f.buf, cx);
        push_b(&mut f.buf, b"\" cy=\"");
        ...
        push_b(&mut f.buf, b"\" r=\"");
        push_f2(&mut f.buf, r);
        push_b(&mut f.buf, b"\" fill=\"url(#irdsh");
        push_i(&mut f.buf, i as i32);
        push_b(&mut f.buf, b")\"/>");
