...
pub fn render(cfg: &BubbleConfig) -> String {
    ...
    for &i in &layout.indices {
        ...
        let col = if use_groups {
            let ci = cat_order.iter().position(|c| i < cfg.categories.len() && c == &cfg.categories[i]).unwrap_or(0);
        ...
        push_b(&mut f.buf, b"\" fill-opacity=\"0.55\" stroke=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stroke-width=\""); push_f2(&mut f.buf, cfg.stroke_width);
        push_b(&mut f.buf, b"\"/>");
    }

    for &i in &layout.indices {
        let label = if i < cfg.labels.len() { cfg.labels[i].as_str() } else { continue; };
