...
pub fn render(cfg: &BubbleConfig) -> String {
    ...
    let pos_hex = if cfg.color_hex != 0 { cfg.color_hex } else { cfg.color_high };
    ...
    for &i in &layout.indices {
        ...
        push_b(&mut f.buf, b"\" data-s=\""); push_f2(&mut f.buf, cfg.sizes[i]);
        if i < cfg.labels.len() {
        ...
        if positive {
            ...
            push_b(&mut f.buf, b"\" fill=\"#"); f.buf.extend_from_slice(hx);
            push_b(&mut f.buf, b"\" fill-opacity=\"0.25\" stroke=\"#"); f.buf.extend_from_slice(hx);
            push_b(&mut f.buf, b"\" stroke-width=\""); push_f2(&mut f.buf, cfg.stroke_width.max(2.0));
    ...
    let ly = f.pt + 6;
    push_b(&mut f.buf, b"<rect x=\""); push_i(&mut f.buf, lx);
    push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, ly);
    push_b(&mut f.buf, b"\" width=\"12\" height=\"12\" fill=\"#"); f.buf.extend_from_slice(&pos_hx);
    push_b(&mut f.buf, b"\" fill-opacity=\"0.6\"/>");
    push_b(&mut f.buf, b"<text x=\""); push_i(&mut f.buf, lx + 18);
    push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, ly + 10);
    push_b(&mut f.buf, b"\" font-family=\"Arial,sans-serif\" font-size=\"11\" fill=\"#cbd5e1\">positive</text>");
    push_b(&mut f.buf, b"<rect x=\""); push_i(&mut f.buf, lx);
    push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, ly + 20);
    push_b(&mut f.buf, b"\" width=\"12\" height=\"12\" fill=\"#"); f.buf.extend_from_slice(&neg_hx);
    push_b(&mut f.buf, b"\" fill-opacity=\"0.25\" stroke=\"#"); f.buf.extend_from_slice(&neg_hx);
    push_b(&mut f.buf, b"\" stroke-width=\"1.5\" stroke-dasharray=\"4,3\"/>");
    push_b(&mut f.buf, b"<text x=\""); push_i(&mut f.buf, lx + 18);
    push_b(&mut f.buf, b"\" y=\""); push_i(&mut f.buf, ly + 30);
    push_b(&mut f.buf, b"\" font-family=\"Arial,sans-serif\" font-size=\"11\" fill=\"#cbd5e1\">negative</text>");
