...
fn plasma_color(i: usize) -> u32 {
    const COLS: [u32; 8] = [
        0x00E5FF, 0xFF00C8, 0x00FF88, 0xFFD600,
        0xFF4D00, 0x7B00FF, 0x00FFD0, 0xFF0055,
    ];
    COLS[i % COLS.len()]
}
...
pub fn render(cfg: &BubbleConfig) -> String {
    ...
    push_b(&mut f.buf, b"<filter id=\"plsf\" x=\"-80%\" y=\"-80%\" width=\"360%\" height=\"360%\">");
    ...
    push_b(&mut f.buf, b"<filter id=\"plscore\" x=\"-100%\" y=\"-100%\" width=\"400%\" height=\"400%\">");
    ...
    for &i in &layout.indices {
        ...
        push_b(&mut f.buf, b"\" r=\""); push_f2(&mut f.buf, r * 2.2);
        push_b(&mut f.buf, b"\" fill=\"none\" stroke=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stroke-width=\"1\" stroke-opacity=\"0.12\" filter=\"url(#plsf)\"/>");

        push_b(&mut f.buf, b"<circle cx=\""); push_i(&mut f.buf, cx);
        push_b(&mut f.buf, b"\" cy=\""); push_i(&mut f.buf, cy);
        push_b(&mut f.buf, b"\" r=\""); push_f2(&mut f.buf, r * 1.6);
        push_b(&mut f.buf, b"\" fill=\"none\" stroke=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stroke-width=\"1.5\" stroke-opacity=\"0.25\" filter=\"url(#plsf)\"/>");

        push_b(&mut f.buf, b"<circle cx=\""); push_i(&mut f.buf, cx);
        push_b(&mut f.buf, b"\" cy=\""); push_i(&mut f.buf, cy);
        push_b(&mut f.buf, b"\" r=\""); push_f2(&mut f.buf, r * 1.15);
        push_b(&mut f.buf, b"\" fill=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" fill-opacity=\"0.06\" stroke=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" stroke-width=\"2\" stroke-opacity=\"0.55\" filter=\"url(#plsf)\"/>");

        push_b(&mut f.buf, b"<circle cx=\""); push_i(&mut f.buf, cx);
        push_b(&mut f.buf, b"\" cy=\""); push_i(&mut f.buf, cy);
        push_b(&mut f.buf, b"\" r=\""); push_f2(&mut f.buf, r * 0.55);
        push_b(&mut f.buf, b"\" fill=\"#"); f.buf.extend_from_slice(&hx);
        push_b(&mut f.buf, b"\" fill-opacity=\"0.9\" filter=\"url(#plscore)\"/>");
