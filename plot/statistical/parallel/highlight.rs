...
pub fn render(cfg: &ParallelConfig) -> String {
    ...
    let hi = if cfg.highlight_index >= 0 { cfg.highlight_index as usize } else { 0 };
    for si in 0..p.n_series {
        let v = &cfg.series_values[si];
        let is_hi = si == hi;
        let col = if is_hi { palette_color(cfg.palette, si) } else { 0xCBD5E1 };
        ...
        if op_back > 0.0 { write_polyline(&mut b, &p, v, col, sw_back, op_back, si, &cfg.series_names[si]); }
        write_polyline(&mut b, &p, v, col, sw_main, op_main, si, &cfg.series_names[si]);
        if is_hi { write_dots(&mut b, &p, v, col, 4.0, 1.0, si); }
    }
    legend(&mut b, cfg, &p);
    finalize(b, cfg)
}
