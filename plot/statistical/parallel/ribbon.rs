use super::common::{finalize, point, prepare};
use super::config::ParallelConfig;
use crate::plot::statistical::common::{
    escape_xml, hex6, palette_color, push_b, push_f2, push_i, svg_open, svg_title,
};
...
pub fn render(cfg: &ParallelConfig) -> String {
    ...
    for ai in 0..p.n_axes {
        ...
        push_i(&mut b, x);
        push_b(&mut b, b"\" y=\"");
        push_i(&mut b, p.pad_t + p.plot_h + 18);
    ...
    for si in 0..p.n_series {
        ...
        let hx = hex6(col);
        push_b(&mut b, b"<linearGradient id=\"rbng");
        ...
        push_b(&mut b, b"<stop offset=\"40%\" stop-color=\"#");
        b.extend_from_slice(&hx);
    ...
    for si in 0..p.n_series {
        ...
        for seg in 0..p.n_axes.saturating_sub(1) {
            ...
            push_i(&mut b, si as i32);
            push_b(&mut b, b"\" data-series=\"");
            push_i(&mut b, si as i32);
            push_b(&mut b, b"\" data-lbl=\"");
            escape_xml(&mut b, &cfg.series_names[si]);
            push_b(&mut b, b"\" fill=\"url(#rbng");
            push_i(&mut b, si as i32);
            push_b(&mut b, b")\" stroke=\"#");
            b.extend_from_slice(&hx);
            push_b(
                &mut b,
                b"\" stroke-width=\"0.5\" stroke-opacity=\"0.5\" d=\"",
            );
            push_b(&mut b, b"M ");
            push_f2(&mut b, x1);
            push_b(&mut b, b" ");
            push_f2(&mut b, y1 - half_w);
            push_b(&mut b, b" C ");
            push_f2(&mut b, c1x);
            push_b(&mut b, b" ");
            push_f2(&mut b, y1 - half_w);
        ...
        push_b(
            &mut b,
            b"\" stroke-width=\"2\" stroke-opacity=\"0.9\" stroke-linejoin=\"round\" points=\"",
        );
        for ai in 0..p.n_axes {
            if ai >= v.len() {
                break;
    ...
    for (li, name) in cfg.series_names.iter().enumerate() {
        ...
        push_b(&mut b, b"\" y=\"");
        push_i(&mut b, ly);
        push_b(&mut b, b"\" width=\"12\" height=\"7\" rx=\"2\" fill=\"#");
        b.extend_from_slice(&hx);
        ...
        push_b(
            &mut b,
            b"\" font-family=\"Arial,sans-serif\" font-size=\"10\" fill=\"#475569\">",
        );
