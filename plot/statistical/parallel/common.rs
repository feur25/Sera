...
use crate::plot::statistical::common::{
    escape_xml, hex6, palette_color, push_b, push_f2, push_i, svg_legend_item, svg_open, svg_title,
};
...
pub fn axes_grid(buf: &mut Vec<u8>, cfg: &ParallelConfig, p: &Prepared) {
    for ai in 0..p.n_axes {
        ...
        push_i(buf, x);
        push_b(buf, b"\" y=\"");
        push_i(buf, p.pad_t + p.plot_h + 18);
        ...
        push_i(buf, x);
        push_b(buf, b"\" y=\"");
        push_i(buf, p.pad_t + p.plot_h + 32);
...
pub fn legend(buf: &mut Vec<u8>, cfg: &ParallelConfig, p: &Prepared) {
    for (li, name) in cfg.series_names.iter().enumerate() {
        ...
        svg_legend_item(
            ...
            name,
            col,
            cfg.width - p.pad_r + 14,
            p.pad_t + 4 + (li as i32) * 20,
            20,
        );
    }
}
...
pub fn write_polyline(
    ...
    let hx = hex6(color);
    push_b(buf, b"<polyline data-idx=\"");
...
pub fn write_dots(
    ...
    for ai in 0..p.n_axes.min(vals.len()) {
        let (x, y) = point(p, ai, vals[ai]);
        push_b(buf, b"<circle data-idx=\"");
        push_i(buf, (si * p.n_axes + ai) as i32);
        push_b(buf, b"\" data-series=\"");
        push_i(buf, si as i32);
        push_b(buf, b"\" cx=\"");
        push_f2(buf, x);
