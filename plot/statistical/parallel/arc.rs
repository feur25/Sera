...
pub fn render(cfg: &ParallelConfig) -> String {
    ...
    for si in 0..p.n_series {
        ...
        let hx = hex6(col);
        push_b(&mut b, b"<linearGradient id=\"arcg");
        ...
        push_b(&mut b, b"<stop offset=\"50%\" stop-color=\"#");
        b.extend_from_slice(&hx);
    ...
    for si in 0..p.n_series {
        ...
        for seg in 0..p.n_axes.saturating_sub(1) {
            ...
            push_b(&mut b, b"\" fill=\"none\" stroke=\"url(#arcg");
            push_i(&mut b, si as i32);
            push_b(
                &mut b,
                b")\" stroke-width=\"2.5\" stroke-opacity=\"0.85\" d=\"M ",
            );
            push_f2(&mut b, x1);
            push_b(&mut b, b" ");
            push_f2(&mut b, y1);
            push_b(&mut b, b" C ");
            push_f2(&mut b, ctrl1x);
            push_b(&mut b, b" ");
            push_f2(&mut b, y1);
            push_b(&mut b, b" ");
            push_f2(&mut b, ctrl2x);
            push_b(&mut b, b" ");
            push_f2(&mut b, y2);
            push_b(&mut b, b" ");
            push_f2(&mut b, x2);
    ...
    for (li, name) in cfg.series_names.iter().enumerate() {
        ...
        push_b(&mut b, b"\" y=\"");
        push_i(&mut b, ly);
        push_b(&mut b, b"\" width=\"12\" height=\"3\" rx=\"1\" fill=\"#");
        b.extend_from_slice(&hx);
        push_b(&mut b, b"\"/>");
        push_b(&mut b, b"<text x=\"");
        push_i(&mut b, lx + 16);
