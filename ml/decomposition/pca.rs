use crate::ml::linalg::{col_means, dot, svd_randomized, svd_truncated, symeig};
use rayon::prelude::*;
...
impl PCA {
    ...
    pub fn fit(&mut self, x: &[f64], n: usize, p: usize) {
        ...
        let use_covariance = !use_randomized
            && match self.svd_solver.as_str() {
                "full" => false,
                _ => n >= p * 4,
            };
        ...
        if use_randomized {
            ...
            if n * p >= 100_000 {
                ...
                for i in 0..n {
                    for j in 0..p {
                        centered[i * p + j] = x[i * p + j] - self.mean[j];
                    }
                }
            }
        ...
        self.bias = (0..k.min(self.components.len() / p.max(1)))
            .map(|c| dot(&self.mean, &self.components[c * p..(c + 1) * p]))
            .collect();
    }
    ...
    pub fn transform(&self, x: &[f64], n: usize, p: usize) -> Vec<f64> {
        ...
        if n * p * k < 200_000 {
            for i in 0..n {
                ...
                for c in 0..k {
                    out[i * k + c] =
                        (dot(xi, &comps[c * p..(c + 1) * p]) - bias[c]) * whiten_scale[c];
                }
            }
            return out;
        }
        ...
        if need_post {
            out.par_chunks_mut(k).for_each(|row| {
                for c in 0..k {
                    row[c] = (row[c] - bias[c]) * whiten_scale[c];
                }
            });
        }
        out
        ...
        if n * p * k < 200_000 {
            for i in 0..n {
                for c in 0..k {
                    let mut s = 0.0;
                    for j in 0..p {
                        s += x[i * p + j] * self.components[c * p + j];
                    }
                    out[i * k + c] = s;
                }
            }
