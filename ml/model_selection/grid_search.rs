...
pub fn grid_search_parallel<F>(n_combos: usize, folds: &[FoldData], eval_fn: F) -> GridSearchResult
where
    F: Fn(usize, &FoldData) -> f64 + Send + Sync,
{
    let all_scores: Vec<Vec<f64>> = (0..n_combos)
        ...
        .map(|combo_idx| {
            folds
                .iter()
                .map(|fold| eval_fn(combo_idx, fold))
                .collect::<Vec<f64>>()
        })
        .collect();
...
fn finalise_results(
    all_scores: Vec<Vec<f64>>,
    combo_indices: Option<&[usize]>,
) -> GridSearchResult {
    let cv_results: Vec<f64> = all_scores
        .iter()
        .map(|scores| scores.iter().sum::<f64>() / scores.len() as f64)
        .collect();

    let cv_std: Vec<f64> = all_scores
        .iter()
        .zip(cv_results.iter())
        .map(|(scores, &mean)| {
            let var =
    ...
    let (best_local_idx, &best_score) = cv_results
        .iter()
        .enumerate()
...
pub fn gs_elasticnet_reg(
    ...
    let mut m = crate::ml::linear::elastic_net::ElasticNet::new(
        alpha,
        l1_ratio,
        max_iter,
        tol,
...
pub fn gs_sgd_cls(
    fold: &FoldData,
    loss: crate::ml::linear::sgd::SGDLoss,
    alpha: f64,
    max_iter: usize,
    tol: f64,
    lr: f64,
    ...
    let mut m =
        crate::ml::linear::sgd::SGDClassifier::new(loss, alpha, max_iter, tol, lr, fit_intercept);
    m.fit(&fold.x_train, fold.n_train, fold.p, &fold.y_train_i);
    let preds = m.predict(&fold.x_test, fold.n_test, fold.p);
    crate::ml::metrics::classification::accuracy_score(&fold.y_test_i, &preds)
}
...
pub fn gs_dt_cls(
    ...
    criterion: crate::ml::tree::decision_tree::TreeCriterion,
) -> f64 {
    let mut m = crate::ml::tree::decision_tree::DecisionTreeClassifier::new(
        max_depth,
        min_samples_split,
        min_samples_leaf,
        max_features,
...
pub fn gs_dt_reg(
    ...
    let mut m = crate::ml::tree::decision_tree::DecisionTreeRegressor::new(
        max_depth,
        min_samples_split,
        min_samples_leaf,
        max_features,
...
pub fn gs_rf_cls(
    ...
    min_samples_split: usize,
    min_samples_leaf: usize,
    max_features: crate::ml::tree::random_forest::MaxFeatures,
) -> f64 {
    let mut m = crate::ml::tree::random_forest::RandomForestClassifier::new(
        n_estimators,
        max_depth,
        min_samples_split,
        min_samples_leaf,
...
pub fn gs_rf_reg(
    ...
    let mut m = crate::ml::tree::random_forest::RandomForestRegressor::new(
        n_estimators,
        max_depth,
        min_samples_split,
        min_samples_leaf,
...
pub fn gs_knn_cls(
    fold: &FoldData,
    k: usize,
    weights: crate::ml::neighbors::knn::KnnWeights,
) -> f64 {
...
pub fn gs_gb_cls(
    ...
    let mut m = crate::ml::tree::gradient_boosting::GradientBoostingClassifier::new(
        n_estimators,
        learning_rate,
        max_depth,
        min_samples_split,
...
pub fn gs_gb_reg(
    ...
    let mut m = crate::ml::tree::gradient_boosting::GradientBoostingRegressor::new(
        n_estimators,
        learning_rate,
        max_depth,
        min_samples_split,
...
fn gu(names: &[String], vals: &[String], key: &str, default: usize) -> usize {
    names
        .iter()
        .position(|n| n == key)
        .and_then(|i| vals[i].parse::<f64>().ok().map(|v| v as usize))
        .unwrap_or(default)
}
fn gb(names: &[String], vals: &[String], key: &str, default: bool) -> bool {
    names
        .iter()
        .position(|n| n == key)
        .map(|i| vals[i] == "true" || vals[i] == "True" || vals[i] == "1")
        .unwrap_or(default)
}
...
pub fn eval_model(
    ...
    match estimator {
        "Ridge" => gs_ridge_reg(
            fold,
            gf(n, v, "alpha", 1.0),
            gb(n, v, "fit_intercept", true),
        ),
        "Lasso" => gs_lasso_reg(
            fold,
            gf(n, v, "alpha", 1.0),
            gu(n, v, "max_iter", 1000),
            gf(n, v, "tol", 1e-4),
            gb(n, v, "fit_intercept", true),
        ),
        "ElasticNet" => gs_elasticnet_reg(
            fold,
            gf(n, v, "alpha", 1.0),
            gf(n, v, "l1_ratio", 0.5),
            gu(n, v, "max_iter", 1000),
        ...
        "LogisticRegression" => gs_logistic_cls(
            fold,
            gf(n, v, "C", 1.0),
            gu(n, v, "max_iter", 100),
            gf(n, v, "tol", 1e-4),
        ...
        "DecisionTreeClassifier" => {
            let criterion = if gs(n, v, "criterion", "gini") == "entropy" {
            ...
            let mf = n
                .iter()
                .position(|k| k == "max_features")
                .and_then(|i| vals[i].parse::<usize>().ok());
            gs_dt_cls(
                fold,
                gu(n, v, "max_depth", 10),
                gu(n, v, "min_samples_split", 2),
                gu(n, v, "min_samples_leaf", 1),
                mf,
                criterion,
            )
        }
        ...
        "RandomForestClassifier" => {
            let mf = match gs(n, v, "max_features", "sqrt") {
                ...
                s => s
                    .parse::<usize>()
                    .map(crate::ml::tree::random_forest::MaxFeatures::Fixed)
                    .unwrap_or(crate::ml::tree::random_forest::MaxFeatures::Sqrt),
            };
            gs_rf_cls(
                fold,
                gu(n, v, "n_estimators", 100),
                gu(n, v, "max_depth", 10),
                gu(n, v, "min_samples_split", 2),
                gu(n, v, "min_samples_leaf", 1),
                mf,
        ...
        "GradientBoostingClassifier" => gs_gb_cls(
            fold,
            gu(n, v, "n_estimators", 100),
            gf(n, v, "learning_rate", 0.1),
            gu(n, v, "max_depth", 3),
            gu(n, v, "min_samples_split", 2),
            gu(n, v, "min_samples_leaf", 1),
        ),
        "GradientBoostingRegressor" => gs_gb_reg(
            fold,
            gu(n, v, "n_estimators", 100),
            gf(n, v, "learning_rate", 0.1),
            gu(n, v, "max_depth", 3),
        ...
        "AdaBoostClassifier" => gs_adaboost_cls(
            fold,
            gu(n, v, "n_estimators", 50),
            gf(n, v, "learning_rate", 1.0),
            gu(n, v, "max_depth", 1),
        ),
        "AdaBoostRegressor" => gs_adaboost_reg(
            fold,
            gu(n, v, "n_estimators", 50),
            gf(n, v, "learning_rate", 1.0),
            gu(n, v, "max_depth", 1),
        ...
        "KNeighborsClassifier" => {
            let w = if gs(n, v, "weights", "uniform") == "distance" {
                crate::ml::neighbors::knn::KnnWeights::Distance
            } else {
                crate::ml::neighbors::knn::KnnWeights::Uniform
            };
...
pub fn subsample_fold(fold: &FoldData, n_sub: usize) -> FoldData {
    ...
    FoldData {
        ...
        y_train_f: if fold.y_train_f.is_empty() {
            Vec::new()
        } else {
            fold.y_train_f[..n].to_vec()
        },
        y_test_f: fold.y_test_f.clone(),
        y_train_i: if fold.y_train_i.is_empty() {
            Vec::new()
        } else {
            fold.y_train_i[..n].to_vec()
        },
...
pub fn compute_gram_cache(fold: &FoldData) -> GramCache {
    ...
    for i in 0..p {
        for j in 0..p {
            xtx_c[i * p + j] -= nf * x_mean[i] * x_mean[j];
        }
    }
    let mut xty_c = xty;
    for j in 0..p {
        xty_c[j] -= nf * x_mean[j] * y_mean;
    }

    let mut gram_s = xtx_c.clone();
    for i in 0..p {
        for j in 0..p {
            gram_s[i * p + j] /= x_std[i] * x_std[j];
        }
    }
...
pub fn compute_knn_dist_cache(fold: &FoldData, max_k: usize) -> KnnDistCache {
    ...
    if is_cls {
        let mut model = crate::ml::neighbors::knn::KNeighborsClassifier::new(
            max_k,
            crate::ml::neighbors::knn::KnnWeights::Uniform,
        );
        ...
        let mut model = crate::ml::neighbors::knn::KNeighborsRegressor::new(
            max_k,
            crate::ml::neighbors::knn::KnnWeights::Uniform,
        );
...
pub fn compute_irls_cache(fold: &FoldData) -> IrlsCache {
    ...
    let classes = if is_binary {
        [classes_vec[0], classes_vec[1]]
    } else {
        [0, 0]
    };
    let y_bin = if is_binary {
        y.iter()
            .map(|&v| if v == classes[1] { 1.0 } else { 0.0 })
            .collect()
    } else {
        Vec::new()
    };
...
pub fn compute_caches(
    ...
    if needs_gram_cache(estimator) {
        folds
            .iter()
            .map(|f| ModelCache::Gram(compute_gram_cache(f)))
            .collect()
    } else if needs_knn_cache(estimator) {
        let max_k = param_names
            .iter()
            .position(|n| n == "n_neighbors")
            .and_then(|i| {
                param_values[i]
                    .iter()
        ...
        folds
            .iter()
            .map(|f| ModelCache::Knn(compute_knn_dist_cache(f, max_k)))
            .collect()
    } else if needs_irls_cache(estimator) {
        folds
            .iter()
            .map(|f| ModelCache::Irls(compute_irls_cache(f)))
            .collect()
    } else {
...
fn gs_ridge_gram(fold: &FoldData, g: &GramCache, alpha: f64, fit_intercept: bool) -> f64 {
    ...
    if fit_intercept {
        ...
        for i in 0..p {
            for j in 0..p {
                a[i * p + j] += nf * g.x_mean[i] * g.x_mean[j];
            }
            a[i * p + i] += alpha;
        }
        let mut b = g.xty_c.clone();
        for j in 0..p {
            b[j] += nf * g.x_mean[j] * g.y_mean;
        }
...
fn gs_elasticnet_gram_eval(
    ...
    if fit_intercept {
        ...
        for i in 0..p {
            for j in 0..p {
                g[i * p + j] += nf * gc.x_mean[i] * gc.x_mean[j];
            }
        }
        let mut bv = gc.xty_c.clone();
        for j in 0..p {
            bv[j] += nf * gc.x_mean[j] * gc.y_mean;
        }
...
pub fn eval_model_cached(
    ...
    match (estimator, cache) {
        ("Ridge", ModelCache::Gram(g)) => gs_ridge_gram(
            fold,
            g,
            gf(n, v, "alpha", 1.0),
            gb(n, v, "fit_intercept", true),
        ),
        ("Lasso", ModelCache::Gram(g)) => gs_lasso_gram_eval(
            fold,
            g,
            gf(n, v, "alpha", 1.0),
            gu(n, v, "max_iter", 1000),
        ...
        ("ElasticNet", ModelCache::Gram(g)) => gs_elasticnet_gram_eval(
            fold,
            g,
            gf(n, v, "alpha", 1.0),
            gf(n, v, "l1_ratio", 0.5),
        ...
        ("KNeighborsClassifier", ModelCache::Knn(kc)) => {
            let wt = if gs(n, v, "weights", "uniform") == "distance" {
                crate::ml::neighbors::knn::KnnWeights::Distance
            } else {
                crate::ml::neighbors::knn::KnnWeights::Uniform
            };
        ...
        ("LogisticRegression", ModelCache::Irls(ic)) => gs_logistic_irls_cached(
            fold,
            ic,
            gf(n, v, "C", 1.0),
            gu(n, v, "max_iter", 100),
        ...
        ("LogisticRegression", _) => gs_logistic_irls(
            fold,
            gf(n, v, "C", 1.0),
            gu(n, v, "max_iter", 100),
            gf(n, v, "tol", 1e-4),
...
where
    F: Fn(usize, &FoldData, &ModelCache) -> f64 + Send + Sync,
{
    let all_scores: Vec<Vec<f64>> = (0..n_combos)
        .into_par_iter()
        .map(|ci| {
    ...
    F: Fn(usize, &FoldData, &ModelCache) -> f64 + Send + Sync,
{
    ...
    let batch_size = if task_id.is_some() {
        (remaining.len() / 8 + 1).min(50).max(1)
    } else {
        remaining.len().max(1)
    };
    for batch in remaining.chunks(batch_size) {
        let scores: Vec<(usize, Vec<f64>)> = batch
            .par_iter()
            .map(|&ci| {
                let fs: Vec<f64> = folds
                    .iter()
                    .zip(caches.iter())
                    .map(|(fold, cache)| eval_fn(ci, fold, cache))
                    .collect();
            ...
            .collect();
        for (ci, fs) in scores {
            partial.combo_fold_scores.insert(ci.to_string(), fs);
        }
        if let Some(id) = task_id {
            task_update(
                id,
                &partial,
                partial.combo_fold_scores.len() as f32 / total as f32,
            );
        }
    }
    if let Some(id) = task_id {
    ...
    F: Fn(usize, &FoldData, &ModelCache) -> f64 + Send + Sync,
{
    ...
    let done: std::collections::HashSet<usize> = partial
        ...
        .filter_map(|k| k.parse().ok())
        .collect();
    let remaining: Vec<usize> = combo_indices
        .iter()
        .copied()
...
fn eval_model_generic_reg(
    fold: &FoldData,
    model: &mut dyn crate::ml::MlRegressor,
    scoring: &str,
) -> f64 {
    model.fit(&fold.x_train, fold.n_train, fold.p, &fold.y_train_f);
...
fn eval_model_generic_cls(
    fold: &FoldData,
    model: &mut dyn crate::ml::MlClassifier,
    scoring: &str,
) -> f64 {
    model.fit(&fold.x_train, fold.n_train, fold.p, &fold.y_train_i);
    let preds = model.predict(&fold.x_test, fold.n_test, fold.p);
...
pub fn eval_model_scored(
    ...
    match estimator {
        "Ridge" => {
            let mut m = crate::ml::linear::ridge::Ridge::new(
                gf(n, v, "alpha", 1.0),
                gb(n, v, "fit_intercept", true),
            );
        ...
        "Lasso" => {
            let mut m = crate::ml::linear::lasso::Lasso::new(
                gf(n, v, "alpha", 1.0),
                gu(n, v, "max_iter", 1000),
                gf(n, v, "tol", 1e-4),
                gb(n, v, "fit_intercept", true),
        ...
        "LinearRegression" => {
            let mut m =
                crate::ml::linear::ols::LinearRegression::new(gb(n, v, "fit_intercept", true));
            eval_model_generic_reg(fold, &mut m, scoring)
        }
        "LogisticRegression" => {
            let mut m = crate::ml::linear::logistic::LogisticRegression::new(
                gf(n, v, "C", 1.0),
                gu(n, v, "max_iter", 100),
                gf(n, v, "tol", 1e-4),
                gb(n, v, "fit_intercept", true),
        ...
        "SGDClassifier" => {
            ...
            let mut m = crate::ml::linear::sgd::SGDClassifier::new(
                loss,
                gf(n, v, "alpha", 0.0001),
                gu(n, v, "max_iter", 1000),
                gf(n, v, "tol", 1e-3),
        ...
        "SGDRegressor" => {
            let mut m = crate::ml::linear::sgd::SGDRegressor::new(
                gf(n, v, "alpha", 0.0001),
                gu(n, v, "max_iter", 1000),
                gf(n, v, "tol", 1e-3),
                gf(n, v, "eta0", 0.01),
        ...
        "AdaBoostClassifier" => {
            let mut m = crate::ml::tree::adaboost::AdaBoostClassifier::new(
                gu(n, v, "n_estimators", 50),
                gf(n, v, "learning_rate", 1.0),
                gu(n, v, "max_depth", 1),
            );
        ...
        "AdaBoostRegressor" => {
            let mut m = crate::ml::tree::adaboost::AdaBoostRegressor::new(
                gu(n, v, "n_estimators", 50),
                gf(n, v, "learning_rate", 1.0),
                gu(n, v, "max_depth", 1),
            );
        ...
        "KNeighborsClassifier" => {
            ...
            let mut m =
                crate::ml::neighbors::knn::KNeighborsClassifier::new(gu(n, v, "n_neighbors", 5), w);
            eval_model_generic_cls(fold, &mut m, scoring)
        }
        "KNeighborsRegressor" => {
            let w = if gs(n, v, "weights", "uniform") == "distance" {
            ...
            let mut m =
                crate::ml::neighbors::knn::KNeighborsRegressor::new(gu(n, v, "n_neighbors", 5), w);
            eval_model_generic_reg(fold, &mut m, scoring)
        }
        "GaussianNB" => {
            let mut m = crate::ml::naive_bayes::gaussian::GaussianNB::new();
        ...
        "LinearSVC" => {
            let mut m = crate::ml::svm::svm::LinearSVC::new(
                gf(n, v, "C", 1.0),
                gu(n, v, "max_iter", 1000),
                gf(n, v, "tol", 1e-4),
            );
        ...
        "LinearSVR" => {
            let mut m = crate::ml::svm::svm::LinearSVR::new(
                gf(n, v, "C", 1.0),
                gf(n, v, "epsilon", 0.1),
                gu(n, v, "max_iter", 1000),
                gf(n, v, "tol", 1e-4),
