...
pub fn stratified_kfold_indices(y: &[i32], k: usize, seed: u64) -> Vec<(Vec<usize>, Vec<usize>)> {
    ...
    classes.dedup();
    let mut class_indices: Vec<Vec<usize>> = classes
        .iter()
        .map(|&c| {
            y.iter()
                .enumerate()
...
pub fn group_kfold_indices(groups: &[i32], k: usize) -> Vec<(Vec<usize>, Vec<usize>)> {
    ...
    let mut group_sizes: Vec<(i32, usize)> = unique_groups
        .iter()
        .map(|&g| (g, groups.iter().filter(|&&v| v == g).count()))
        .collect();
    ...
    for (g, sz) in group_sizes {
        ...
        for fi in 1..kk {
            if fold_sizes[fi] < fold_sizes[min_fi] {
                min_fi = fi;
            }
        }
    ...
    (0..kk)
        .map(|fi| {
            ...
            for (i, &g) in groups.iter().enumerate() {
                if group_to_fold.get(&g).copied().unwrap_or(0) == fi {
                    test.push(i);
                } else {
                    train.push(i);
                }
...
fn scatter_rows(
    ...
    if idx.len() * p > 200_000 {
        ...
        y_out.par_iter_mut().enumerate().for_each(|(i, dst)| {
            *dst = y[idx[i]];
        });
    } else {
