...
impl IsolationForest {
    ...
    pub fn fit(&mut self, x: &[f64], n: usize, p: usize) {
        ...
        let k = ((self.contamination * n as f64).round() as usize)
    ...
    pub fn score_samples(&self, x: &[f64], n: usize, p: usize) -> Vec<f64> {
        ...
        (0..n)
            ...
            .map(|i| {
                let row = &x[i * p..i * p + p];
                let avg: f64 = self.trees.iter().map(|t| t.path_length(row)).sum::<f64>()
    ...
    pub fn decision_function(&self, x: &[f64], n: usize, p: usize) -> Vec<f64> {
        self.score_samples(x, n, p)
            .into_iter()
            .map(|s| s - self.threshold_)
            .collect()
    }
