    ...
    pub fn fit(&mut self, x: &[f64], n: usize, p: usize) {
        ...
        if self.with_std {
            ...
            if n >= 50_000 && nc >= 2 {
                ...
                for j in 0..p {
                    self.mean[j] *= inv;
                    self.scale[j] = (sum2[j] * inv - self.mean[j] * self.mean[j])
                        .max(0.0)
                        .sqrt()
                        .max(1e-15);
                }
...
impl MinMaxScaler {
    ...
    pub fn partial_fit(&mut self, x: &[f64], n: usize, p: usize) {
        ...
        self.n_samples_seen += n as u64;
        self.range = (0..p)
            .map(|j| (self.max[j] - self.min[j]).max(1e-15))
            .collect();
    }
    ...
    pub fn fit(&mut self, x: &[f64], n: usize, p: usize) {
        ...
        if n >= 50_000 {
            ...
            for lmax in &partials {
                for j in 0..p {
                    if lmax[j] > self.max_abs[j] {
                        self.max_abs[j] = lmax[j];
                    }
                }
            }
        } else {
        ...
        if n >= 50_000 {
                ...
                .for_each(|(ci, block)| {
                    ...
                    for r in 0..rows {
                        ...
                        let scale = match norm {
                            NormType::L1 => {
                                let s: f64 = xi.iter().map(|v| v.abs()).sum();
                                s.max(1e-15)
                            }
                            NormType::L2 => {
                                let s: f64 = xi.iter().map(|v| v * v).sum();
                                s.sqrt().max(1e-15)
                            }
                            NormType::Max => {
                                let s = xi.iter().map(|v| v.abs()).fold(0.0f64, f64::max);
                                s.max(1e-15)
                            }
                        };
                        let inv = 1.0 / scale;
                        for j in 0..p {
                            block[r * p + j] = xi[j] * inv;
                        }
                    }
                });
        } else {
            for i in 0..n {
                ...
                let scale = match self.norm {
                    ...
                    NormType::Max => {
                        let s = row.iter().map(|v| v.abs()).fold(0.0f64, f64::max);
                        s.max(1e-15)
                    }
                };
                for j in 0..p {
                    out[i * p + j] = row[j] / scale;
                }
            }
        }
        out
