...
pub fn detect_devices() -> Vec<DeviceInfo> {
    ...
    if let Some(d) = detect_cuda() {
        v.push(d);
    }
    if let Some(d) = detect_metal() {
        v.push(d);
    }
    if let Some(d) = detect_rocm() {
        v.push(d);
    }
    v
}
...
pub fn select(backend: Backend) -> DeviceInfo {
    ...
    if matches!(backend, Backend::Auto) {
        ...
        devices
            .into_iter()
            .find(|d| d.backend == backend && d.available)
...
fn active_lock() -> &'static std::sync::Mutex<Backend> {
    ACTIVE.get_or_init(|| std::sync::Mutex::new(Backend::Cpu))
}

pub fn set_active(b: Backend) {
    *active_lock().lock().unwrap() = b;
}
