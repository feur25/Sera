...
pub fn precision_score(y_true: &[i32], y_pred: &[i32], average: Average) -> f64 {
    match average {
        ...
        Average::Macro => {
            ...
            let sum: f64 = classes
                .iter()
                .map(|&c| precision_score(y_true, y_pred, Average::Binary(c)))
                .sum();
...
pub fn recall_score(y_true: &[i32], y_pred: &[i32], average: Average) -> f64 {
    match average {
        ...
        Average::Macro => {
            ...
            let sum: f64 = classes
                .iter()
                .map(|&c| recall_score(y_true, y_pred, Average::Binary(c)))
                .sum();
            sum / classes.len().max(1) as f64
        }
        Average::Weighted => {
...
pub fn f1_score(y_true: &[i32], y_pred: &[i32], average: Average) -> f64 {
    match average {
        ...
        Average::Macro => {
            ...
            let sum: f64 = classes
                .iter()
                .map(|&c| f1_score(y_true, y_pred, Average::Binary(c)))
                .sum();
            sum / classes.len().max(1) as f64
        }
        Average::Weighted => {
...
pub fn confusion_matrix(y_true: &[i32], y_pred: &[i32]) -> (Vec<i32>, Vec<usize>) {
    ...
    for (&t, &p) in y_true.iter().zip(y_pred.iter()) {
        if let (Some(ti), Some(pi)) = (
            classes.iter().position(|&c| c == t),
            classes.iter().position(|&c| c == p),
        ) {
            mat[ti * k + pi] += 1;
        }
    }
...
pub fn jaccard_score(y_true: &[i32], y_pred: &[i32], pos_label: i32) -> f64 {
    let tp = y_true
        .iter()
        .zip(y_pred.iter())
        .filter(|(&t, &p)| t == pos_label && p == pos_label)
        .count();
    let fp = y_true
        .iter()
        .zip(y_pred.iter())
...
pub fn fbeta_score(y_true: &[i32], y_pred: &[i32], beta: f64, average: Average) -> f64 {
    match average {
        ...
        Average::Macro => {
            ...
            let s: f64 = classes
                .iter()
                .map(|&c| fbeta_score(y_true, y_pred, beta, Average::Binary(c)))
                .sum();
...
pub fn roc_curve(
    ...
    if p_total == 0.0 || n_total == 0.0 {
        return (vec![0.0, 1.0], vec![0.0, 1.0], vec![]);
    }
