...
fn persistence_enabled() -> bool {
    ...
    *FLAG.get_or_init(|| {
        std::env::var("SERAPLOT_CHECKPOINT")
...
impl Fp {
    ...
    pub fn bval(mut self, v: bool) -> Self {
        v.hash(&mut self.0);
        self
    }
    ...
    pub fn labels(mut self, d: &[i32]) -> Self {
        ...
        for i in 0..4.min(d.len()) {
            d[i].hash(&mut self.0);
        }
        for i in d.len().saturating_sub(4)..d.len() {
            d[i].hash(&mut self.0);
        }
        self
    }
    ...
    pub fn strings(mut self, d: &[String]) -> Self {
        d.len().hash(&mut self.0);
        for i in 0..2.min(d.len()) {
            d[i].hash(&mut self.0);
        }
        for i in d.len().saturating_sub(2)..d.len() {
            d[i].hash(&mut self.0);
        }
        self
    }
...
impl TaskHandle {
    ...
    pub fn partial(&self) -> &PartialState {
        &self.partial_state
    }

    pub fn cached_result(&self) -> Option<String> {
        if self.has_result_file {
            std::fs::read_to_string(result_path(self.id)).ok()
        } else {
            None
        }
    }
...
pub fn task_is_running(id: u64) -> bool {
    task_read(id)
        .map(|e| matches!(e.status, TaskStatus::Running { .. }))
        .unwrap_or(false)
}
...
pub fn task_cleanup_old(max_age_ms: u64) -> usize {
    ...
    for e in rd.filter_map(|e| e.ok()) {
        ...
        let Ok(task) = serde_json::from_slice::<TaskEntry>(&bytes) else {
            continue;
        };
        if matches!(task.status, TaskStatus::Completed)
            && now.saturating_sub(task.updated_ms) > max_age_ms
        {
...
pub fn checkpoint_load(id: u64) -> Option<CheckpointEntry> {
    ...
    if !entry.partial.weights.is_empty() {
        Some(CheckpointEntry {
            weights: entry.partial.weights,
            iteration: entry.partial.iteration,
        })
    } else {
        None
    }
...
pub fn checkpoint_list() -> Vec<u64> {
    let mut ids: Vec<u64> = mem_store()
        .lock()
        .map(|s| s.keys().copied().collect())
        .unwrap_or_default();
    for e in task_list_all() {
        if !ids.contains(&e.task_id) {
            ids.push(e.task_id);
        }
    }
    ids
