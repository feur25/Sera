...
#[crate::sera_doc(
    category = "Registry",
    en = "Save model to the in-memory registry with name, version, and metadata.",
    fr = "Sauvegarder le modèle dans le registre en mémoire avec nom, version et métadonnées.",
    file = "registry.md"
)]
...
pub fn ml_save_model(input: &str) -> String {
    ...
    let params: std::collections::HashMap<String, String> = v
        .get("params")
        .and_then(|x| serde_json::from_value(x.clone()).ok())
        .unwrap_or_default();
    let metrics: std::collections::HashMap<String, f64> = v
        .get("metrics")
        .and_then(|x| serde_json::from_value(x.clone()).ok())
        .unwrap_or_default();
...
pub fn ml_load_model(input: &str) -> String {
    ...
    match crate::ml::registry::get(name, version) {
        Some(r) => format!(
            r#"{{"found":true,"name":"{}","version":{},"kind":"{}","payload":{}}}"#,
            r.name,
            r.version,
            r.kind,
