...
pub fn jf(v: &serde_json::Value, k: &str, d: f64) -> f64 { v.get(k).and_then(|x| x.as_f64()).unwrap_or(d) }
pub fn ju(v: &serde_json::Value, k: &str, d: usize) -> usize { v.get(k).and_then(|x| x.as_u64()).map(|x| x as usize).unwrap_or(d) }
pub fn jb(v: &serde_json::Value, k: &str, d: bool) -> bool { v.get(k).and_then(|x| x.as_bool()).unwrap_or(d) }
pub fn js<'a>(v: &'a serde_json::Value, k: &str, d: &'a str) -> &'a str { v.get(k).and_then(|x| x.as_str()).unwrap_or(d) }
pub fn yf(v: &serde_json::Value) -> Vec<f64> { v.get("y_train").or_else(|| v.get("y")).and_then(|x| serde_json::from_value(x.clone()).ok()).unwrap_or_default() }
pub fn yi(v: &serde_json::Value) -> Vec<i32> { v.get("y_train").or_else(|| v.get("y")).and_then(|x| serde_json::from_value(x.clone()).ok()).unwrap_or_default() }
...
pub fn pred_x<'a>(xf: &'a [f64], xtf: &'a [f64], nt: usize) -> (&'a [f64], usize) {
    if nt > 0 { (xtf, nt) } else { (xf, 0) }
}

pub fn pred_n(n: usize, nt: usize) -> usize { if nt > 0 { nt } else { n } }

pub fn flat_to_json_str(data: &[f64], n: usize, cols: usize) -> String {
    let mut rows: Vec<Vec<f64>> = Vec::with_capacity(n);
    for r in 0..n { rows.push(data[r * cols..(r + 1) * cols].to_vec()); }
