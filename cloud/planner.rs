...
pub fn plan(n_rows: usize, n_cols: usize, mem_budget_mb: u64) -> ScalePlan {
    ...
    let chunk_rows = if n_cols == 0 {
        ...
        target
            .min(max_rows_in_mem.max(1024))
            .max(1024)
            .min(n_rows.max(1))
    };
    let n_chunks = if chunk_rows == 0 {
        1
    } else {
    ...
    let estimated_seconds = if throughput > 0.0 {
        cells / throughput
    } else {
        0.0
    };
    let strategy = if bytes_total < 64 * 1024 * 1024 {
        "in_memory".into()
    } else if bytes_total < 2u64 * 1024 * 1024 * 1024 {
        "chunked".into()
    } else {
        "streamed".into()
    };
