...
impl CsvChunkReader {
    pub fn open<P: AsRef<Path>>(path: P, chunk_rows: usize, has_header: bool, delimiter: u8) -> std::io::Result<Self> {
        let f = File::open(path)?;
        let mut r = BufReader::with_capacity(1 << 20, f);
        let header = if has_header {
            let mut s = String::new();
            r.read_line(&mut s)?;
            Some(s.trim_end_matches(&['\n', '\r'][..]).split(delimiter as char).map(|x| x.to_string()).collect())
        } else { None };
        Ok(Self { reader: r, chunk_rows: chunk_rows.max(1), header, delimiter, eof: false })
    }
    ...
    pub fn next_chunk(&mut self) -> std::io::Result<Option<Vec<Vec<f64>>>> {
        ...
        while out.len() < self.chunk_rows {
            ...
            let row: Vec<f64> = trimmed.split(self.delimiter as char)
