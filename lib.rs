...
pub use seraplot_macros::{chart_demo, ml_doc, model, params, sera_alias, sera_bind, sera_binding, sera_builder, sera_class, sera_doc, sera_doc_impl, sera_impl, sera_register, sera_sig};
...
pub fn config(font: Option<&str>, font_size: Option<i32>, title_size: Option<i32>, border_radius: Option<i32>, opacity: Option<f64>, responsive: Option<bool>, animation: Option<bool>, animation_duration: Option<i32>, crosshair: Option<bool>, zoom: Option<bool>, tooltip: Option<&str>, locale: Option<&str>, thousands_sep: Option<&str>, margin: Option<i32>, export_button: Option<bool>, palette: Option<Vec<u32>>, background: Option<&str>, gridlines: Option<bool>, text_auto: Option<&Bound<'_, PyAny>>, text_position: Option<&str>, text_angle: Option<i32>, text_font_size: Option<i32>, text_font_color: Option<&str>, uniform_text_min_size: Option<i32>, uniform_text_mode: Option<&str>, bar_corner_radius: Option<&Bound<'_, PyAny>>) {
    use std::sync::atomic::Ordering::Relaxed;
    if let Some(value) = font { if let Ok(mut field) = GLOBAL_FONT.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = font_size { GLOBAL_FONT_SIZE.store(value, Relaxed); }
    if let Some(value) = title_size { GLOBAL_TITLE_SIZE.store(value, Relaxed); }
    if let Some(value) = border_radius { GLOBAL_BORDER_RADIUS.store(value, Relaxed); }
    if let Some(value) = opacity { if let Ok(mut field) = GLOBAL_OPACITY.lock() { *field = if value < 1.0 { Some(value) } else { None }; } }
    if let Some(value) = responsive { GLOBAL_RESPONSIVE.store(value, Relaxed); }
    if let Some(value) = animation { GLOBAL_ANIMATION.store(value, Relaxed); }
    if let Some(value) = animation_duration { GLOBAL_ANIMATION_DURATION.store(value, Relaxed); }
    if let Some(value) = crosshair { GLOBAL_CROSSHAIR.store(value, Relaxed); }
    if let Some(value) = zoom { GLOBAL_ZOOM.store(value, Relaxed); }
    if let Some(value) = tooltip { if let Ok(mut field) = GLOBAL_TOOLTIP.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = locale { if let Ok(mut field) = GLOBAL_LOCALE.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = thousands_sep { if let Ok(mut field) = GLOBAL_THOUSANDS_SEP.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = margin { GLOBAL_MARGIN.store(value, Relaxed); }
    if let Some(value) = export_button { GLOBAL_EXPORT_BTN.store(value, Relaxed); }
    if let Some(value) = background { if let Ok(mut field) = GLOBAL_BACKGROUND.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = palette { if let Ok(mut field) = GLOBAL_PALETTE.lock() { *field = Some(value); } }
    if let Some(value) = gridlines { GLOBAL_GRIDLINES.store(value, Relaxed); }
    if let Some(value) = text_auto {
        let text = if let Ok(boolean) = value.extract::<bool>() {
            if boolean { String::new() } else { reset_text_auto(); return; }
        } else if let Ok(text) = value.extract::<String>() {
            text
        } else {
            return;
        };
        if let Ok(mut field) = GLOBAL_TEXT_AUTO.lock() { *field = Some(text); }
    }
    if let Some(value) = text_position { if let Ok(mut field) = GLOBAL_TEXT_POSITION.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = text_angle { GLOBAL_TEXT_ANGLE.store(value, Relaxed); }
    if let Some(value) = text_font_size { GLOBAL_TEXT_FONT_SIZE.store(value, Relaxed); }
    if let Some(value) = text_font_color { if let Ok(mut field) = GLOBAL_TEXT_FONT_COLOR.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = uniform_text_min_size { GLOBAL_UNIFORM_TEXT_MIN.store(value, Relaxed); }
    if let Some(value) = uniform_text_mode { if let Ok(mut field) = GLOBAL_UNIFORM_TEXT_MODE.lock() { *field = Some(value.to_string()); } }
    if let Some(value) = bar_corner_radius {
        ...
        if let Ok(mut field) = GLOBAL_BAR_CORNER_RADIUS.lock() { *field = Some(radius); }
    }
}
